...
- `docs.careti.ai/` is a multilingual delivery site generated from `docs/` + Caret additions.
...
- **User-facing content**: update `careti-docs/user-guide/**` (KO) and create `.en` variants if English is required.
...
2. Update the Korean guide in `careti-docs/development/**` for human-readable parity.
3. If the feature spec is user-facing, update `careti-docs/features.en/**` (English).
4. Add/refresh links in `careti-docs/development/index.md`.

### B) AI Guide Updates (System Prompt / Behavior)
1. Update SoT rules or system prompt sources under `.agents/context/**`.
2. Update the readable Korean prompt docs under `careti-docs/system-prompts-ko/**`.
3. If prompt behavior changes affect developers, add notes to `careti-docs/development/**`.
...
rg -n "src/caret" .agents/context careti-docs/development || true
