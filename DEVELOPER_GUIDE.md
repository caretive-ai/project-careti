...
확장 프로그램의 TypeScript 코드를 컴파일하고 빌드합니다:

```bash
# Protocol Buffer 컴파일 (필요시)
npm run protos

# 개발용 컴파일 (타입체크 + 린트 + esbuild)
npm run compile

# 프로덕션 빌드 (프로토 + 웹뷰 + esbuild --production)
npm run build
```

**빌드 명령어 선택 가이드:**
- `npm run compile`: 개발 중 코드 검증용 (더 빠름)
- `npm run build`: 완전한 프로덕션 빌드 (F5 디버깅 전 권장)
...
**🔧 빌드 문제 해결:**
만약 빌드 에러가 발생하면:
1. **TypeScript 타겟 확인**: `webview-ui/tsconfig.app.json`에서 `target: "ES2022"` 설정 확인
2. **의존성 재설치**: `npm run install:all`
3. **clean 빌드**: `npm run clean && npm run build`
...
# 🚀 프로덕션 빌드 (프로토 + 웹뷰 + esbuild --production) - 추천!
npm run build

# 🌐 Webview UI 빌드만 (프론트엔드)
...
#### 빌드 명령어 선택 가이드
- **`npm run build`**: **✅ 권장** - 완전한 프로덕션 빌드 (F5 디버깅 전 필수)
- **`npm run compile`**: 개발 중 코드 검증용 (더 빠름)
- **`npm run package`**: 기존 Cline 방식의 VSIX 패키징
- **Protocol Buffer**: MCP 서버 통신을 위한 protobuf 컴파일 자동화
- **esbuild**: 빠른 TypeScript 번들링으로 개발 속도 향상

> **💡 중요**: Caret v3.26.6부터 `npm run build`가 새로 추가되었습니다. 이는 원래 Cline에는 없던 Caret 독자적 개선사항입니다.
