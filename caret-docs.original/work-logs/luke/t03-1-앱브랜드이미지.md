...
설정 파일 분산:
├── caret-assets/brand-config.json    # cline ↔ caret 설정
...
// 하드코딩된 문제
name: "Caret",                    // 고정된 이름
iconPath: "caret-assets/icons/caret_shell_icon.svg"  // 고정된 경로
```
...
- [ ] `caret-assets/icons/` → `brands/caret/assets/icons/` 복사  
- [ ] `caret-assets/brand-config.json` → `brands/caret/brand-config.json` 이동
...
// 기존 (하드코딩)
name: "Caret",
iconPath: "caret-assets/icons/caret_shell_icon.svg"
...
#### 2.2 설정 파일 통일
- [x] `caret-assets/brand-config.json` → `brands/caret/`로 이동
