...
### 2.4 Type Sharing Principle (tsconfig)

> **CRITICAL**: The webview (`webview-ui`) is prohibited from directly referencing the source code of the backend (`src`, `caret-src`). Type information must be shared exclusively through the `src/shared` directory.

**`webview-ui/tsconfig.app.json` Configuration Rules:**

1.  **Use `@shared` Path Alias**: Type definitions (including gRPC-generated types) shared with the backend must be referenced using the `@shared/*": ["../src/shared/*"]` path alias.
2.  **No Direct Backend References**: Do not add path aliases like `@/` or `@caret/*` that point directly to the backend's source directories. This increases coupling between the frontend and backend and can lead to unexpected build errors.
3.  **Adhere to `cline` Standard**: The overall structure and settings of the `tsconfig.app.json` file must always be maintained based on the `cline-latest` configuration.

**Expected Benefits**:

- **Clear Dependencies**: The dependency between the frontend and backend is unified to `src/shared`, clarifying the code structure.
- **Build Stability**: Eliminates unnecessary dependencies, increasing the stability of the webview build and preventing potential errors.
