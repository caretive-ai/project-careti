...
Cline 업스트림에서 간헐적으로 **빌드가 안 되는 버전**을 릴리즈하는 경우가 있습니다. 이런 경우 **불가피한 버그 수정**이 필요할 수 있습니다:

**수정 조건**:

- 명백한 타입 정의 누락, 컴파일 에러 등 **빌드 차단 요소**
- **최소한의 변경**으로 해결 가능한 경우

**수정 방법**:

```typescript
// CARET MODIFICATION: Cline build bug fix - [설명]
// [상세 이유]
...
자세한 가이드는 [업스트림 머징 가이드](../guides/upstream-merging.mdx#-cline-빌드-버그-수정-가이드)를 참조하세요. 3. **개발 요청**: Auth0 관련 기능 개발이 필요한 경우 다음과 같이 진행하세요:
`bash
    # 개발 요청 프로세스
