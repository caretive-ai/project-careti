...
| Area | Cline | Caret |
| --- | --- | --- |
| Prompt management | Hardcoded `system.ts` strings | JSON configs in `caret-src/core/prompts/json/` |
...
## 🏗 Code Scope
- **Core switch**: `src/core/prompts/system-prompt/index.ts` branches to `CaretPromptWrapper` when `modeSystem === "caret"`; otherwise returns Cline registry prompt.
- **Caret modules**: `caret-src/core/prompts/CaretPromptWrapper.ts`, `CaretModeManager.ts`, `CaretJsonAdapter.ts` (JSON parsing + terminology replacement).
- **JSON resources**: `caret-src/core/prompts/json/AGENT_BEHAVIOR_DIRECTIVES.json`, `CHATBOT_BEHAVIOR_DIRECTIVES.json`, `CARET_SYSTEM_INFO.json`.
- **Init guard**: `src/common.ts` must call `JsonTemplateLoader.initialize(<extension>/caret-src/core/prompts/sections)` right after `StateManager.initialize` (avoids “JsonTemplateLoader has not been initialized”).
...
- **Terminology replacement** (`CaretJsonAdapter`): replaces PLAN/ACT with CHATBOT/AGENT inside tool descriptions so users only see Caret wording.

## 🛡️ Cline Independence
- When `StateManager.setMode("cline")`, Cline registry is used directly; CaretPromptWrapper is not touched. 
- Caret prompt code resides in `caret-src/` with no upstream dependency.
