...
## 📋 Overview
Dynamic feature flags for white-label builds (Caret B2C, CodeCenter B2B, etc.). Combines compile-time JSON defaults with runtime delivery to toggle features like account, persona, providers, and cost display.

## 🆚 Improvements vs Cline
| Area | Cline | Caret |
| --- | --- | --- |
| Feature control | Hardcoded | **Dynamic flags** via JSON |
| Environments | Single | **Multi-environment** (Caret/CodeCenter) |
| Delivery | None | Backend injects config into webview on init |

## 🏗 Code Scope
- **Definition**: `caret-src/shared/FeatureConfig.ts`, values in `caret-src/shared/feature-config.json` (replaceable at build time).
...
- `enableCaretAccountFeatures` (runtime) – show Caret account flows. 
...
## 🔧 Configuration
- **Default (built-in)**: fully enabled Caret experience.
- **Custom**: override `caret-src/shared/feature-config.json` (e.g., enterprise build disabling account/persona, forcing `cline` mode, showing only default provider).
