...
⚠️ **analysis-of-102 오류 발견**: `src/shared/api.ts`가 F02로 잘못 분류됨!

### **Backend Files (1개)**

| 파일 경로 | 실제 변경 내용 | 충돌 위험도 | 머지 전략 |
|---|---|---|---|
| `src/shared/Languages.ts` | LLM 언어 → UI 언어 매핑 유틸리티 추가 (110 lines) | **중간** | CARET MODIFICATION 주석 4개 있음, Cline 완전 채택 + 유틸리티 재추가 |

**`src/shared/Languages.ts` 변경 상세:**
- ✅ CARET MODIFICATION 주석 있음
- `UILanguageKey` 타입 추가 (ko, en, ja, zh-CN)
- `LLM_TO_UI_LANGUAGE_MAP` 매핑 객체 추가
- `DIRECT_UI_SUPPORTED_LANGUAGES` 배열 추가
- 언어 옵션 정렬 순서 변경 (UI 지원 언어 우선)
- `isUILanguageSupported()` 유틸리티 함수 추가

### **Frontend Files (3개, 모두 caret-src)**

| 파일 경로 | 실제 변경 내용 | 충돌 위험도 |
|---|---|---|
| `webview-ui/src/caret/context/CaretI18nContext.tsx` | Caret i18n Context Provider | **없음 (caret-src)** |
| `webview-ui/src/caret/utils/i18n.ts` | 커스텀 i18n 구현 (번역 함수, 네임스페이스) | **없음 (caret-src)** |
| `webview-ui/src/caret/hooks/useCaretI18n.ts` | React Hook (언어 전환, 번역) | **없음 (caret-src)** |

### **잘못 분류된 파일 (F02가 아님)**

- ❌ **`src/shared/api.ts`** → F04 (Caret Account) + F09 (Feature Config System)
  - analysis-of-102 오류: "Caret Google API 키 매핑 모델"은 i18n과 무관
  - 실제 변경: ApiHandlerSecrets 삭제 + Caret provider 추가
  - **버그 발견**: ApiProvider enum에 "caret"이 2번 등장 (중복)

### **Merge Conflict Risk Analysis**

#### **🟡 Medium Risk Files (1개)**

**`src/shared/Languages.ts`**
- **위험 요인**: 110 lines 변경, 언어 옵션 순서 변경
- **충돌 시나리오**: Cline이 새 언어 추가 시 순서 충돌 가능
- **해결 전략**:
  ```bash
  # Cline 완전 채택 후 Caret 변경 재적용
  git checkout upstream/main -- src/shared/Languages.ts

  # Caret MODIFICATION 주석과 함께 추가
  # 1. UILanguageKey, LLM_TO_UI_LANGUAGE_MAP 추가
  # 2. isUILanguageSupported() 함수 추가
  # 3. languageOptions 정렬은 선택적 (Cline 기본값 유지 권장)
  ```

#### **🟢 No Risk Files (3개)**

모든 frontend 파일은 `webview-ui/src/caret/` 디렉토리에 있어 충돌 없음.

### **Recommended Merge Strategy**

```bash
# Step 1: Languages.ts Cline 채택 후 최소 침습 수정
git checkout upstream/main -- src/shared/Languages.ts

# Step 2: CARET MODIFICATION 주석과 함께 필수 유틸리티만 추가
# - UILanguageKey 타입 (필수)
# - LLM_TO_UI_LANGUAGE_MAP (필수)
# - isUILanguageSupported() (필수)
# - languageOptions 정렬 변경은 생략 가능 (Cline 따라가기)

# Step 3: caret-src 파일들은 그대로 유지
# (충돌 없음)
```

### **Implementation Checklist**

#### **Backend (Phase 4.4) - ✅ COMPLETED**
- [x] `src/shared/Languages.ts` Cline 원본 채택 확인
- [x] UILanguageKey 타입 추가 (CARET MODIFICATION 주석 포함)
- [x] LLM_TO_UI_LANGUAGE_MAP 추가 (CARET MODIFICATION 주석 포함)
- [x] isUILanguageSupported() 함수 추가
- [x] getUILanguageKey() 함수 추가 (보너스)
- [x] DIRECT_UI_SUPPORTED_LANGUAGES 배열 추가
- [x] languageOptions 정렬 변경 여부 결정 → Cline 순서 유지 (선택적이므로 생략)
- [x] 컴파일 테스트 통과 (TypeScript: 0 errors, Lint: 0 errors)

**Phase 4.4 완료일**: 2025-10-10
**변경 파일**: `src/shared/Languages.ts` (59 lines added)

#### **Frontend (Phase 5.x) - ⏳ PENDING**
- [ ] caret-src 파일들 독립성 검증
- [ ] `webview-ui/src/caret/utils/i18n.ts` 복원
- [ ] `webview-ui/src/caret/context/CaretI18nContext.tsx` 복원
- [ ] `webview-ui/src/caret/hooks/useCaretI18n.ts` 복원
- [ ] locale JSON 파일들 복원 (ko/en/ja/zh)

#### **Out of Scope (F04/F09 관련)**
- [ ] `src/shared/api.ts`를 F04/F09로 재분류
- [ ] ApiProvider enum 중복 "caret" 제거

---

**작성일**: 2025-08-20
**마지막 업데이트**: 2025-10-09 (실제 소스 재검증 완료)
**구현 상태**: ✅ 완전 구현 (대부분 caret-src 독립 파일)
**충돌 위험**: 🟡 **중간** (Languages.ts 1개 파일만 주의 필요)