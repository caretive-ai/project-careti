...
**중요 발견**: F03 파일들은 단순 브랜딩 이상의 변경 포함!
- `disk.ts`: 303 lines diff (동적 브랜드 해결 로직)
- `vscode-lm.ts`: 251 lines diff (타입 리팩토링 + 브랜딩)
- `notifications/index.ts`: `replaceBrand()` 함수 로직 추가

### **Backend API Files (4개)**
| 파일 경로 | 수정 목적 | 충돌 위험도 | 실제 diff | 머지 전략 |
|---|---|---|---|---|
| `src/core/api/index.ts` | B2B 브랜딩 - 환경 변수 기반 BrandedApiProvider 전환 | **중간** | - | caret-src로 Wrapper 분리 |
| `src/core/api/providers/vercel-ai-gateway.ts` | defaultHeaders "Cline" → "Caret" 변경 | **낮음** | - | 브랜딩 문자열만 수정 |
| `src/core/api/providers/vscode-lm.ts` | VS Code LM API + 타입 리팩토링 | **중간** ⚠️ | 251 lines | 타입 중앙화 + 브랜딩 |
| `src/shared/mcp.ts` | McpViewTab에 'brandMarketplace' 추가 | **낮음** | - | 타입 확장만 |

### **Storage & State Files (3개)**
| 파일 경로 | 수정 목적 | 충돌 위험도 | 실제 diff | 머지 전략 |
|---|---|---|---|---|
| `src/core/storage/disk.ts` | 동적 브랜딩 + Persona 지원 | **높음** ⚠️⚠️ | 303 lines | 5개 CARET MODIFICATION, 복잡한 리팩토링 |
| `src/core/storage/state-keys.ts` | 전역 브랜드 모드 시스템 (`globalBrandMode`) | **낮음** | - | 타입 확장만 |
| `src/shared/ExtensionMessage.ts` | ExtensionState에 브랜딩 필드 추가 | **높음** ⚠️ | - | Cline 최신 + Caret 필드 병합 |

### **Integration Files (6개)**
| 파일 경로 | 수정 목적 | 충돌 위험도 | 실제 diff | 머지 전략 |
|---|---|---|---|---|
| `src/integrations/checkpoints/CheckpointUtils.ts` | 에러 메시지 "Cline" → "Caret" | **낮음** | - | 브랜딩 문자열만 |
| `src/integrations/checkpoints/CheckpointGitOperations.ts` | Git 커밋 author "Cline" → "Caret" | **낮음** | - | 브랜딩 문자열만 |
| `src/integrations/terminal/TerminalRegistry.ts` | 터미널 아이콘 Caret 커스텀 설정 | **낮음** | - | 아이콘 경로만 |
| `src/integrations/notifications/index.ts` | replaceBrand() 함수 + 동적 브랜딩 | **중간** ⚠️ | ~40 lines | 1개 CARET MODIFICATION, 로직 추가 |
| `src/integrations/editor/detect-omission.ts` | 도움말 URL을 Caret Wiki로 변경 | **낮음** | - | URL만 수정 (F01에서도 발견) |
| `src/extension.ts` | CaretProviderWrapper 임포트 (이미지 주입) | **중간** | - | Wrapper 패턴 |

### **VS Code Host Files (3개)**
| 파일 경로 | 수정 목적 | 충돌 위험도 | 머지 전략 |
|---|---|---|---|
| `src/hosts/vscode/commit-message-generator.ts` | VS Code context key `caret.isGeneratingCommit` | **낮음** | 네임스페이스만 |
| `src/hosts/vscode/VscodeDiffViewProvider.ts` | Diff 뷰 제목 "Caret's Changes" | **낮음** | 브랜딩 문자열만 |
| `src/hosts/vscode/commandUtils.ts` | 명령어 `caret.focusChatInput` 하드코딩 | **낮음** | 명령어 ID만 |

### **Scripts (1개)**
| 파일 경로 | 수정 목적 | 충돌 위험도 | 머지 전략 |
|---|---|---|---|
| `scripts/report-issue.js` | 이슈 URL Caret GitHub로 변경 | **낮음** | URL만 수정 |
...
1. **`src/shared/ExtensionMessage.ts`**
   - **충돌 원인**: Caret과 Cline 양쪽에서 `ExtensionState` 타입 확장
   - **해결 전략**:
     - Cline 최신 `ExtensionState` 완전 채택
     - Caret 브랜딩 필드는 선택적 (`globalBrandMode?: string`)
     - 타입 확장은 `CaretExtensionState extends ExtensionState` 방식 고려

#### **🟡 Medium Risk Files (3개)**

1. **`src/core/storage/disk.ts`**
   - **전략**: 브랜딩 관련 파일명은 caret-src/utils/branding-paths.ts로 분리

2. **`src/core/api/index.ts`**
   - **전략**: BrandedApiProvider Wrapper를 caret-src에서 구현

3. **`src/extension.ts`**
   - **전략**: CaretProviderWrapper는 진입점만 수정, 나머지는 caret-src에서 구현

#### **🟢 Low Risk Files (13개)**
모든 "Cline" → "Caret" 문자열 변경 파일들은 충돌 위험이 낮습니다:
- 단순 브랜딩 문자열 교체만 수행
- 로직 변경 없음
- Merge 시 Cline 최신 로직 채택 후 브랜딩 문자열만 재적용

### **Recommended Merge Strategy**

```bash
# Step 1: Cline upstream 완전 채택
git checkout upstream/main -- src/integrations/ src/hosts/vscode/

# Step 2: 브랜딩 문자열 일괄 변경 스크립트
./caret-scripts/apply-branding.sh \
  "src/integrations/" \
  "src/hosts/vscode/" \
  --replace "Cline" "Caret"

# Step 3: 동적 브랜딩 유틸리티 분리
mkdir -p caret-src/utils/
# disk.ts, api/index.ts 등의 브랜딩 로직을 유틸로 분리

# Step 4: ExtensionMessage.ts 신중하게 병합
# Cline 타입 + Caret 선택적 필드 추가
```

### **Implementation Checklist**
- [ ] 모든 브랜딩 문자열 변경을 스크립트로 자동화
- [ ] 동적 브랜딩 로직을 caret-src/utils로 분리
- [ ] ExtensionMessage.ts 타입 확장 검증
- [ ] B2B 브랜딩 환경 변수 테스트
- [ ] 컴파일 테스트 통과
- [ ] 모든 UI에서 "Caret" 브랜딩 확인
...
**작성일**: 2025-08-16
**마지막 업데이트**: 2025-10-09 (실제 소스 재검증 완료)
**Phase**: Phase 6-2 (웹뷰 UI 차이점)
**TDD 상태**: ✅ 100% 커버리지 달성
**구현 상태**: ✅ 완전 구현 (통합 브랜딩 자동화 완료)
**충돌 위험**: 🟡🔴 **중간-높음** (disk.ts 303 lines, vscode-lm.ts 251 lines)
