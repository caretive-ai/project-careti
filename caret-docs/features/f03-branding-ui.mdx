...
#### **cline-brand-change.js** - 고성능 Cline ↔ Caret 브랜딩 전환

완전 자동화된 브랜딩 전환 시스템으로 workspace 경로 변경에 따른 컴파일까지 처리합니다.

**주요 기능:**
- ⚡ **자동 빌드**: workspace 경로 변경 후 proto 재생성 및 컴파일 자동 실행
- 🎯 **TDD 검증**: 포괄적 테스트 케이스로 변환 무결성 보장
- 🔄 **양방향 변환**: cline → caret, caret → cline 완벽 지원
- 📋 **상태 관리**: 현재 브랜드 상태 추적 및 복원 기능

**사용법:**
```bash
# Cline에서 Caret으로 브랜딩 변경
node caret-b2b/tools/cline-brand-change.js caret

# Caret에서 Cline으로 되돌리기
node caret-b2b/tools/cline-brand-change.js cline

# 자동 빌드 포함 변환 (권장)
node caret-b2b/tools/cline-brand-change.js caret --build

# 현재 브랜드 상태 확인
node caret-b2b/tools/cline-brand-change.js status
```

**자동 처리 과정:**
1. 🔧 package.json 브랜드 메타데이터 변경 (displayName, author 등)
2. 🖼️ 아이콘 에셋 경로 변환 (assets ↔ caret-assets)
3. 📁 규칙 파일 경로 변환 (.clinerules ↔ .caretrules)
4. 🏗️ 자동 빌드: `npm run compile` 실행 (proto 재생성 포함)
5. ✅ 변환 완료 상태 검증

**개선된 백업 시스템:**
- 🗂️ **통합 관리**: `caret-b2b/backups/` 폴더에 6개 파일만 유지
- ♻️ **중복 제거**: 기존 166개+ 백업파일 완전 정리
- 🎯 **브랜드별 백업**: cline/caret 상태별로 체계적 관리
- 🔄 **자동 감지**: getCurrentBrand() 메소드로 현재 상태 자동 판단
...
**작성일**: 2025-08-16  
**마지막 업데이트**: 2025-08-31 15:30 KST  
**Phase**: Phase 6-2 (웹뷰 UI 차이점)  
**TDD 상태**: ✅ 100% 커버리지 달성  
**구현 상태**: ✅ 완전 구현 (cline-brand-change.js 자동화 + 빌드 통합)
