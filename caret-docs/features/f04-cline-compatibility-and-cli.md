...
Caret은 Cline의 모든 기능(Plan/Act 모드, MCP, Provider 등)을 100% 호환하면서, **Dual Mode System**을 통해 Caret 고유의 기능(JSON 프롬프트, 전용 인증)을 선택적으로 제공한다.  
...
| **시스템 프롬프트** | 프롬프트 레지스트리(`src/core/prompts/system-prompt/*`) | **Dynamic JSON Prompt System** (`caret-src/core/prompts/system`)을 `CaretPromptWrapper`로 분기 |
...
## 테스트 체크리스트 (TDD)
- `mode-system.test.ts`: 글로벌 스테이트 영속화, caret/cline 분기, UI 라벨(Plan/Act).
