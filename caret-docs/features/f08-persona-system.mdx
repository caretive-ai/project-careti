...
**검증 완료**: F08은 **하이브리드 패턴 기반 독립 구현**
- **Backend 래퍼**: CaretProviderWrapper (Cline WebviewProvider 래핑)
- **Frontend 독립**: caret-src 및 webview-ui/src/caret 전용 컴포넌트
- **Proto 정의**: proto/caret/persona.proto (Caret 전용)

### **Caret 전용 파일들 (충돌 없음)**

| 파일 경로 | 역할 | 충돌 위험도 |
|---|---|---|
| `caret-src/core/webview/CaretProviderWrapper.ts` | 하이브리드 패턴 래퍼 | **없음** - Caret 전용 |
| `caret-src/services/persona/*.ts` | 페르소나 초기화/저장 | **없음** - Caret 전용 |
| `caret-src/controllers/persona/*.ts` | gRPC 핸들러 | **없음** - Caret 전용 |
| `proto/caret/persona.proto` | gRPC 서비스 정의 | **없음** - Caret 전용 |
| `webview-ui/src/caret/components/Persona*.tsx` (3개) | UI 컴포넌트 | **없음** - caret/ 디렉토리 |
| `assets/template_characters/*` | 페르소나 이미지/데이터 | **없음** - Caret 전용 |

### **Cline 최소 수정 파일들**

| 파일 경로 | 수정 목적 | 충돌 위험도 | 머지 전략 |
|---|---|---|---|
| `src/extension.ts` | CaretProviderWrapper 임포트 및 초기화 | **중간** | `CARET MODIFICATION` 주석 유지 |
| `src/core/storage/disk.ts` | 페르소나 파일 경로 지원 | **높음** ⚠️ | **F03 참조** (Branding과 공유) |
| `webview-ui/src/components/chat/ChatRow.tsx` | PersonaAvatar 통합 | **중간** | `CARET MODIFICATION` 주석 유지 |
...
cp -r caret-src/services/persona caret-backup/
cp -r caret-src/core/webview caret-backup/

# Step 2: Cline 수정 파일 재적용 (3개)
# - extension.ts: CaretProviderWrapper 초기화 재추가
# - disk.ts: F03과 함께 처리 (Branding + Persona)
# - ChatRow.tsx: PersonaAvatar 통합 재추가

# Step 3: Proto 빌드 및 검증
npm run protos
npm run compile
```

### **Implementation Checklist**
- [ ] Caret 전용 디렉토리 보존 (caret-src/services/persona)
- [ ] CaretProviderWrapper 초기화 재추가 (extension.ts)
- [ ] PersonaAvatar ChatRow 통합 재추가
- [ ] Proto 빌드 시스템 자동화 유지
- [ ] 페르소나 이미지 로딩 테스트
