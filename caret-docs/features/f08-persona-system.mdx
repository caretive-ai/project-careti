...
3. **localStorage 지속성**: 사용자 설정이 브라우저 재시작 후에도 유지됨

```typescript
// ExtensionStateContext.tsx - 초기화 로직
enablePersonaSystem: (() => {
    try {
        const stored = localStorage.getItem("caret-enablePersonaSystem")
        return stored !== null ? JSON.parse(stored) : true // Caret 모드 기본값: true
    } catch (error) {
...
- **동기화**: 설정 변경 시 즉시 localStorage에 저장되어 세션 간 지속성 확보
