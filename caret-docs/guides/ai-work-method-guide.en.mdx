...
**🚨 Core Principle**: **Do not start coding without checking mandatory documents based on task nature.**
...
**🚨 CRITICAL: Task Nature Analysis & Mandatory Document Check**
...
**🚨 Mandatory Documents to Check by Task Nature:**
...
- **File Modification Checklist** (Checklist within `caret-docs/caretrules.en.md`) ⚡ **MANDATORY**
...
- **TDD Mandatory Principles** must be confirmed (RED → GREEN → REFACTOR).
...
5. **Analyze Architecture Patterns** ⚡ **NEW**:
...
6. **Check Technical Constraints**:
    ...
    - Build order when changing Protocol Buffers (`npm run protos` → `npm run compile`).
...
8. **Report to Developer and Obtain Approval** ⚡ **MANDATORY**:
...
**🛑 STOP POINT 1: Test File Location Verification**
...
**🛑 STOP POINT 2: Cline Original File Modification Checklist**
...
**🛑 STOP POINT 3: New File Creation Directory Check**
...
1. **Strict TDD Adherence** ⚡ **MANDATORY**:
    - **Always write tests before** any code changes.
    - Strictly adhere to RED → GREEN → REFACTOR order.
...
- [ ] Strictly adhere to TDD (RED → GREEN → REFACTOR)
- [ ] Declare test-first writing
- [ ] Apply AAA pattern (Arrange, Act, Assert)
- [ ] Include integration tests

## 6. AI Mistake Prevention Summary ⚡ **NEW**
