...
> **Note**: Caret v0.4.0 は Cline v3.38.2 をベースにしています。

### 追加
- **Cline v3.38.2 完全統合**: Claude Opus 4.5 を含む最新のモデルサポートとコア改善を統合。
- **デュアルアカウントシステム**: Caret / Cline アカウントを同時に利用可能。
- **Caret CLI (ベータ)**: LiteLLM 対応の統合 CLI。
- **プロバイダー設定強化**: LiteLLM / BizRouter モデルの自動取得とリアルタイムヘルスチェック。
- **入力履歴・ショートカット**: ターミナル風の履歴ナビゲーションと取消/再開ショートカット。
