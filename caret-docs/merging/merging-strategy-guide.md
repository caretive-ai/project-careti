...
| 기능 | 이유 | 충돌 위험 | 이식 복잡도 |
|------|------|-----------|-------------|
| **Rule Priority System** | 핵심 차별화, 토큰 절약 | 🟡 MEDIUM | 낮음 |
| **Account & Organization** | 비즈니스 로직, 026 완성 | 🟢 LOW | 낮음 |
| **다국어 i18n (30 파일)** | 사용자 경험 핵심 | 🟢 LOW | 중간 |
| **로깅 시스템** | 개발 필수 도구 | 🟢 LOW | 낮음 |
| **Chatbot/Agent Mode** | 핵심 차별화 기능 | 🚨 HIGH | **높음** |
...
### **Phase 2: 기본 브랜딩** ✅
```bash
# ✅ 앱명 변경: Cline → Caret (package.json 전체 브랜딩 완료)
# ✅ 규칙 파일명: .clinerules → .caretrules (디렉토리명 변경 완료)
# ✅ 기본 브랜딩 요소 적용 (walkthrough, commands, categories 모두 완료)
```
...
#### **🚨 HIGH 위험**
1. **Chatbot/Agent Mode**: 5개 이상 파일에 산재된 복잡한 로직
2. **Proto 시스템**: Cline Proto와 Caret Proto 간 충돌
3. **상태 관리**: GlobalState vs ChatSettings 중복 관리
