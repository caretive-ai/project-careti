...
사용자 요청에 따라 **3-way 비교(Base/Cline/Caret)** 및 **테스트 코드 검증**을 재수행했습니다.
...
- **분석**: `// CARET MODIFICATION` 주석과 함께 1줄만 추가되어 **Minimal Invasion** 원칙을 완벽히 준수함.

### 2. `system-prompt/index.ts`
- **Cline Original**: `PromptRegistry`만 사용.
- **Current**: `context.modeSystem === "caret"` 분기 추가.
- **분석**: Caret 모드일 때만 `CaretPromptWrapper`를 로드하고, Cline 모드는 원본 로직을 100% 보존함. **Dual Shape** 반환 구조도 정확함.

### 3. `cli-detector.ts`
- **Cline Original**: `isClineCliInstalled`만 존재.
- **Current**: `isCaretCliInstalled` 함수 추가.
- **분석**: 기존 함수를 건드리지 않고 Caret 전용 감지 함수를 추가하여 충돌 위험 없음.
...
- **Status**: **Caret-Only Component** (Cline 원본에는 존재하지 않음).
- **분석**: Caret 전용 컴포넌트이지만, 내부 로직을 수정하여 `modeSystem`이 'cline'일 경우 Cline 설치 링크를 보여주도록 개선됨. 이는 Caret 확장에서 Cline 모드를 사용할 때도 적절한 가이드를 제공하기 위함으로 적절함.
...
### 1. `caret-src/__tests__/prompt-system/mode-system.test.ts`
- **내용**: `getSystemPrompt` 함수가 `modeSystem` 값('caret' vs 'cline')에 따라 올바른 경로로 분기되는지 검증.
- **평가**: Mocking을 통해 `CaretPromptWrapper`와 `PromptRegistry` 호출 여부를 정확히 테스트하고 있음.

### 2. `caret-src/__tests__/prompt-system/set-prompt-system-mode.test.ts`
- **내용**: `SetPromptSystemMode` 실행 시 `globalState`에 값이 영속화되는지 검증.
- **평가**: `CaretGlobalManager`와 `stateManager`의 상태 변화를 모두 커버하고 있음.
...
특히 3-way 비교 결과, Cline v3.38.1의 변경 사항을 침해하지 않으면서 Caret의 기능을 안전하게 주입한 것이 확인되었습니다.
