...
**작성일:** 2025-11-19
**작성자:** Codex (GPT-5)
**목표:** Cline v3.38.1 전체 코드를 베이스로 다시 도입하고, Caret 고유 기능을 최소 침습으로 재적용해 완전한 병합을 달성한다.

---

## 1. 전략 요약
- **접근법:** `Cline-Base + Caret-Overlay` (Full upstream checkout 후 Caret 변경 재적용)
- **브랜치:** `merge/cline-v3.38.1-attempt2`
- **핵심 원칙:**
  - Upstream 태그(`cline/v3.38.1`)를 그대로 가져와 파일 누락을 허용하지 않는다.
  - **Backend/Proto는 Cline을 기준으로 삼고**, Caret 고유 로직만 재주입해 최소 침습으로 정리한다.
  - **Webview는 Caret 버전을 기준으로 유지**하되, Cline이 분기 이후 업데이트한 구간만 역으로 이식한다(이전 작업과 동일한 방식).
  - Caret 변경은 `CARET MODIFICATION`/`caret-*` 디렉터리 기준으로 추출해 재적용한다.
...
3. **변경 파일 매트릭스 작성 (자동화)**:
   - `git diff --name-only cline/v3.35.0..cline/v3.38.1 > caret-docs/merging/v3.38.1/upstream-files.txt`
   - `git diff --name-only cline/v3.35.0..merge/cline-v3.34.0-method3 > caret-docs/merging/v3.38.1/caret-files.txt`
   - **🚨 개선: `scripts/classify-files.ts` 작성**
     - 7가지 머지 전략 자동 분류:
       - `AUTO_ADOPT` (Upstream-only)
       - `AUTO_KEEP` (Caret-only)
       - `SIMPLE_MERGE` (CARET MODIFICATION만)
       - `COMPLEX_MERGE` (구조 변경)
       - `PROTO_MERGE` (필드 충돌)
       - `UI_MERGE` (컴포넌트 충돌)
       - `MANUAL_REVIEW` (불확실)
     ...
     - 의존성 그래프 생성
4. **Caret 전용 자산 자동 추출**:
   - **🚨 개선: `scripts/extract-caret-mods.ts` 작성**
     - `CARET MODIFICATION` 주석 자동 탐지
     - Proto 1072+ 필드 자동 추출
     - `caret-src/**`, `webview-ui/src/caret/**` 등 브랜딩 자산 목록화
     - 카테고리별 패치 생성
     - 상세 리포트 자동 생성 (caret-mods-report.md)
5. **의존성 분석 (숨은 위험 탐지)**:
...
2. **카테고리별 머지 순서 (의존성 순)**:
   ...
   2. Controller (20개)
      - RPC 핸들러 머지
      - Caret 로직 재적용 (CARET MODIFICATION)
      - BizRouter, Persona 컨트롤러
      → 검증: npm run compile

   3. Services & API (15개)
      - Provider 업데이트 (anthropic, openai 등)
      - BizRouter provider (Caret 전용)
      - Transform 로직
...
3. **충돌 해결 전략 (시나리오별)**:
   - **Proto 필드 충돌**: Cline 필드 채택 + Caret 1072+ 유지
   - **Controller 시그니처 변경**: Cline 변경 수용 + Caret 로직 재삽입
   - **UI 컴포넌트 리팩토링**: Cline 구조 채택 + Caret 컴포넌트 추가
...
7. **Layer 7: 자동화 검증 스크립트**
   ...
   # Caret 수정사항 유지 확인
   npm run verify:caret-mods
   # → All CARET MODIFICATION preserved
   ```

### Phase D. 문서 & 릴리스 커뮤니케이션
1. **CHANGELOG 시나리오**
   - `CHANGELOG-CLINE.md`에 upstream v3.38.1 기능을 기록하고, `CHANGELOG.md` 및 `caret-docs/{ko,ja,zh}/CHANGELOG.md`에는 Caret 릴리스 내역(버전, 날짜, 머지 브랜치, 주요 기능)을 작성한다.
   - 버전 규칙: Major=Cline 대규모 흡수, Minor=Caret 기능 추가/소규모 머지, Patch=버그 수정. Marketplace 실제 배포 날짜만 기록한다.
2. **announcement.json 업데이트**
   - `webview-ui/src/caret/locale/{ko,en,ja,zh}/announcement.json`의 `current`와 `previous`를 업데이트해 Current에는 이번 Cline 머지 기능, Previous에는 직전 Caret 기능을 넣는다.
...
4. **scripts/verify-caret-mods.mjs (Caret 수정사항 검증)**
   ```javascript
   // Phase A에서 추출한 Caret 수정사항 로드
   const caretMods = JSON.parse(
     fs.readFileSync('caret-docs/merging/v3.38.1/caret-mods.json')
   )
   ...
   for (const mod of caretMods) {
     ...
     if (mod.type === 'comment') {
       ...
       if (!content.includes(mod.content)) {
         console.error(`❌ Missing CARET MODIFICATION: ${file}:${mod.lineNumber}`)
         process.exit(1)
       }
     } else if (mod.type === 'field') {
       // Proto 필드 존재 확인
       if (!content.includes(mod.fieldName)) {
         console.error(`❌ Missing Caret field: ${mod.fieldName} in ${file}`)
         process.exit(1)
       }
     }
   }
...
5. **package.json 스크립트 추가**
   ```json
   {
     "scripts": {
       ...
       "verify:caret-mods": "node scripts/verify-caret-mods.mjs",
       "verify:all": "npm run verify:no-deletions && npm run verify:no-stubs && npm run verify:providers && npm run verify:caret-mods"
     }
   }
   ```
...
- [ ] `caret-docs/merging/v3.38.1/attempt-2-plan.md` (본 문서) + 실행 로그 추가 예정
...
| **Caret 자산 유실** | 15% | Critical | `CARET MODIFICATION` 탐색 | → 자동 추출 (`extract-caret-mods.ts`)<br>→ 자동 검증 (`verify:caret-mods`)<br>→ Phase A에서 리포트 생성 |
...
| **🆕 암묵적 Caret 변경 유실** | 2% | Medium | (없음) | → Git blame 분석<br>→ Diff 전수 검사<br>→ 브랜딩/페르소나 체크리스트 |
| **🆕 아키텍처 충돌** | 2% | High | (없음) | → Remote Rules vs Caret Rules 분리<br>→ 네임스페이스 충돌 방지<br>→ 수동 아키텍처 리뷰 |
