...
## 2025년 8월 20일: 페르소나 시스템 마이그레이션 중 타입스크립트 오류 분석 및 해결 계획

### 1. 문제 현황
- **이슈**: `npm run compile` 실행 시, 자동 생성된 `protobus` 및 `host-bridge` 관련 파일에서 100개 이상의 타입스크립트 오류 발생.
- **핵심 원인**: `cline`의 빌드 스크립트가 `proto/caret`라는 별도의 패키지를 인식하지 못하고, 모든 것을 `cline` 네임스페이스 기준으로 처리하려고 하면서 발생.

### 2. 근본 원인 분석
1.  **`proto` 패키지 인식 실패**: `cline`의 빌드 스크립트는 `caret` 패키지의 존재를 모르기 때문에, `caret.PersonaService`를 `cline.PersonaService`로 잘못 처리하고, 관련 타입(`PersonaProfile` 등)을 `cline` 네임스페이스에서 찾으려고 시도함.
2.  **잘못된 경로 생성**: 위 문제로 인해, 핸들러 함수를 `@caret/core/controller` 같은 존재하지 않는 경로에서 `import`하려고 시도함.
3.  **잘못된 해결 시도**: 이 문제를 해결하기 위해 `cline`의 원본 빌드 스크립트를 직접 수정하는 것은 "원본 코드 수정 최소화" 원칙에 위배되며, 향후 머징 시 큰 문제를 유발할 수 있는 위험한 접근임.

### 3. 새로운 해결 계획 (설정 중심)
`cline`의 빌드 시스템을 직접 수정하는 대신, TypeScript 설정과 `proto` 컴파일 옵션을 조정하여 문제를 해결한다.

1.  **`tsconfig.json` 경로 재설정**: `paths` 설정에 `@caret/controllers/*`와 같은 구체적인 경로 별칭을 추가하여, 빌드 스크립트가 생성하는 `import` 구문이 실제 파일 위치(`caret-src/controllers/*`)로 정확히 연결되도록 수정한다.
2.  **`proto` 타입 네임스페이스 문제 해결**: `scripts/build-proto.mjs` 스크립트가 `proto/caret` 폴더를 별도의 패키지로 인식하고, 그 결과물이 `src/shared/proto/index.ts`에 `export * as caret from "./caret"` 형태로 포함되도록 최소한으로 수정한다.
3.  **최종 검증**: `npm run compile`을 다시 실행하여 모든 타입스크립트 오류가 해결되었는지 확인한다.
