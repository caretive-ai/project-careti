...
**1단계: 영어 완전체 완성**
```bash
# cline-latest 원본과 현재 en/[파일명].json 비교
# 누락된 원본 텍스트를 Cline→Caret 브랜딩 적용하여 영어에 추가
# en/[파일명].json 완전체 달성
```

**2단계: 다른 언어 동기화**  
```bash
# 완성된 en/[파일명].json 구조를 기준으로
# ja/zh/ko/[파일명].json에 누락 키 번역 추가
# 4개 언어 완전 구조 통일
```
...
2. **settings.json** 
3. **welcome.json**
4. **chat.json, persona.json, models.json**
5. **announcement.json, validate-api-conf.json, rules.json**

### **🎯 최종 목표**
- ✅ **모든 JSON 파일 영어 완전체 달성**
- ✅ **모든 언어 구조 완전 통일**  
- ✅ **누락 키 0개**
- ✅ **미사용 키 정리**
- ✅ **타입 체크 통과**
