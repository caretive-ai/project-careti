...
### Step2. Cline의 코드를 기반으로 된 Caret-merging 프론트 코드에 i18n 적용 
   ...
   * [ ] **1단계: JSON 파일 4개 모두 {} 로 비우기** - en/ko/ja/zh/common.json 초기화
   * [ ] **2단계: 에러 발생 키들 찾아서 한글로 복구** - 빌드 가능한 상태까지 복구
   * [ ] **3단계: 진척도 스크립트 실행 및 커밋 푸시** - 깔끔한 JSON 상태로 커밋
   * [ ] **4단계: 146개 파일 한글 i18n 완성** - ko/common.json에만 한글 키 추가
   * [ ] **5단계: 3개국어 번역 완성** - ko → en → ja → zh 순서로 번역
