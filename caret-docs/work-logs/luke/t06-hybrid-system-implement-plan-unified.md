...
class CaretJsonComponentProvider {
    ...
    // [GREEN] 기본 JSON 로딩 메커니즘
    private loadJsonSection(sectionName: string): any {
        if (!this.jsonCache.has(sectionName)) {
            const content = fs.readFileSync(`webview-ui/src/caret/locale/ko/system-prompt/${sectionName}.json`);
            this.jsonCache.set(sectionName, JSON.parse(content));
        }
        return this.jsonCache.get(sectionName);
    }
    
    // [GREEN] 단일 어댑터 예시 (SYSTEM_INFORMATION)
    private adaptSystemInfo(): ComponentFunction {
        return async (variant: string, context: any) => {
            const json = this.loadJsonSection('SYSTEM_INFORMATION');
            return json.content || json.default?.content;
        };
    }
    
    // [GREEN] 레지스트리 등록
    public registerCaretComponents(): void {
        this.registry.registerComponent('system_info', this.adaptSystemInfo());
        // TODO: 다른 컴포넌트들 순차 등록
    }
}
