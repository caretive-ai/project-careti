...
## 루크의 고민
 * caret-main 은 caret-docs\features\f06-json-system-prompt.mdx 의 내용이 구현 되어 있음
 * 이를 신규 머징인 이 프로젝트 caret-merging으로 구현해와야함
 * 그런데, \cline-latest가 원본인 이 프로젝트는 caret-main 의 구조와 다르므로 간단한 이식이 되지 않음
 * 그리고 추가적으로  caret-docs\보고서(reports)\성능 및 안정성 검증\caret Json 시스템프롬프트 의미론적 동등성 분석 보고서-20250717.md 에서 추출한 의미론적으로 동일함은 스크립트를 확인 결과 하드코딩된 잘못된 결과로서 완벽히 cline의 기능을 다 포괄하고 있는지에 대해 검증이 어려워졌음. 
 * 고민은 cline-latest의 기능을 caret의 json 프롬프트가 다 동등하게 구현하고 있는가 ? 임
 * 그리고, cline-latest에서 plan모드는 이전과 달리 좀 유연해졌으며, act모드 역시 무식하게 일만 할려 하는것 같지는 않음. 그러면 우리가 목표한바와 비슷해진것 같은데 무엇이 개선되었는지도 궁금함. 그리고 이에 따라 머징 전략을 고민해야함

-------- 아래는 이전 AI가 해당 미션에 대해 분석하고 작업을 도출한 내용 -----
