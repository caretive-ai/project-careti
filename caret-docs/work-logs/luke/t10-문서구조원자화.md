...
### 구조 원칙
1. **1:1 매핑**: .agents/context ↔ caret-docs 완전 대응
2. **분류 원칙**: 
   - 규칙 (JSON) = 불변 핵심 원칙
   - 워크플로우 (MD) = 동적 작업 절차
...
#### 4. **caret-architecture-and-implementation-guide.md**
- Fork 기반 아키텍처 상세
...
- `.agents/context/caret-rules.json` 완전 재구성
...
4. `caret-architecture-and-implementation-guide.md` - Caret 아키텍처 가이드
