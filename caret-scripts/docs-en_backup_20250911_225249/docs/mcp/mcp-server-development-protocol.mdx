...
The heart of effective MCP server development is following a structured protocol. This protocol is implemented through a `.agents/context` file that lives at the **root** of your MCP working directory (/Users/your-name/Documents/Cline/MCP).

### Using `.agents/context` Files

A `.agents/context` file is a special configuration that Cline reads automatically when working in the directory where it's placed. These files:
...
Here's the complete MCP Server Development Protocol that should be placed in your `.agents/context` file:
...
When this `.agents/context` file is present in your working directory, Cline will:
...
### 1. Create a `.agents/context` file (🚨 IMPORTANT)

First, add a `.agents/context` file to the root of your MCP working directory using the protocol above. This file configures Cline to use the MCP development protocol when working in this folder.
