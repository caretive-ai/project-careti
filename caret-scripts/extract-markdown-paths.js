...
function extractMarkdownPaths(obj, paths = new Set()) {
    ...
    if (typeof obj === 'string') {
        ...
        while ((match = markdownRegex.exec(obj)) !== null) {
            ...
            if (!fullPath.includes('.en.') && (fullPath.startsWith('caret-docs/') || fullPath.startsWith('docs/') || fullPath.startsWith('./') || fullPath.startsWith('../') || fullPath.match(/^[a-zA-Z0-9_-]+\.(md|mdx)$/))) {
                paths.add(fullPath);
            }
        }
