...
export class CaretProviderWrapper implements vscode.WebviewViewProvider {
		...
		try {
			...
			webviewView.webview.options = {
				...
				localResourceRoots: [
					...
					vscode.Uri.joinPath(this.context.extensionUri, "caret-assets"),
				],
			}
	...
	private async injectTemplateImagesAsBase64(webviewView: vscode.WebviewView): Promise<void> {
		try {
			...
			const templateDir = path.join(this.context.extensionUri.fsPath, "caret-assets", "template_characters")
	...
	private async handleLoadPersonaImage(message: any, webviewView: vscode.WebviewView): Promise<void> {
		try {
			...
			const fullPath = path.join(this.context.extensionUri.fsPath, "caret-assets", "template_characters", imagePath)
