...
export class PersonaInitializer {
	...
	public async initialize(): Promise<void> {
		...
		try {
			...
			// 		Logger.info("[CARET-PERSONA] PersonaInitializer: 웹뷰에 페르소나 이미지 변경 알림 전송 완료")
			// 	} else {
			// 		Logger.warn("[CARET-PERSONA] PersonaInitializer: CaretProvider 인스턴스를 찾을 수 없어 알림을 보내지 못했습니다")
			// 	}
			// } catch (error) {
			// 	Logger.error(`[CARET-PERSONA] PersonaInitializer: 페르소나 이미지 변경 알림 전송 실패: ${error}`)
			// }
		} catch (error) {
	...
	private async updatePersonaImages(persona: any): Promise<void> {
		try {
			...
			Logger.debug(
				`[CARET-PERSONA] PersonaInitializer: 이미지 경로 변환 - normal: ${normalImagePath}, thinking: ${thinkingImagePath}`,
			)
			...
			// await personaStorage.replacePersonaImageFromTemplate(this.context, "normal", normalImagePath)
			// await personaStorage.replacePersonaImageFromTemplate(this.context, "thinking", thinkingImagePath)

			Logger.info("[CARET-PERSONA] PersonaInitializer: 페르소나 이미지 업데이트 완료")
		} catch (error) {
			Logger.error(`[CARET-PERSONA] PersonaInitializer: 페르소나 이미지 업데이트 실패: ${error}`)
		}
	}
	...
	public async initializeOnLanguageSet(language: string): Promise<void> {
		...
		// 	if (provider) {
		// 		provider.notifyPersonaImagesChanged()
		// 		Logger.info("[CARET-PERSONA] PersonaInitializer: 언어 설정 후 웹뷰에 페르소나 이미지 변경 알림 전송 완료")
		// 	} else {
		// 		Logger.warn(
		// 			"[CARET-PERSONA] PersonaInitializer: 언어 설정 후 CaretProvider 인스턴스를 찾을 수 없어 알림을 보내지 못했습니다",
		// 		)
		// 	}
		// } catch (error) {
		// 	Logger.error(`[CARET-PERSONA] PersonaInitializer: 언어 설정 후 페르소나 이미지 변경 알림 전송 실패: ${error}`)
		// }
	}
}
