...
> **注**: Careti v0.4.8はClaude Codeスタイルのメッセージキューシステムを導入し、v0.4.7の無限ローディング問題を修正しました。

### ✨ 新機能
- **F19 メッセージキューシステム**: AIストリーミング中に次の指示を先行入力できます。キューされた入力は入力フィールド上部にインラインプレビューとして表示されます。
- **ESC即時キャンセル**: ESCキー1回でストリーミングを即座に停止し、キューされた入力をエディターに復元して再送信前に編集できます。
- **インラインキュー編集/削除**: 保留中の入力プレビューに編集（入力に復元）と削除（破棄）ボタンを提供します。
- **CLIエージェント/チャットボットモード**: headless/yoloサポート、EOF再接続、改善された入力処理を含む独立したCLIエージェントおよびチャットボットモードを追加しました。
- **CLIテレメトリー**: PostHogベースのCLI使用量分析イベントトラッキングを追加しました。
- **CLI E2Eテスト**: エージェントモード、CLI動作、出力、インタラクティブモードのE2Eテストを追加しました。
- **clearPendingInput RPC**: 編集/削除ボタンからキューされた入力をクリアするための新しいgRPCエンドポイントを追加しました。

### ✨ 改善
- **Feature flags並列ポーリング**: 逐次フラグ取得（14リクエスト）を5秒の全体タイムアウト付き並列処理に変更しました。
- **ノンブロッキング初期化**: `featureFlagsService.poll()`をfire-and-forget方式に変更し、拡張機能のアクティベーションブロックを防止します。
- **サブエージェントコマンドパターン**: careti/cline CLIサブエージェントサポート用のCARETI_COMMAND_PATTERNを追加しました。

### 修正
- **v0.4.7 無限ローディング**: `data.cline.bot`に到達できない場合、`featureFlagsService.poll()`が拡張機能の初期化をブロックしていた問題をタイムアウト付きノンブロッキングに修正しました。
- **ApiProviderマイグレーション**: v0.4.6からアップグレードしたユーザーの保存設定に残っていた`"caret"` → `"careti"`プロバイダー名マイグレーションを追加しました。
- **APIハンドラーフォールバック**: マイグレーション未適用のプロバイダー値を処理するため、APIプロバイダーswitchに`case "caret":`フォールバックを追加しました。
- **レガシーマイグレーション呼び出し**: `migrateLegacyApiConfigurationToModeSpecific()`が定義されていたが初期化時に呼び出されていなかった問題を修正しました。
- **pendingInputレースコンディション**: `cancelTask()`が`sessionManager.delete()`でセッションを破棄した後に`consumePendingInput`が呼び出されるレースコンディションを修正しました。
- **デバッグログクリーンアップ**: `[CORE-DEBUG]`、`[CLI-DEBUG]`、`[GLM4.7-DEBUG]`コンソールログを削除しました。
