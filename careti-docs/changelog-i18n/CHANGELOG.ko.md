...
> **참고**: Careti v0.4.8은 Claude Code 스타일의 메시지 큐 시스템을 도입하고 v0.4.7 무한 로딩 문제를 수정했습니다.

### ✨ 신규 기능
- **F19 메시지 큐 시스템**: AI 스트리밍 중에 다음 지시를 미리 입력할 수 있습니다. 대기 중인 입력은 입력 필드 상단에 인라인 미리보기로 표시됩니다.
- **ESC 즉시 취소**: ESC 키 한 번으로 스트리밍을 즉시 중단하고, 대기 중이던 입력을 에디터에 복원하여 재전송 전에 편집할 수 있습니다.
- **인라인 큐 편집/삭제**: 대기 입력 미리보기에서 편집(입력으로 복원) 및 삭제(폐기) 버튼을 제공합니다.
- **CLI 에이전트/챗봇 모드**: headless/yolo 지원, EOF 재연결, 개선된 입력 처리를 포함한 CLI 독립 에이전트 및 챗봇 모드를 추가했습니다.
- **CLI 텔레메트리**: PostHog 기반 CLI 사용량 분석 이벤트 트래킹을 추가했습니다.
- **CLI E2E 테스트**: 에이전트 모드, CLI 동작, 출력, 인터랙티브 모드에 대한 E2E 테스트를 추가했습니다.
- **clearPendingInput RPC**: 편집/삭제 버튼에서 대기 입력을 초기화하는 새 gRPC 엔드포인트를 추가했습니다.

### ✨ 개선
- **Feature flags 병렬 폴링**: 순차적 플래그 조회(14건)를 5초 전체 타임아웃의 병렬 처리로 변경했습니다.
- **논블로킹 초기화**: `featureFlagsService.poll()`을 fire-and-forget 방식으로 변경하여 확장 활성화 차단을 방지합니다.
- **서브에이전트 명령 패턴**: careti/cline CLI 서브에이전트 지원을 위한 CARETI_COMMAND_PATTERN을 추가했습니다.

### 버그 수정
- **v0.4.7 무한 로딩**: `data.cline.bot` 접근 불가 시 `featureFlagsService.poll()`이 확장 초기화를 차단하던 문제를 타임아웃 기반 논블로킹으로 수정했습니다.
- **ApiProvider 마이그레이션**: v0.4.6에서 업그레이드 시 저장된 설정에 남아있던 `"caret"` → `"careti"` 프로바이더명 마이그레이션을 추가했습니다.
- **API 핸들러 폴백**: 마이그레이션되지 않은 프로바이더 값을 처리하기 위해 API 프로바이더 switch에 `case "caret":` 폴백을 추가했습니다.
- **레거시 마이그레이션 호출**: `migrateLegacyApiConfigurationToModeSpecific()`이 정의만 되고 초기화 시 호출되지 않던 문제를 수정했습니다.
- **pendingInput 레이스 컨디션**: `cancelTask()`가 `sessionManager.delete()`로 세션을 이미 파괴한 뒤 `consumePendingInput`이 호출되던 레이스 컨디션을 수정했습니다.
- **디버그 로그 정리**: `[CORE-DEBUG]`, `[CLI-DEBUG]`, `[GLM4.7-DEBUG]` 콘솔 로그를 제거했습니다.
