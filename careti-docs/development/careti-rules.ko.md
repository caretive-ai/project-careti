...
### 문서 접근 패턴 (AI 동작 방식)
- **1단계 (초기화)**: AI는 세션 시작 시 `.agents/context/careti-rules.json` (JSON 인덱스)만 읽습니다.
...
- 절차:
  ...
  5. 사람용 문서 갱신은 `careti-docs/development/**`를 기준으로 한다.
...
### 파일 매핑 (동기화 구조)
```
.agents/context/careti-rules.json           ↔ careti-docs/development/careti-rules.ko.md (본 문서)
.agents/context/workflows/*.md             ↔ careti-docs/development/*.md (대응 가이드)
.agents/context/workflows/atoms/*.md        ↔ careti-docs/development/ 내 관련 가이드(예: testing-guide.md)
```
...
- **철학**: Cline 코어 보존, careti-src/를 통한 확장

#### Fork 기반 구조
Careti은 [Cline](https://github.com/cline/cline)의 완전한 Fork입니다:

- **`src/`**: Cline 원본 코드 (최대한 보존)
- **`careti-src/`**: Careti 고유 확장 기능 (최소화)
- **`webview-ui/`**: React 프론트엔드 (Cline 빌드 시스템 활용)

#### Frontend-Backend 상호작용 패턴
Careti의 webview와 Extension Host 간 통신 표준 패턴:
...
### 파일 수정 체크리스트
1. Cline 원본 파일인가?
2. CARETI MODIFICATION 주석 추가했는가?
3. 최대 1-3줄인가?
...
### 개발 도구 (`careti-scripts/tools/`)
- `careti-cline-comparison.js` - Careti vs Cline API 제공자 및 모델 커버리지 비교
...
- Cline 파일 수정: 백업 + CARETI MODIFICATION 주석 필수
- AI는 워크플로우를 통해 개발자와 동일한 정보에 접근해야 함

### 사용 가능한 워크플로우
상세 절차는 `.agents/context/workflows/` 참조:

- **주요 워크플로우**: `ai-work-index.md`, `ai-work-protocol.md`, `careti-development.md`
...
- `comment-protocol.md` - CARETI MODIFICATION 추적
...
2. **새로운 Careti 타입 추가**: 스크립트에 replacement 패턴 추가
3. **필드 번호**: `current_cline_max + 1000` 규칙 사용

### 예시: 새로운 Careti 서비스 타입 추가
...
- 단위 테스트로 시작
- 오래된 코드 주석 처리
- CARETI MODIFICATION 주석 건너뛰기

---

## 문서 구조 (KO 우선, features만 EN 병행)

- **규칙/워크플로우 (AI & 프로젝트 규칙)**: `.agents/context/careti-rules.json`, `.agents/context/workflows/*`
- **개발자 문서 (KO 우선)**: `careti-docs/development/index.md`
- **기능 스펙 (EN 병행 유지)**: `careti-docs/features.en/index.md`

**상호 참조**:
- AI 규칙 인덱스: [.agents/context/careti-rules.json](../../.agents/context/careti-rules.json)
