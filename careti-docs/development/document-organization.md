...
## 핵심 원칙
- **SoT는 `.agents/context`** 입니다. (AI는 `.agents/context/careti-rules.json` → 필요한 워크플로우/원자만 온디맨드 로드)
- 개발자 문서는 **KO 우선**으로 `careti-docs/development/**`에 두고, 진입점은 `careti-docs/development/index.md`로 통일합니다.
- 기능 스펙은 **EN 중심(`careti-docs/features.en/**`)**으로 유지하고, 필요 시 KO 가이드로 링크를 연결합니다.
...
## SoT(규칙/워크플로우) 구조
- 진입점: `.agents/context/careti-rules.json`
- 워크플로우: `.agents/context/workflows/*.md`
- 원자(Atoms): `.agents/context/workflows/atoms/*`

## 개발자 문서(사람용) 구조
- 개발 문서 대시보드: `careti-docs/development/index.md`
- 개발 가이드: `careti-docs/development/**`
- 기능 스펙: `careti-docs/features.en/**` (EN)
- 작업 기록: `careti-docs/work-logs/**`

## 문서 추가/수정 시 체크리스트
1) 문서 종류를 먼저 결정합니다.
- **AI가 따라야 하는 절차/규칙**: `.agents/context`에 반영(SoT 우선)
- **로컬 개발/빌드/테스트/실행 가이드**: `careti-docs/development/**`에 반영
- **제품 기능 스펙**: `careti-docs/features.en/**`에 반영
- **사용자 안내**: `careti-docs/user-guide/**` (필요 시 `.en` 별도 분리)

2) 미러링(중복) 대신 연결을 우선합니다.
- SoT workflow가 있으면, 개발 문서에서는 **핵심만 요약**하고 **SoT로 링크**합니다.
- `careti-docs/development/workflows/**` 같이 과거 미러가 남아 있다면, “Deprecated mirror”로 표기하고 SoT를 가리키도록 유지합니다.

3) 네비게이션을 갱신합니다.
- 새 문서/중요 문서는 `careti-docs/development/index.md`에서 항상 접근 가능해야 합니다.
...
rg -n "npm run (test:backend|clean\\b|CLAUDE\\.md)" .agents/context careti-docs/development || true
rg -n "src/careti" .agents/context careti-docs/development || true
...
## 성공 기준
- `.agents/context`와 `careti-docs/development`가 동일한 현실(경로/스크립트/구조)을 설명함
- `careti-docs/development/index.md`에서 고립 문서 없이 탐색 가능
- 기능 스펙은 `careti-docs/features.en/**`에 있고, 개발 runbook과 중복되지 않음
</detailed_sequence_of_steps>
