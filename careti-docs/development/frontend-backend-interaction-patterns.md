...
## 1. Careti 전용 gRPC 서비스 원칙 (Backend ↔ Webview 통신)
...
이 문서는 Careti 프로젝트에서 **webview(React) ↔ Extension Host(Node.js)** 간의 상호작용을 위한 표준화된 패턴을 제시합니다. UI 언어 설정 구현 과정에서 발견된 **순환 메시지 문제**와 **상태 불일치 문제**를 해결하기 위한 검증된 패턴들을 담고 있습니다.
...
> **⚠️ 주의**: 아래는 순환 메시지 방지를 위한 **권장 패턴**입니다. 현재 구현(`src/core/controller/state/updateSettings.ts`)은 모든 업데이트 후 무조건 `postStateToWebview()`를 호출합니다. 순환 메시지 문제가 발생하는 경우 아래 패턴을 적용하는 것을 고려하세요.

```typescript
// Backend: src/core/controller/state/updateSettings.ts (권장 패턴)
...
// 현재 구현 (실제 코드):
// - 모든 필드를 개별 처리 후 무조건 postStateToWebview() 호출
// - 순환 메시지 방지 로직 없음
```
