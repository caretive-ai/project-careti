# Careti JSON 파일 주석 작성 규칙

**중요:** 표준 JSON은 주석을 지원하지 않습니다. 다음 규칙들은 내부 문서화 및 Careti 전용 변경사항 추적을 위한 것으로, 특히 `cline/webview-ui/package.json` 같은 공유 파일에서 사용됩니다. 이러한 주석들은 린터나 빌드 프로세스에서 문제가 발생할 경우 전처리기에 의해 제거되거나 처리되어야 할 수 있습니다.
...
```json
{
	"dependencies": {
		...
		// 이 패키지들은 Careti WelcomeView 및 기타 향후 UI 컴포넌트에서
...
- **주석 키 형식:** `comment_[원래키이름]_caret` (Careti 관련인 경우) 또는 `comment_[원래키이름]`
- **주석 값:** 설명하는 주석 문자열
...
{
	"someSetting": true,
	"comment_someSetting_caret": "Careti의 향상된 기능 X를 위해 활성화됨."
}
```
...
```json
{
	"_comment_block_start_feature_Y_settings_caret": "Careti의 기능 Y 관련 설정들.",
	"settingA": "valueA",
	"comment_settingA_caret": "기능 Y 전용.",
	"settingB": false,
	"comment_settingB_caret": "기능 Y에서 기본적으로 비활성화됨.",
	"_comment_block_end_feature_Y_settings_caret": "Careti의 기능 Y 설정 끝."
}
```
