# Careti System Prompt Architecture & Implementation Guide

**작성일**: 2025-01-27 (대폭 업데이트)  
**작성자**: Careti Development Team  
**대상**: Careti 개발자, 시스템 프롬프트 작업자, AI 협업 시스템 개발자

## 🎯 **개요**

Careti은 Cline의 707라인 하드코딩된 시스템 프롬프트를 **JSON 기반 모듈형 시스템**으로 완전히 재설계했습니다. 이는 **Chatbot/Agent 모드**, **협력적 AI 태도**, **메타인지 시스템**을 통합하여 Cursor 수준의 자연스러운 협업 경험을 제공합니다.

## ⭐ **핵심 개선사항**

Careti은 Cline 대비 다음과 같은 혁신적 개선을 달성했습니다:
...
```typescript
📁 careti-src/core/prompts/ (Careti JSON 시스템)
├── sections/ (15개 파일) - 모듈형 프롬프트 구조
│   ├── BASE_PROMPT_INTRO.json - Careti 정체성 + Chatbot/Agent 모드
│   ├── COLLABORATIVE_PRINCIPLES.json - 5개 협력 원칙 (Careti 고유)
│   ├── TOOL_DEFINITIONS.json - 15개 도구 완전 보존
│   ├── TOOL_USE_GUIDELINES.json - 협력적 도구 사용
│   ├── CHATBOT_AGENT_MODES.json - Chatbot/Agent 철학 (Careti 고유)
...
2. **비교 검증**: Careti JSON 시스템과 1:1 비교
...
```typescript
import { CaretSystemPrompt } from "@careti-src/core/prompts/CaretSystemPrompt"
...
```typescript
import { ClineFeatureValidator } from "@careti-src/core/verification"
...
```bash
# 검증 시스템 테스트
npm run test careti-src/__tests__/cline-feature-validation.test.ts
...
4. **CARETI MODIFICATION 주석**: 모든 수정에 명확한 주석

#### **예시: BASE_PROMPT_INTRO.json 수정**

```json
{
	"introduction": "You are Careti, an AI coding partner. In AGENT MODE (default), you naturally combine thinking with action to solve problems. In CHATBOT MODE (safety), you provide consultation without system changes. You read context well, work collaboratively, and focus on practical solutions.",
...
Careti Agent 모드:     5,509 tokens (22,036 chars)  → 53.28% 절약 ✨
Careti Chatbot 모드:   3,575 tokens (14,299 chars)  → 69.68% 절약 🚀
```
...
- **완전 구현**: `careti-src/core/prompts/` 15개 JSON 섹션 완료
- **로딩 성능**: < 100ms 목표 달성
...
```typescript
// careti-src/core/prompts/JsonSectionAssembler.ts
private async generateMcpServerSection(mcpHub: any): Promise<string> {
    // CARETI MODIFICATION: JIT Phase 1 - Complete MCP section removal
...
- ✅ **CARETI MODIFICATION 주석**: 모든 수정에 명확한 주석
...
1. **완전한 기능 보존**: Cline 707줄 → Careti JSON 변환 시 0% 기능 손실
...
- **[Prompt System Evolution Report](../보고서(reports)/Careti 시스템 프롬프트 개선 분석 보고서/prompt-system-evolution-report.md)**: 프롬프트 시스템 변화 요약
- **[System Prompt Comparison](../보고서(reports)/Careti 시스템 프롬프트 개선 분석 보고서/system-prompt-comparison-cc-cline-careti.md)**: 비교 분석 보고서
- **[Careti Architecture Guide](./careti-architecture-and-implementation-guide.md)**: 전체 아키텍처
