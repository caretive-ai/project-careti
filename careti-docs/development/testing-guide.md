...
본 문서는 Careti 프로젝트의 테스팅 전략, 작성 방법, 실행 절차에 대한 종합 가이드입니다. Careti은 **100% 테스트 커버리지**를 목표로 하며, **테스트 주도 개발(TDD)** 방식을 권장합니다.
...
**Careti 전용 코드 100% 커버리지 원칙:**

- **🥕 새로운 Careti 로직**: `careti-src/` 및 `webview-ui/src/careti/` 디렉토리의 비즈니스 로직 및 기능 코드는 **100% 테스트 커버리지 필수**입니다.
- **🔗 재수출 파일**: Cline 모듈의 단순 재수출 파일(예: `export { ... } from "..."`)은 테스트에서 제외 가능합니다.
- **📦 타입 정의**: 런타임 로직이 없는 TypeScript 인터페이스/타입 정의만 포함된 파일은 제외 가능합니다.
- **🤖 원본 Cline 코드**: `src/` 및 `webview-ui/src/` (careti 폴더 제외)는 Cline의 기존 테스트를 활용하며, 추가 테스트 작성을 강제하지 않습니다.
- **📊 커버리지 분석**: `careti-scripts/careti-coverage-check.js`를 사용하여 Careti vs. Cline 코드의 커버리지를 별도로 분석합니다.
- **🔍 Cline 코드 수정 테스트**: Cline 소스 코드 수정 시 추가 테스트가 필요한 경우, 가능한 한 `careti-src/__tests__/` 디렉토리에 별도의 테스트 파일을 생성하여 관리하고 테스트 범위의 과도한 확장을 방지합니다.

#### 현재 제외된 파일 (근거 포함)

- `careti-src/core/prompts/system.ts` - Cline 모듈의 재수출.
- `careti-src/shared/providers/types.ts` - TypeScript 인터페이스 정의만 포함.
- `careti-src/core/task/index.ts` - 일부 래퍼 로직 (향후 테스트 추가 예정).
...
- **위치**: `careti-src/__tests__/integration.test.ts`, `src/test/`, `webview-ui/src/__tests__/`.
...
│   │   ├── CaretiProvider.ts
│   │   └── __tests__/
│   │       └── CaretiProvider.test.ts
│   └── utils/
│       ├── careti-logger.ts
│       └── careti-logger.test.ts  // 동일 디렉토리 허용
```

**프론트엔드 테스트 (`webview-ui/src/careti/`)**
