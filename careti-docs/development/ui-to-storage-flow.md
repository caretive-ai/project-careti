...
이 문서는 Careti의 UI와 Storage 간의 완전한 데이터 흐름을 설명합니다. 프론트엔드(React Webview)와 백엔드(VSCode Extension) 간의 데이터 저장, 로드, 시크릿 관리, 이미지 처리, URI 변환 등의 모든 상호작용을 다룹니다.
...
sequenceDiagram
    ...
    participant Provider as CaretiProvider
...
```typescript
// webview-ui/src/careti/hooks/useStorage.ts
...
// webview-ui/src/careti/hooks/useSecrets.ts
...
// webview-ui/src/careti/hooks/useImages.ts
...
```typescript
// careti-src/core/storage/StorageManager.ts
...
```typescript
// careti-src/core/storage/SecretManager.ts
...
```typescript
// careti-src/core/storage/ImageManager.ts
...
```typescript
// careti-src/shared/types/messages.ts
...
```typescript
// careti-src/core/errors/StorageError.ts
...
```typescript
// careti-src/core/storage/BackupManager.ts
...
```typescript
// careti-src/core/storage/CacheManager.ts
...
```typescript
// careti-src/core/storage/BatchProcessor.ts
...
```typescript
// careti-src/core/security/Encryption.ts
...
export class Encryption {
	...
	static encrypt(text: string, key: string): { encrypted: string; iv: string; tag: string } {
		...
		cipher.setAAD(Buffer.from("careti-data"))
	...
	static decrypt(encrypted: string, key: string, iv: string, tag: string): string {
		...
		decipher.setAAD(Buffer.from("careti-data"))
...
```typescript
// careti-src/core/security/AccessControl.ts
...
```typescript
// careti-src/core/storage/__tests__/StorageManager.test.ts
...
```typescript
// webview-ui/src/careti/context/__tests__/ExtensionStateContext.test.tsx
...
- 2025-06-21: 모든 UI-to-storage-flow 문서들을 통합하여 완전한 가이드로 재작성 (.md → .md 변환)
