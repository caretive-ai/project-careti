...
실제 구현 위치: `careti-src/core/storage/StorageManager.ts`
...
실제 구현 위치: `careti-src/core/storage/SecretManager.ts`
...
export class CaretiProvider extends WebviewProvider {
	...
	protected postMessage(message: any): void {
		try {
			...
			caretLogger.debug(`Message sent: ${message.type}`, "CaretiProvider")
		} catch (error) {
			caretLogger.error(`Failed to send message: ${error.message}`, "CaretiProvider")
		}
	}
	...
	protected async handleWebviewMessage(message: any): Promise<any> {
		try {
			caretLogger.debug(`Message received: ${message.type}`, "CaretiProvider")
...
### 4.1 Careti Logger 통합

실제 구현 위치: `careti-src/utils/careti-logger.ts`
...
실제 구현 위치: `webview-ui/src/careti/utils/urls.ts`
...
**지원 URL 카테고리:**
- Careti 서비스 관련 URL (공식 사이트, 계정, 크레딧)  
...
- 2025-06-21: Careti Logger 통합, Vitest 테스트 유틸리티 추가
