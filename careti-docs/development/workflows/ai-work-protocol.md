...
3. **작업 로그 확인**: `careti-docs/work-logs/{username}/{date}.md` 확인/생성
...
🛑 **중지점**: 테스트 파일 위치 확인
- webview: `src/careti/**/*.test.tsx`만 사용
- backend: `careti-src/__tests__/`
- 즉시 검증: 생성 후 테스트 실행
...
🛑 **중지점**: 새 파일 생성 디렉토리 확인
- Careti 기능은 `careti-src/`, `careti-docs/`에 (완전 자유)
