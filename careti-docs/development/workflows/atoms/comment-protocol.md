...
### 올바른 예시:
```typescript
// CARETI MODIFICATION: persona 시스템을 위한 Careti wrapper 초기화
...
export function initialize(context: vscode.ExtensionContext) {
  // CARETI MODIFICATION: Careti provider 초기화 추가
  const caretProvider = new CaretiProvider(context);
...
```typescript
/*
 * CARETI MODIFICATION: persona 인식 메시지 처리 통합
...
### 좋은 설명:
```typescript
// CARETI MODIFICATION: 다중 테넌트 지원을 위한 동적 브랜딩 활성화
// CARETI MODIFICATION: careti-src 테스트를 위한 TDD 통합 테스트 러너 추가
// CARETI MODIFICATION: 시스템 프롬프트 생성에 persona context 구현
```
...
# 히스토리 전체에서 모든 Careti 수정사항을 보여줍니다
```

## 유지보수 가이드라인

### 수정사항 업데이트 시:
- 원래 CARETI MODIFICATION 주석 유지
- 중요한 변경사항일 경우 새 타임스탬프나 버전 추가
- CARETI MODIFICATION 마커 제거 금지
...
<general_guidelines>
이 protocol은 코드베이스 전체에서 모든 Careti 수정사항의 추적을 가능하게 합니다.
