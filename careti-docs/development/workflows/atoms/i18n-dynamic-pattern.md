> ⚠️ (Deprecated mirror) 최신 Atom은 `.agents/context/workflows/atoms/i18n-dynamic-pattern.md`를 기준으로 보세요.
...
function Component() {
    // CARETI MODIFICATION: i18n 컨텍스트로 언어 변경 감지
    const { language } = useCaretI18nContext()

    // CARETI MODIFICATION: 언어 의존성과 함께 동적 함수 사용
...
### 2. Import 추가
```typescript
// CARETI MODIFICATION: i18n 반응성을 위한 useMemo 추가
import { useMemo } from "react"
// CARETI MODIFICATION: 언어 반응성을 위한 i18n 컨텍스트 import
import { useCaretI18nContext } from "@/careti/context/CaretI18nContext"
```
...
// CARETI MODIFICATION: 언어 변경 감지를 위해 i18n 컨텍스트 사용
const { language } = useCaretI18nContext()

// CARETI MODIFICATION: 언어 의존성과 함께 동적 함수 사용하여 i18n 업데이트
const dynamicData = useMemo(() => getDynamicData(), [language])
```
