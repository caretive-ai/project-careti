> ⚠️ (Deprecated mirror) 최신 Atom은 `.agents/context/workflows/atoms/message-flow.md`를 기준으로 보세요.

# Frontend-Backend Message Flow Pattern

## Overview
Standardized patterns for webview (React) ↔ Extension Host (Node.js) communication in Careti.

## Critical: Careti gRPC Service Principle

**NEVER modify Cline's original proto files**. Create separate proto files for Careti-specific communication.

### Required Implementation Steps:

1. **Create `proto/careti/` directory** - Namespace for Careti `.proto` files
2. **Define new `.proto` files** - e.g., `proto/careti/persona.proto` with service and message definitions
3. **Implement independent service** - gRPC handlers in `careti-src`
4. **Register in `extension.ts`** - Add Careti service to gRPC server separately from Cline's `UiService`
...
- **Clarity**: Clear separation of Careti-specific APIs
