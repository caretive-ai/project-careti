...
✅ CaretiProvider.test.tsx   (CaretiProvider.tsx와 매칭)
❌ brandUtilsTest.ts
❌ test-brand-utils.ts
```
...
❌ careti-src/services/PersonaServiceTest.ts (소스 + .test.ts여야 함)
```
...
<general_guidelines>
이러한 convention들은 Careti 확장을 위한 명확한 패턴을 설정하면서 Cline의 기존 코드베이스와의 일관성을 유지합니다.
