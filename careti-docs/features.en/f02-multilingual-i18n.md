...
Careti’s i18n layer supports Korean, English, Japanese, and Chinese with a lightweight, custom implementation in `@/careti/utils/i18n.ts` (no `react-i18next`) for performance and project-specific needs such as Korean postposition handling.
...
| **JSON key** (Frontend) | kebab-case (current) | `vercel-ai-gateway`, `qwen-code` | `webview-ui/src/careti/locale/` |
...
t("providers.careti.name", "settings")
t("message.welcome", "common", { user: "Luke" })

// Korean postposition helper
t("brand.appName|을", "common", {}, "ko")   // → "Careti을"
```
...
- **Unused keys**: `node careti-scripts/tools/report-i18n-unused-key.js`
- **Sync keys**: `npm run sync:i18n-keys` or `node careti-scripts/tools/i18n-key-synchronizer.js`
- **Full sync (delete unused)**: `node careti-scripts/tools/i18n-key-synchronizer.js --delete-unused`
- **Remove unused keys**: `node careti-scripts/tools/remove-i18n-unused-keys.js`
- **Analysis**: `careti-scripts/i18n-checklist-report.md`, `node careti-scripts/tools/find-existing-i18n-key.js`
...
2. Add English source strings in `webview-ui/src/careti/locale/en/`.  
...
- ✅ Careti provider cleanup: removed improper privacy text; restored missing keys; ensured consistency across four languages  
