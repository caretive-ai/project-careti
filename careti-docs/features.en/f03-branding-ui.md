...
Careti’s branding/UI layer is a standalone design system that gives Careti a distinct visual identity while remaining switchable with Cline when needed.

## 📋 Highlights
- **Assets**: careti logo (`^` symbol), dedicated palette, welcome/about/announcement pages, persona illustrations.
- **Separation**: Independent from Cline; can toggle branding per user setting.
- **Consistency**: Shortcut catalog in `webview-ui/src/careti/shortcuts/shortcuts.json` with labels such as “Cancel (Esc)” driven by brand utils + i18n.
...
├── careti-main-banner.webp
├── agent_profile.png
└── template_characters/  # persona images

webview-ui/src/assets/
└── careti-logo.png, CaretLogoWhite.tsx
```
...
| --- | --- | --- | --- |
| OS notification | User | `getCurrentBrandName()` | “Careti wants to…” |
| Chat UI (normal) | User | `getCurrentBrandName()` | “Careti has completed…” |
...
**`getCurrentBrandName()`** (in `careti-src/utils/brand-utils.ts`) reads `package.json` `displayName`, caches it, and falls back to `"Cline"` on error.
...
```css
:root {
  --careti-primary: #6366f1;
  --careti-secondary: #8b5cf6;
  --careti-accent: #10b981;
  --careti-bg-primary: #f8fafc;
  --careti-text-primary: #1e293b;
}
[data-theme="dark"] {
...
- **CaretApiSetup.tsx**: API setup UI with Careti branding.

## 🧪 Tests
- Coverage target: ✅ 100% for core branding components.
- Files: `webview-ui/src/careti/components/__tests__/CaretWelcome.test.tsx`, `CaretWelcomeSection.test.tsx`, `CaretAnnouncement.test.tsx`, `CaretFooter.test.tsx`.
...
- **Phases (summary)**
  ...
  2) Copy assets (`assets/`, `webview-ui/src/assets/`) and Careti styles.
...
- **Completion**: All branding components render; Careti colors/logos visible; multilingual text shows; dark/light supported; tests at 100%.

## 🔄 Branding Switch System
- Supports **Cline ↔ Careti** and **Careti ↔ CodeCenter**, with headroom for more brands.
- Automates:
  - VS Code extension metadata (42+ fields in `package.json`)
  - Icon/banner swaps and terminal branding (name, extension ID)
  - Rule file paths now standardize via `getBrandRulesFileName()` → `.agents/context` (brand-agnostic)
- **VS Code command namespace fix**: converted `cline.*` → `careti.*` to avoid conflicts when both extensions are installed; command titles use `getCurrentBrandDisplayName()`.
- **Add-to chat pipeline**: webview always subscribes to `subscribeToAddToInput` with `EmptyRequest` so “Add to Careti” context flows even after brand switches, while streams stay Careti-only (no Cline cross-talk when both extensions are present).
...
- Changelog separation: Careti release notes stay in `CHANGELOG.md`; upstream Cline release notes are mirrored in `CHANGELOG-CLINE.md` and referenced from localized changelogs.

## 🏢 B2B & Consumer Branding
- B2B: CodeCenter/private branding via the automated switcher.
- Consumer: Quick Careti ↔ Cline toggles without UI conflicts.
