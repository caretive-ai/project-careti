...
Careti의 **다국어 지원 시스템**은 **Sovereign Cloud** 원칙(Provider Country = UI Language Support)에 따라 7개 언어를 지원하는 포괄적인 국제화(i18n) 시스템입니다.

> **중요**: Careti은 기존의 `react-i18next` 라이브러리를 사용하지 않고, `@/careti/utils/i18n.ts`에 구현된 자체 경량화 시스템을 사용합니다. 이는 성능 최적화와 프로젝트 맞춤 기능(예: 한국어 조사 처리)을 위함입니다.

## 📋 **기능 개요**

### **지원 언어 (Sovereign Cloud)**
- 🇰🇷 **한국어 (ko)**: 완전 번역 및 현지화 (조사 자동 처리 포함) - Careti, Upstage, Naver Cloud
...
Careti의 i18n 시스템은 **기능별 네임스페이스**를 기본 원칙으로 합니다. 각 주요 기능(View)은 자신만의 고유한 네임스페이스(JSON 파일)를 가지며, 이를 통해 번역 리소스를 체계적으로 관리하고 유지보수성을 높입니다.
...
| **JSON i18n 키** (Frontend) | 케밥케이스 (현재) | `vercel-ai-gateway`, `qwen-code`, `claude-code` | `webview-ui/src/careti/locale/` |
...
```json
{
  "providers": {
    ...
    "careti": {
      "name": "Careti",
      "description": "Careti provides high-quality AI models...",
      "login": "Login to Careti"
    }
  }
}
```
...
t('providers.careti.name', 'settings')         // -> "Careti"

// 변수 삽입 - t(key, namespace, options)
t('message.welcome', 'common', { user: 'Luke' }) // -> "Luke님, 환영합니다."

// 한국어 조사 처리
t('brand.appName|을', 'common', {}, 'ko')      // -> "Careti을" (받침 있음)
```
...
### **한국어 조사 자동 처리**
Careti의 i18n 시스템은 한국어의 복잡한 조사를 자동으로 처리하여 자연스러운 문장을 생성합니다. (현재는 `ko-postposition` 라이브러리를 통해 구현되어 있지 않지만, 향후 확장될 수 있습니다.)
...
-   **미사용 키 검증**: `node careti-scripts/tools/report-i18n-unused-key.js`
    코드에서 사용되지 않는 번역 키를 찾아냅니다.

#### **🔧 자동화 도구**
-   **키 자동 동기화**: `npm run sync:i18n-keys` 또는 `node careti-scripts/tools/i18n-key-synchronizer.js`
    누락된 키를 자동으로 추가하고 영어(en)를 기본값으로 채웁니다.
-   **완전 동기화 (삭제 포함)**: `node careti-scripts/tools/i18n-key-synchronizer.js --delete-unused`
    누락된 키를 추가하고, 영어에 없는 키를 다른 언어에서 삭제합니다.
-   **미사용 키 자동 삭제 (안전 모드)**: `node careti-scripts/tools/remove-i18n-unused-keys-safe.js`
    동적 키(템플릿 리터럴로 생성되는 키)를 보존하면서 미사용 키를 삭제합니다.
    - 보존되는 동적 키 패턴: `bullets.current.1-5`, `slashCommandMenu.*`, `apiLineOptions.*`, `subagents.*`
-   **미사용 키 자동 삭제 (전체)**: `node careti-scripts/tools/remove-i18n-unused-keys.js`
    사용되지 않는 번역 키를 모든 언어 파일에서 자동으로 삭제합니다. (동적 키 주의)

**i18n-key-synchronizer.js 사용법:**
```bash
# 기본 모드: 누락된 키만 추가
node careti-scripts/tools/i18n-key-synchronizer.js

# 완전 동기화 모드: 누락된 키 추가 + 영어에 없는 키 삭제
node careti-scripts/tools/i18n-key-synchronizer.js --delete-unused
node careti-scripts/tools/i18n-key-synchronizer.js -d
```
...
#### **📊 분석 도구**
-   **i18n 전체 체크리스트**: `careti-scripts/i18n-checklist-report.md`
    i18n 시스템의 전반적인 상태와 권장사항을 종합 분석합니다.
-   **키 사용 현황 분석**: `node careti-scripts/tools/find-existing-i18n-key.js`
    특정 키의 사용 위치와 패턴을 분석합니다.

## 🔄 **번역 추가 및 관리 프로세스**

1.  **네임스페이스 결정**: 추가할 번역이 기존 네임스페이스에 맞지 않으면 새로운 네임스페이스(`{namespace}.json`)를 생성합니다.
2.  **JSON 파일 수정**: `webview-ui/src/careti/locale/en/` 폴더에 기준이 될 영어 번역을 추가합니다.
3.  **다른 언어 파일 동기화**: `ko`, `ja`, `zh`, `fr`, `de`, `ru` 폴더의 해당 네임스페이스 파일에도 동일한 키를 추가하고 번역합니다.
4.  **네임스페이스 등록**: 새 네임스페이스를 생성한 경우, `webview-ui/src/careti/utils/i18n.ts` 파일의 `translations` 객체와 `loadLanguagePack` 함수 내 `Promise.all` 배열에 반드시 추가해야 합니다. **⚠️ 이 과정을 누락하는 것이 가장 흔한 실수입니다.**
...
2. **프로바이더 국가 플래그 표시**
   ...
   - 🇰🇷 한국: Careti, Upstage, Naver Cloud, BizRouter
...
3. **다국어 문서 폴더 구조 개선**
   - ✅ `careti-docs/readme-i18n/`: README.xx.md 형식으로 통합
   - ✅ `careti-docs/changelog-i18n/`: CHANGELOG.xx.md 형식으로 통합
