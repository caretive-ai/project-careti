...
Careti의 **페르소나 시스템**은 AI와의 상호작용을 개인화하는 핵심 기능입니다.  
사전 정의된 캐릭터(캐러티, 오사랑 등)를 선택하거나 커스텀 이미지를 업로드하여, 채팅 UI와 AI 응답에 캐릭터성을 부여합니다.
...
- **`proto/careti/persona.proto`**: 페르소나 데이터 구조 및 서비스 정의.

### 2. Webview (UI)
- **`webview-ui/src/careti/components/PersonaManagement.tsx`**: 페르소나 선택 및 관리 UI.
- **`webview-ui/src/careti/components/PersonaAvatar.tsx`**: CSP 호환 이미지 렌더링 및 상태(Thinking) 반영.
...
| 페르소나                             | 컨셉            | 특성                     | 전문 분야                    |
| ------------------------------------ | --------------- | ------------------------ | ---------------------------- |
| **🤖 캐러티 (Careti)**                  | 코딩 로봇       | 친근하고 도움되는 조수   | 개발자 지원, 문제 해결       |
...
```typescript
// careti-src/services/persona/persona-initializer.ts
...
- **프론트엔드 최소 수정**: ChatRow.tsx에 PersonaAvatar 통합 (`CARETI MODIFICATION` 주석 포함)
...
- **업스트림 호환성**: `CARETI MODIFICATION` 주석을 통한 명확한 수정 지점 표시로 병합 충돌 최소화
...
export class CaretProviderWrapper implements vscode.WebviewViewProvider {
	...
	async resolveWebviewView(webviewView: vscode.WebviewView) {
		...
		// 2. Careti 페르소나 이미지 주입 (window 변수)
...
- **Careti 기능 완전 분리**: 페르소나 시스템은 `cline`의 핵심 코드를 직접 수정하지 않고, `careti-src` 및 `webview-ui/src/careti` 등 전용 디렉토리에 독립적으로 구현
- **전용 gRPC 서비스**: `proto/careti/persona.proto`라는 독립적인 proto 파일을 사용하여 명확하고 안정적인 API 경계 설정
...
- **문서화 우선**: 모든 수정 사항을 `CARETI MODIFICATION` 주석으로 명확히 기록
...
1. **Careti 모드 시**: `modeSystem === "careti"`일 때 기본적으로 활성화
...
- **경로**: Settings → General → Enable Persona System (Careti 모드에서만 표시)
...
enablePersonaSystem: (() => {
    try {
        const stored = localStorage.getItem("careti-enablePersonaSystem")
...
- **해결**: git history에서 깨끗한 버전 복원 후 careti-compare 참고하여 PersonaAvatar 통합 (size=64, flex 레이아웃)
...
const convertAssetToBase64 = async (assetUri: string): Promise<string> => {
	...
	if (assetUri.includes("careti.png") && (window as any).templateImage_caret) {
