...
Careti의 **Feature Config System**은 다양한 배포 환경(일반 사용자용, 엔터프라이즈용 CodeCenter 등)에 맞춰 기능을 동적으로 켜고 끌 수 있는 설정 시스템입니다.  
...
| **배포 환경** | 단일 배포 (Cline) | **Multi-Environment Support**. Careti(B2C)과 CodeCenter(B2B) 등 다양한 환경에 맞는 빌드 구성 지원. |
...
- **`careti-src/shared/FeatureConfig.ts`**: 기능 설정 인터페이스 정의.
- **`careti-src/shared/feature-config.json`**: 실제 설정 값 (빌드 시 교체 가능).
...
- **`webview-ui/src/careti/components/CaretGeneralSettingsSection.tsx`**: `showModeSystemToggle` 플래그에 따른 UI 제어.
...
- **`webview-ui/src/components/settings/providers/CaretiProvider.tsx`**: `showAccountUI` 플래그에 따른 계정 버튼 제어.
...
### **핵심 개념**
- **기본 설정**: 모든 기능이 활성화된 완전한 Careti 경험
- **커스텀 설정**: `.careti-feature-config.json` 파일을 통한 특정 기능 오버라이드
...
| **defaultModeSystem** | "careti" | **최초 설정 (globalState 우선)** | `StateManager.ts:105` ⚠️ 특수<br>Fallback: `state-helpers.ts:601,626,628`<br>`ExtensionStateContext.tsx:247`<br>`CaretSettings.ts:44` |
| **firstListingProvider** | "careti" | 런타임 동적 | `ApiOptions.tsx:202` |
| **defaultProvider** | "careti" | 최초 설정 | `StateManager.ts:94`, `state-helpers.ts:433`, `controller/index.ts:165,466` |
...
| **showAccountUI** | true | 런타임 동적 | `App.tsx:82`<br>`Navbar.tsx:66`<br>`CaretiProvider.tsx:25` |
...
└── components/
    ...
    │   ├── providers/CaretiProvider.tsx      # showAccountUI
    │   └── sections/
...
export interface FeatureConfig {
    /** Careti 계정 관련 기능 활성화 여부 (로그인/가입 UI 등) */
    ...
    /** 기본 모드 시스템 */
    defaultModeSystem: "careti" | "cline"
...
const defaultFeatures: FeatureConfig = {
    ...
    defaultModeSystem: "careti",
    firstListingProvider: "careti",
    defaultProvider: "careti",
...
### **커스텀 설정 (정적 JSON 파일)**
```json
// careti-src/shared/feature-config.json (엔터프라이즈 환경 예시)
{
...
### **2. 정적 설정 (배포 환경)**
```bash
# 간소화 모드 활성화 (careti-src/shared/feature-config.json 수정)
cat > careti-src/shared/feature-config.json << 'EOF'
{
    "enableCaretAccountFeatures": false,
    "showPersonaSettings": false,
    "defaultPersonaEnabled": false,
...
const showPersona = featureConfig?.showPersonaSettings && modeSystem === "careti"
```

### **4. 브랜드 유틸리티 함수**
```typescript
import { getBrandRulesFileName, getBrandMcpSettingsFileName } from '@careti/utils/brand-utils'

// 표준 컨텍스트 경로 (.agents/context)
const rulesFileName = getBrandRulesFileName()
console.log(rulesFileName) // ".agents/context"

// MCP 설정 파일명 생성 (Careti → caret_mcp_settings.json, CodeCenter → codecenter_mcp_settings.json)
...
// "Careti has a question..." → "CodeCenter has a question..." 동적 변경
...
// "Careti wants to start a new task..." → "CodeCenter wants to start a new task..."
showSystemNotification({
...
{config.showPersonaSettings === true && modeSystem === "careti" && (
    <PersonaSection>
        <PersonaToggle />
        <PersonaSettings />
    </PersonaSection>
...
describe("FeatureConfig Integration Tests", () => {
    it("should load feature config from static JSON import", () => {
        ...
        expect(config.defaultModeSystem).toBe("careti")
        expect(config.firstListingProvider).toBe("careti")
        expect(config.defaultProvider).toBe("careti")
...
EOF
   npm run compile
   npm run watch
   # VS Code F5 → 계정 내부 UI 숨겨짐, Careti provider 제외, LiteLLM만 표시되는지 확인
   ```
...
**⚠️ 중요**: `careti-src/shared/feature-config.json` 파일 변경 후 반드시 `npm run compile` 실행 필요 (정적 import 반영)
...
### **브랜드별 기능 차별화**
Careti과 CodeCenter를 동일한 코드베이스로 관리하되 feature-config.json 파일만 변경하여 배포:
...
- `enableCaretAccountFeatures: false` - 계정 기능 제한
  ...
  - Careti provider를 프로바이더 목록에서 제외
...
**관련 이슈**:
- [careti-b2b/worklog/20250929-issue1-litellm-default.md]
- [careti-docs/work-logs/alpha/20251030-whitelabeling-correct-implementation.md]
- [careti-docs/work-logs/alpha/20251031-feature-flag-implementation-verification.md]

**v3.4 주요 변경사항** (2026-01-02):
- **`showModeSystemToggle`, `showAccountUI`, `showDictationToggle` 플래그 추가**: 모드/계정/음성 입력 UI 노출 제어
  - 수정 파일: FeatureConfig.ts, feature-config.json, App.tsx, Navbar.tsx, CaretiProvider.tsx, FeatureSettingsSection.tsx, CaretGeneralSettingsSection.tsx
  - VS Code 메뉴 컨텍스트 연동: `careti.showAccountUI` (package.json, extension.ts)

**v3.3 주요 변경사항** (2025-10-31):
- **`defaultPersonaEnabled` 플래그 추가**: 페르소나 시스템의 초기 활성화 상태 제어
  - `enablePersonaSystem`의 기본값을 브랜드별로 설정 가능
  - Careti: `true` (기본 활성화), CodeCenter: `false` (기본 비활성화)
...
- **`enableCaretAccountFeatures` 동작 확장**: Account 페이지 내부 UI와 Careti provider 제어
  - `false`일 때: Account 페이지 이메일/dropdown/버튼 숨김, Careti provider 목록에서 제외
...
- **defaultModeSystem 하드코딩 완전 제거**: 모든 fallback이 feature-config를 참조하도록 수정
  - Backend: `state-helpers.ts:601,626,628` - `|| "careti"` → `|| getCurrentFeatureConfig().defaultModeSystem`
  - Frontend: `ExtensionStateContext.tsx:247` - `"careti"` → `getCurrentFeatureConfig().defaultModeSystem`
  - Frontend: `CaretSettings.ts:44` - `"careti"` → `getCurrentFeatureConfig().defaultModeSystem`
