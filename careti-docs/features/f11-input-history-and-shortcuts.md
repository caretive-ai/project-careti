...
Careti의 **Input History & Shortcuts** 시스템은 터미널과 유사한 사용자 경험을 제공합니다.  
...
- **`careti-src/managers/CaretiGlobalManager.ts`**: 입력 히스토리 캐싱 및 gRPC 저장 리졸버 연동.
- **`src/core/controller/index.ts`**: 웹뷰 초기화 시 저장된 히스토리 로드 (`getStateToPostToWebview`).

### 2. Webview (Hooks & Components)
- **`webview-ui/src/careti/hooks/usePersistentInputHistory.ts`**: 백엔드 상태 구독 및 로컬 캐시 관리 훅.
- **`webview-ui/src/careti/hooks/useInputHistory.ts`**: 키보드 이벤트(`ArrowUp/Down`) 처리 및 입력창 제어 로직.
- **`webview-ui/src/components/chat/ChatTextArea.tsx`**: 키보드 이벤트 핸들러 통합.
- **`webview-ui/src/careti/shortcuts/shortcuts.json`**: 단축키 정의 파일.
...
- **단축키 카탈로그**:
  ...
  - 정의는 전역 JSON(`webview-ui/src/careti/shortcuts/shortcuts.json`)에 저장하고 매니저(`ShortcutManager`)를 통해 로드·표시한다.
...
```
careti-src/managers/
└── CaretiGlobalManager.ts           # 통합 상태 관리 및 gRPC 연동

webview-ui/src/careti/hooks/
├── usePersistentInputHistory.ts    # CaretiGlobalManager 연동 로직
...
### **3. CaretiGlobalManager 확장**
```typescript
// CaretiGlobalManager.ts - 캐시 및 백엔드 저장 담당
...
export default function ChatView() {
    // CARETI MODIFICATION: CaretiGlobalManager를 통한 persistent input history
    const { inputHistory, addToHistory } = usePersistentInputHistory()

    // CARETI MODIFICATION: Enhanced message handler with async history tracking
    const enhancedMessageHandlers = useMemo(() => ({
        ...messageHandlers,
        handleSendMessage: async (text: string, images: string[], files: string[]) => {
            await addToHistory(text) // CaretiGlobalManager를 통한 비동기 저장
...
const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(({
    ...
    const handleKeyDown = useCallback((event: React.KeyboardEvent<HTMLTextAreaElement>) => {
        // CARETI MODIFICATION: Handle persistent input history before other key events
...
### **저장 방식**
- **로컬 캐시**: CaretiGlobalManager 메모리 (즉시 접근)
...
모든 수정사항은 `// CARETI MODIFICATION:` 주석으로 명확히 표시:
