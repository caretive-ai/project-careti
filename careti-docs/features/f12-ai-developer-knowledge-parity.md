...
Careti의 **AI-Developer Knowledge Parity**는 AI와 개발자가 동일한 지식(규칙, 컨벤션, 아키텍처)을 공유하도록 보장하는 문서화 시스템입니다.  
...
| **동기화** | AI용 규칙과 사람용 문서가 별개 | **Single Source of Truth**. 개발자가 보는 문서(`careti-docs`)와 AI가 보는 규칙(`.agents/context`)이 1:1로 대응됨. |
...
- **`.agents/context/careti-rules.json`**: 전체 규칙 시스템의 인덱스 파일. (AI 진입점)
...
### 3. Developer Docs
- **`careti-docs/`**: 개발자가 읽는 문서 (AI 규칙의 Human-readable 버전).
...
### 1. 핵심 파일: `careti-rules.md`
- **위치**: `.agents/context/careti-rules.md`
- **역할**: AI에게 문서 시스템의 전체 구조와 파일 탐색 경로를 알려주는 최상위 규칙 파일입니다.
- **핵심 로직**:
  ```markdown
  ### Document Access Pattern (On-Demand System)
  - **1. Initialize**: AI reads `.agents/context/careti-rules.json` (JSON Index)
...
2. **워크플로우 로드**: `careti-rules.md`의 정의에 따라 `.agents/context/workflows/new-component.md` 워크플로우를 읽습니다.
...
4. **투명성**: 개발자는 `careti-docs`를 통해 AI가 어떤 절차로 작업하는지 명확히 알 수 있습니다.
