...
3. **규칙 인덱스 로드**: `.agents/context/careti-rules.json` 읽기 (필수)
...
- **Careti vs Cline 디렉토리 결정**: 새 기능은 `careti/` 폴더, Cline 원본은 최소 수정
- **백업 요구사항**: `.cline` 백업 파일은 deprecated (comment-only + git history로 원본/복구 추적)
- **테스트 파일 위치**: webview 테스트는 `src/careti/**/*.test.tsx`만 허용
...
- **파일 수정 체크리스트** (`careti-docs/development/careti-rules.ko.md` 내 체크리스트) ⚡ **MANDATORY**
- **백업 생성 규칙** 확인 필수
- **CARETI MODIFICATION 주석** 추가 규칙 확인
...
- `careti-docs/development/component-architecture-principles.md` ⚡ **MANDATORY**
- VSCode 테마 통합 가이드
- i18n 국제화 패턴
...
- `careti-docs/development/testing-guide.md` ⚡ **MANDATORY**
...
- `careti-docs/development/frontend-backend-interaction-patterns.md` (setPersona 패턴)
- `careti-docs/development/component-architecture-principles.md`
- `assets/template_characters/` 구조 분석

### **Phase 3: 기술적 제약사항 및 아키텍처 패턴 분석**

4. **관련 작업 문서 식별**:
    - `careti-docs/tasks/task-status.md` 또는 지정된 작업 문서 확인
    - 작업의 목적, 범위, 제약사항 파악
...
test('웹뷰에서 백엔드 메시지가 한국어로 번역되어야 함', () => {
  ...
  const rendered = renderMessage(message, { mode: 'careti', language: 'ko' })
  expect(rendered).toContain('Careti이 파일을 편집하려고 합니다')
})
```
...
- [ ] **Level 1 독립 모듈 우선 고려** (`careti-*` 디렉토리 사용)
- [ ] `// CARETI MODIFICATION: [간단한 설명]` 주석 필수 추가
- [ ] 최소 수정 원칙 (1-3라인) 준수
- [ ] 파일 수정 체크리스트 완료
- [ ] 보호 디렉토리에 신규 파일 추가가 불가피한 경우(예: 테스트) 파일 상단에 `// CARETI MODIFICATION:` 헤더로 Careti 추가 파일임을 표기
...
- **Careti 전용 기능**: `careti/` 폴더에 배치
- **Cline 원본**: 최소 수정 원칙 적용

### **테스트 위치 실수 방지**

- **webview 테스트**: `src/careti/**/*.test.tsx`만 허용
- **include path 설정**: 테스트 생성 전 검증 필수

### **독립 모듈 우선 원칙**

- **Level 1 독립 모듈**: `careti-*` 디렉토리 사용 우선 고려 (충돌 최소화)
- **불가피한 Cline 수정**: 백업 생성 및 `// CARETI MODIFICATION:` 주석 필수
