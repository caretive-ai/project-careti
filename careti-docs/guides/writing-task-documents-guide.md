...
이 문서는 Careti 프로젝트에서 **AI와의 협업**을 위한 작업 문서 작성 방법을 안내합니다. 모든 작업은 **한 번의 AI 대화 세션에서 완료 가능한 크기**로 설계되어야 하며, Plan과 Checklist를 통합한 단일 문서 형태를 사용합니다.
...
- {Cline 원본 파일 수정 시: `.cline` 백업은 deprecated. `// CARETI MODIFICATION:` + git 기반 복구(필요시 체크포인트/커밋) 필수}
- {특정 라이브러리/패턴 사용 제한}
- {테스트 커버리지 요구사항}
...
- `careti-docs/tasks/task-status.md`에서 전체 작업 상태 관리
...
- 일일 작업 로그: `careti-docs/work-logs/{username}/{date}.md`
