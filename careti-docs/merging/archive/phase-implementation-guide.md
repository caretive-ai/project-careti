...
이 문서는 Careti 머징의 각 Phase별 상세 구현 단계와 실행 가이드를 제공합니다.
...
| **Phase 3** | 핵심 Careti 기능 | 3-4일 | 🚨 HIGH | 🚨 HIGH |
...
### **목표**
Cline → Careti 기본 브랜딩 적용 및 에이전트 표준화(AAIF SoT) 구축
...
# package.json 수정
sed -i 's/"name": "cline"/"name": "careti"/g' package.json
sed -i 's/"displayName": "Cline"/"displayName": "Careti"/g' package.json
...
##### **🧪 TDD 1단계: 테스트 코드 작성**
```typescript
// careti-src/__tests__/rule-priority.test.ts 생성
...
## ⚡ **Phase 3: 핵심 Careti 기능**

### **목표**
Careti의 핵심 차별화 기능인 모드 시스템과 JSON 프롬프트 이식
...
# 의존성 매핑
node careti-scripts/analyze-mode-dependencies.js
...
export class ModeManager {
    private static modeMapping = {
        careti: { plan: "chatbot", act: "agent" },
        cline: { chatbot: "plan", agent: "act" }
    }
    
    static mapMode(from: string, toSystem: "careti" | "cline"): string {
...
##### **🧪 TDD 1단계: 통합 테스트 작성**
```typescript
// careti-src/__tests__/mode-system-integration.test.ts
describe('Mode System Integration', () => {
    test('Careti chatbot mode should map to Cline plan mode', () => {
        // 매핑 테스트
    })
...
```bash
# 1. 핵심 타입 정의 이식
cp careti-main/src/shared/ChatSettings.ts src/shared/

# 2. Proto 변환 로직 이식  
cp careti-main/src/shared/proto-conversions/state/chat-settings-conversion.ts \
...
```typescript
// careti-src/core/prompts/__tests__/json-prompt-system.test.ts
...
```bash
# 1. JSON 프롬프트 파일들 이식
mkdir -p careti-src/core/prompts/json-system-prompts/mode-specific
cp -r careti-main/careti-src/core/prompts/json-system-prompts/* \
      careti-src/core/prompts/json-system-prompts/

# 2. 프롬프트 빌더 시스템 이식
cp careti-main/careti-src/core/prompts/prompt-builder.ts \
   careti-src/core/prompts/

# 3. 시스템 프롬프트 선택기 이식
cp careti-main/careti-src/core/prompts/system-prompt-selector.ts \
   careti-src/core/prompts/
...
- [ ] Careti ↔ Cline 모드 시스템 seamless 전환
- [ ] 모든 모드 조합 정상 동작
- [ ] JSON 프롬프트 시스템 정상 동작
- [ ] 통합 테스트 100% 통과
...
```bash
# 테스트 이식
cp careti-main/webview-ui/src/careti/components/__tests__/PersonaManagement.test.tsx \
   webview-ui/src/careti/components/__tests__/

cp careti-main/careti-src/utils/__tests__/persona-initializer.test.ts \
   careti-src/utils/__tests__/
```

##### **🔧 페르소나 시스템 이식**
```bash
# 1. 페르소나 에셋 이식
cp -r careti-main/assets/template_characters/ \
      assets/template_characters/

# 2. 페르소나 초기화 로직 이식
cp careti-main/careti-src/utils/persona-initializer.ts \
   careti-src/utils/

# 3. UI 컴포넌트 이식
cp careti-main/webview-ui/src/careti/components/PersonaManagement.tsx \
...
cp careti-main/src/services/account/CaretAccountService.ts \
   src/services/account/

# 2. 프론트엔드 UI 이식
cp -r careti-main/webview-ui/src/careti/components/CaretAccount* \
      webview-ui/src/careti/components/
```
...
```bash
# 1. 백엔드 로거 이식
cp careti-main/careti-src/utils/careti-logger.ts \
   careti-src/utils/

# 2. 프론트엔드 로거 이식
cp careti-main/webview-ui/src/careti/utils/webview-logger.ts \
   webview-ui/src/careti/utils/

# 3. 테스트 이식
cp careti-main/webview-ui/src/careti/utils/__tests__/webview-logger.test.ts \
   webview-ui/src/careti/utils/__tests__/
```

#### **5.2 기존 시스템에 로깅 통합**
```typescript
// 주요 컴포넌트와 서비스에 로깅 추가
import { caretLogger } from "../../../careti-src/utils/careti-logger"
...
```bash
# i18n 테스트 이식
cp careti-main/webview-ui/src/careti/utils/__tests__/i18n.test.ts \
   webview-ui/src/careti/utils/__tests__/
```
...
```bash
# 1. 언어 파일 이식 (30개 JSON)
cp -r careti-main/webview-ui/src/careti/locale/ \
      webview-ui/src/careti/locale/

# 2. i18n 유틸리티 이식
cp careti-main/webview-ui/src/careti/utils/i18n.ts \
   webview-ui/src/careti/utils/

# 3. 언어 설정 Hook 이식
cp careti-main/webview-ui/src/careti/hooks/useCurrentLanguage.ts \
   webview-ui/src/careti/hooks/

# 4. 언어 설정 UI 이식
cp careti-main/webview-ui/src/careti/components/CaretUILanguageSetting.tsx \
   webview-ui/src/careti/components/
```
