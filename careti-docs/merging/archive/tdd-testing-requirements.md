...
Careti의 **Test-Driven Development (TDD)** 원칙과 100% 테스트 커버리지 요구사항을 정의하는 문서입니다.
...
- **100% 커버리지**: 모든 Careti 소스코드 (`careti-src/`) 100% 테스트 커버리지 필수
...
### **필수 100% 커버리지 범위**
```
careti-src/                          # Careti 전용 소스 (100% 필수)
├── core/                           # 핵심 로직
├── utils/                          # 유틸리티 함수
├── services/                       # 서비스 클래스
└── __tests__/                      # 테스트 파일

webview-ui/src/careti/               # Careti UI 컴포넌트 (90% 이상)
...
├── *.ts (CARETI MODIFICATION 부분)  # 수정 부분 테스트 추가
...
{
  "scripts": {
    "test:backend": "vitest run src/ careti-src/",
    "test:backend:watch": "vitest watch src/ careti-src/",
    "test:coverage": "vitest run --coverage src/ careti-src/",
...
{
  "scripts": {
    "test:frontend": "vitest run src/careti/",
    "test:frontend:watch": "vitest watch src/careti/",
    "test:frontend:coverage": "vitest run --coverage src/careti/"
  },
  "devDependencies": {
...
```typescript
// careti-src/__tests__/rule-priority.test.ts
...
```typescript
// webview-ui/src/careti/components/__tests__/CaretAccountView.test.tsx
describe('Careti Account View', () => {
...
### **다국어 i18n System**
```typescript
// webview-ui/src/careti/utils/__tests__/i18n.test.ts
...
describe('Mode System Integration', () => {
    test('should map Careti modes to Cline modes correctly', () => {
        // 모드 매핑 테스트
    })
...
### **JSON System Prompt**
```typescript
// careti-src/core/prompts/__tests__/prompt-builder.test.ts
...
### **Persona System**
```typescript
// careti-src/utils/__tests__/persona-initializer.test.ts
...
### **Branding & UI System**
```typescript
// webview-ui/src/careti/components/__tests__/CaretWelcome.test.tsx
describe('Careti Welcome Page', () => {
...
echo "Checking Careti source coverage..."
CARET_COVERAGE=$(npm run test:coverage -- careti-src/ --reporter=json | jq '.coverage.total.lines.pct')

if (( $(echo "$CARET_COVERAGE < 100" | bc -l) )); then
    echo "❌ Careti source coverage $CARET_COVERAGE% < 100% (REQUIRED)"
    exit 1
else
    echo "✅ Careti source coverage: $CARET_COVERAGE%"
fi
...
### **공통 테스트 헬퍼**
```typescript
// careti-src/__tests__/helpers/test-utils.ts
...
### **API 모킹**
```typescript
// careti-src/__tests__/mocks/careti-api.ts
...
### **파일 시스템 모킹**
```typescript
// careti-src/__tests__/mocks/fs.ts
...
- **테스트 커버리지**: Careti 소스 100%, UI 90% 이상
