...
| 최소 침습 원칙 | ✅ PASS | CARETI MODIFICATION 주석 포함 |
...
- [x] CaretiPromptWrapper 동적 import
...
**오류 내용**:
```
careti-src/core/prompts/system/adapters/CaretiJsonAdapter.ts(226,60): error TS2345
...
**원인 분석**:
- `CaretiJsonAdapter.ts`에서 사용하는 mock variant에 `matcher` 속성 누락
- Cline v3.38.1에서 `PromptVariant` 타입에 `matcher` 필드가 추가된 것으로 추정
- D-1 구현과 직접 관련은 없지만, CaretiPromptWrapper 호출 경로에 영향

**해결 방안**:
```typescript
// careti-src/core/prompts/system/adapters/CaretiJsonAdapter.ts:226
...
**설계에 명시된 테스트**:
- `careti-src/__tests__/prompt-system/mode-system.test.ts` (신규)

**필요한 테스트 케이스**:
1. `should persist caretModeSystem to globalState on mode change`
2. `should route to CaretiPromptWrapper when modeSystem is careti`
3. `should use Cline registry when modeSystem is cline`
4. `should show Chatbot/Agent labels in Careti mode`
...
| 3 | 최소 침습 + CARETI MODIFICATION 주석 | ✅ | 양쪽 파일 모두 주석 포함 |
| 4 | 하드코딩/정책 위반 | ✅ | 없음 |
| 5 | Careti 정책 준수 | ✅ | modeSystem 분기, 브랜딩 유지 |
| 6 | 보안 위험 코드 | ✅ | 없음 |
...
1. **TypeScript 오류 수정** (우선순위: HIGH)
   - 파일: `careti-src/core/prompts/system/adapters/CaretiJsonAdapter.ts:226`
   - 조치: `matcher` 속성 추가
...
- CARETI MODIFICATION 주석으로 추적 용이
- 반환 타입 호환성 유지
...
Phase D-1의 핵심 로직은 올바르게 구현되었습니다. `SetPromptSystemMode.ts`의 globalState 영속화와 `system-prompt/index.ts`의 Careti 분기가 설계대로 추가되었습니다.

그러나 **TypeScript 컴파일 오류**로 인해 현재 빌드가 실패하며, 이는 `CaretiJsonAdapter.ts`의 `PromptVariant` 타입 불일치에서 발생합니다. 이 오류를 해결하고 테스트를 추가해야 D-1이 완료됩니다.
