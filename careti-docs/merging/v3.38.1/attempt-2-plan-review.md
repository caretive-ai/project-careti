...
### 1. 전략 선택: Cline-Base + Careti-Overlay ⭐⭐⭐⭐⭐
...
| Careti 자산 유실 | CARETI MODIFICATION 탐색 | ✅ 적절 |
...
2. Careti-only
   a. 브랜딩 (careti-src/) → 자동 유지 ✅
   b. 테스트 (careti-src/test/) → 자동 유지 ✅

3. Both (복잡함! ⚠️)
   a. CARETI MODIFICATION만 있는 경우
      - Cline 변경 + Careti 주석
      - 전략: Cline 업데이트 적용 → CARETI 주석 재삽입
      - 예: src/core/api/index.ts

   b. 구조적 변경 + Careti 로직
      - Cline이 함수 시그니처 변경
      - Careti이 같은 함수에 로직 추가
      - 전략: 3-way merge 필요
...
enum MergeStrategy {
  ...
  AUTO_KEEP = 'auto_keep',          // Careti-only → 자동
  SIMPLE_MERGE = 'simple_merge',    // Both: CARETI MODIFICATION만
...
const classifications: FileClassification[] = [
  {
    ...
    estimatedTime: '4h',
    dependencies: [],
    risks: ['필드 번호 충돌', 'CARETI 필드 손실 위험']
  },
  {
...
```bash
# Codex 제안
git diff --binary cline/v3.34.0..HEAD -- src careti-src webview-ui assets > careti.patch
...
for category in "${categories[@]}"; do
  ...
  if git apply --check "/tmp/$category.patch" 2>/dev/null; then
    ...
    for file in $(git diff --name-only cline/v3.34.0..merge/cline-v3.34.0-method3 -- "$category"); do
      ...
      # CARETI MODIFICATION 확인
      if grep -q "CARETI MODIFICATION" /tmp/ours 2>/dev/null; then
        echo "      🔍 Careti modification detected"
        ...
        # Careti 수정 없음 → Cline 버전 채택
        echo "      ℹ️ No Careti modifications, adopting Cline version"
        cp /tmp/theirs "$file"
      fi
    done
  fi
...
### 4. Careti 수정사항 추출 자동화 부족 ⭐⭐

**문제:**
```
Phase A.4: "CARETI MODIFICATION 주석이 있는 diff를
...
async function extractCaretModifications(): Promise<CaretMod[]> {
  ...
  // 1. CARETI MODIFICATION 주석
  const commentMods = await execCommand(
    `git grep -n "CARETI MODIFICATION" merge/cline-v3.34.0-method3`
  )
  ...
  // 3. Careti 전용 파일
  const caretFiles = await glob('careti-src/**/*')
...
async function generateReport(mods: CaretMod[]): Promise<string> {
  ...
  let report = '# Careti Modifications Extraction Report\n\n'
...
async function generatePatches(mods: CaretMod[]): Promise<void> {
  ...
  for (const [category, items] of Object.entries(byCategory)) {
    ...
    await fs.writeFile(
      `careti-docs/merging/v3.37.1/patches/${category}.patch`,
      patch
    )
  }
}
...
await fs.writeFile('careti-docs/merging/v3.37.1/careti-mods-report.md', report)
await generatePatches(mods)

console.log(`✅ Extracted ${mods.length} Careti modifications`)
console.log(`📄 Report: careti-docs/merging/v3.37.1/careti-mods-report.md`)
console.log(`📦 Patches: careti-docs/merging/v3.37.1/patches/`)
```
...
message ModelsApiConfiguration {
  string apiProvider = 1;
  // ... 필드 70개
  // CARETI MODIFICATION: Careti providers (1072+)
...
// 원칙: Cline 필드는 그대로, Careti 필드는 1072+ 유지
```
...
// Ours (Careti)
async function updateApiConfiguration(config: ApiConfig) {
  // CARETI MODIFICATION: Careti branding
...
function SettingsView() {
  return (
    <div>
      <ApiSection />
      <ModelSection />
      {/* CARETI MODIFICATION: Persona section */}
      <PersonaSection />
    </div>
  )
}
  ...
  return (
    <SettingsContainer>
      <Tabs>
        ...
        {/* CARETI MODIFICATION: Persona tab */}
        <Tab label="Persona">
...
function isIntentionallyExcluded(file: string): boolean {
  const excludePatterns = [
    /^cli\//,           // Cline CLI (Careti은 미사용)
    /^docs\//,          // Cline docs (Careti 별도)
...
async function main() {
  ...
  if (deletedFiles.length > 0) {
    ...
    fs.writeFileSync('careti-docs/merging/v3.37.1/deleted-files-report.md', report)
    console.error('📄 Report saved to: careti-docs/merging/v3.37.1/deleted-files-report.md\n')

    process.exit(1)
  }

  // 4. 추가된 파일 검증
  const caretFiles = byStatus.A.filter(f => f.file.startsWith('careti-'))

  console.log('✅ No unintentional deletions detected!')
  console.log(`\n📁 Careti-specific files added: ${caretFiles.length}`)
...
### Phase B: Careti Overlay (0/50 완료)
**Proto (0/16):**
- [ ] models.proto
- [ ] state.proto
- [ ] ... (나머지)
