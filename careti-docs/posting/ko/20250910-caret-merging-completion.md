---
title: "11일간 빡센 작업: careti-merging 완료 기록"
date: 2025-09-10
description: "Cline 58개 파일 충돌 문제를 최소 침습 방식으로 해결한 11일간의 작업 기록. AI와 함께 어떻게 해냈는지 솔직하게 정리했습니다."
tags: ["Careti", "Cline", "Architecture", "AI Collaboration", "Minimal Invasion", "Technical Transformation"]
---
...
**careti-merging 프로젝트**가 끝났습니다. 11일간 정말 빡세게 작업했어요.

사실 9월 6일까지만 해도 완전 막막했거든요. Cline이 너무 많이 바뀌어서 기존 careti-main의 수정 파일들이 다 충돌나고, RooCode로 아예 갈아탈까 진짜 고민했었습니다.
...
### **Before (careti-main): 침습적 Fork의 한계**
...
### **After (careti-merging): 최소 침습의 혁신**
...
t05: 에이전트 표준화(AAIF SoT) - 충돌 해결 메커니즘
```

#### **t06: 핵심 전환의 치열함** (18개 로그!)
```
메인 로그:
- t06-careti-modification-analysis.md
...
```typescript
// careti-src/ - 완전한 자유
export class CaretiJsonAdapter implements IPromptAdapter {
  buildPrompt(): string {
...
export function buildSystemPrompt() {
  // CARETI MODIFICATION: Use mode system registry
...
const SYSTEM_PROMPT = `You are Careti, an AI assistant...
[707 lines of hardcoded text]`
```

**After (careti-merging)**:
```json
{
  "CORE_IDENTITY": {
...
export class MessageHandlerFactory {
  static createHandler(mode: CaretiMode): IMessageHandler {
    switch(mode) {
      case CaretiMode.CHATBOT: return new ChatbotMessageHandler()
      case CaretiMode.AGENT: return new AgentMessageHandler()
...
**효과**: Careti과 Cline의 메시지 처리가 완전 분리되어 상호 독립 작동
...
```
📁 생성/수정 파일: 871개+
💻 TypeScript 파일: 28개 (careti-src)
📖 문서 파일: 182개 (careti-docs)
🧪 테스트 파일: 79개
⚙️ JSON 설정: 18개 섹션
📝 총 코드 라인: ~22,500+ 라인
```
...
**careti-merging 프로젝트**는 단순한 코드 업데이트가 아니었습니다. 이는:
...
52284266: 안정화 진행중
05d160f8: feat: Remove careti-b2b directory from Git tracking
...
0c126745: feat: Implement Careti-specific mode management
...
### **C. 파일 구조**
```
careti-src/ (28 TypeScript files)
├─ core/
│  ├─ mode-system/ (8 files)
│  ├─ prompts/ (12 files)
│  └─ messaging/ (8 files)
...
careti-docs/ (182 documentation files)
...
*이 포스팅은 careti-merging 프로젝트의 전체 여정을 기록한 완전판입니다. 모든 데이터와 코드는 오픈소스로 공개되어 있으며, 커뮤니티의 학습과 발전을 위해 자유롭게 활용하실 수 있습니다.*