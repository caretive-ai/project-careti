...
### 2. 파일 이름 반대로 저장
```
cline-full-prompt.txt → 실제로는 Careti 프롬프트 (371줄)
careti-full-prompt.txt → 실제로는 Cline 프롬프트 (572줄)
```
...
### Careti vs Cline 시스템 프롬프트
- **Careti**: 371줄 (JSON 기반, 구조화)
- **Cline**: 572줄 (하드코딩, 상세 설명)
- **실제 차이**: Careti이 35% 더 짧음 (보고서와 정반대)
...
**현재 Careti의 시스템 프롬프트 의미론적 분석은 완전히 가짜입니다.**
