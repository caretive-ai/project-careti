# Spec-Kit과 Careti 통합 분석 및 실현 가능한 구현 방안 (v2.0)

**작성일**: 2024-09-17
**기반**: 실제 코드베이스 심층 분석
**목적**: Spec-Kit과 Careti의 실제 아키텍처를 바탕으로 한 구체적 통합 방안 제시
...
CaretiGlobalManager.initialize(initialMode) // "careti" | "cline"
```
...
1. **기존 Tool Handler 활용**: Careti의 `NewTaskHandler`를 확장하여 Spec-Kit 워크플로우 구현
2. **Slash Command 확장**: `/specify`, `/plan`, `/tasks` 명령어를 기존 시스템에 추가
3. **Level 1 독립 구조**: `careti-src/spec-integration/` 디렉토리로 완전 독립 구현
4. **기존 프롬프트 시스템 활용**: Careti의 이중 모드 시스템 확장
...
```typescript
// careti-src/spec-integration/commands/SpecCommands.ts
...
// src/core/slash-commands/index.ts에 CARETI MODIFICATION 추가
const SUPPORTED_DEFAULT_COMMANDS = [
  "newtask", "smol", "compact", "newrule", "reportbug", "deep-planning",
  // CARETI MODIFICATION: Spec-Kit integration commands
  "specify", "plan", "tasks"
]
```
...
```typescript
// careti-src/spec-integration/handlers/SpecifyHandler.ts
...
```typescript
// careti-src/spec-integration/handlers/PlanHandler.ts
...
```typescript
// careti-src/spec-integration/analyzers/LegacyAnalyzer.ts
...
```typescript
// careti-src/spec-integration/generators/PRDGenerator.ts
...
# 결과: careti-docs/legacy-analysis/ 디렉토리 생성
...
```typescript
// src/core/slash-commands/index.ts (CARETI MODIFICATION)
import {
  specifyToolResponse,
  planToolResponse,
  tasksToolResponse
} from "@careti/spec-integration/commands/responses"
...
export async function parseSlashCommands(
  ...
  const commandReplacements: Record<string, string> = {
    ...
    // CARETI MODIFICATION: Spec-Kit commands
    specify: specifyToolResponse(),
    plan: planToolResponse(),
    tasks: tasksToolResponse()
  }
...
```typescript
// careti-src/spec-integration/commands/responses.ts
...
```typescript
// careti-src/spec-integration/validators/ConstitutionalValidator.ts
...
1. **최소 침입적 구현**
   - Careti의 Level 1 독립 구조 활용
   - 기존 Cline 코드 최소 변경 (CARETI MODIFICATION만)
   - 기존 사용자 경험 보존
