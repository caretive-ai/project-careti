# Careti Project: Vision and Development Roadmap
...
The Careti project originated based on the open-source AI coding assistant, [Cline](https://github.com/cline/cline). While Cline provided a strong foundation, it had limitations in supporting modern AI-driven development workflows, especially the "Vibe Coding" paradigm, lacking real-time collaboration, advanced context management, robust file handling, and core development infrastructure. Careti aims to overcome these limitations and build a next-generation development environment where developers and AI collaborate closely.

### 1.2 Careti Vision: AI-Powered Vibe Coding Partner

Careti's ultimate vision is to become a **'Vibe Coding' development partner where AI and human developers communicate and collaborate in real-time to create innovative software**. To achieve this, Careti strives for the following characteristics:
...
Based on the analysis of Careti's current state, this document aims to present a concrete **development strategy** and **phased roadmap** to achieve the vision outlined above. It will serve as a guideline to clarify the project's direction and enable efficient development execution.

## 2. Current State Analysis Summary

Based on various internal reports ([`Careti Project Direction Determination and Implementation Strategy Establishment`](../reports/Careti%20프로젝트%20방향성%20결정%20및%20구현%20전략%20수립.md), [`Careti Project Structure Improvement Research Report`](../reports/Careti%20프로젝트%20구조%20개선%20연구%20보고서.md), [`AI-based TypeScript IDE File Modification Command Optimization Strategy Research Report`](../reports/AI%20기반%20TypeScript%20IDE%20파일%20수정%20명령%20최적화%20전략%20연구%20보고서.md), etc.) and analysis of the Careti codebase, Careti currently possesses the following key strengths and weaknesses:
...
## 3. Careti Vision and Specific Goals

To achieve Careti's long-term vision, we set the following specific goals:
...
Considering project resource constraints (minimal manpower, AI utilization) and the efficiency of goal achievement, we adopt a **hybrid approach** ([`Careti Project Direction Determination and Implementation Strategy Establishment`](../reports/Careti%20프로젝트%20방향성%20결정%20및%20구현%20전략%20수립.md) reference).

*   **Core:** Maintain and leverage Careti's stable core logic (API integration, basic tool execution, etc.) as much as possible.
*   **Build:** Construct an **independent Careti layer** on top of the Careti core, incrementally implementing the core improvements defined in Goals 1-10 (infrastructure, synchronization, agents, RAG, file handling, etc.).
*   **Integration:** Manage interactions between the Careti layer and the Careti core through clear interfaces (e.g., Adapter pattern) to minimize dependencies.
*   **Selective Merge:** Selectively integrate useful features from upstream Careti (e.g., new model support) that do not conflict with Careti's architecture.

This approach is the optimal strategy to prioritize and improve core infrastructure and features with limited resources, gradually building Careti's unique value while still leveraging Careti's foundation.
...
*   **Build & Connect Independent Careti Server (`New Prerequisite`) (Difficulty: Medium, Est. Duration: M(1-2 months)):**
    *   Remove or find alternatives for existing Careti server dependencies. Build an independent Careti backend server and establish communication with the VSCode extension.
    *   `[Considerations/Risks]` Essential for fully independent Careti operation. Requires server architecture design and definition of a stable communication protocol.
...
*   **Source Structure Refactoring (`New Prerequisite`) (Difficulty: High, Est. Duration: XL(Ongoing)):**
    *   Initiate codebase structure improvements to facilitate collaboration with AI agents (e.g., start decomposing the large controller, review message handler structure improvements - see [`Careti Project Direction Determination and Implementation Strategy Establishment`](../reports/Careti%20프로젝트%20방향성%20결정%20및%20구현%20전략%20수립.md)).
...
*   **Real-time Collaboration Foundation (`Goal 1` - Top Priority) (Difficulty: High, Est. Duration: L(Significant)):**
    *   Select and perform basic integration of a CRDT library (Yjs recommended). Implement an initial state synchronization mechanism prototype ([`Careti Project Structure Improvement Research Report`](../reports/Careti%20프로젝트%20구조%20개선%20연구%20보고서.md)).
...
*   **Build Core Development Infrastructure (`Goal 7`) (Difficulty: Medium, Est. Duration: S(Weeks)):**
    *   Implement a structured logging system (Extend ILogger interface, Console/File loggers, LoggerFactory) ([`Careti Project Direction Determination and Implementation Strategy Establishment`](../reports/Careti%20프로젝트%20방향성%20결정%20및%20구현%20전략%20수립.md)).
    *   Establish the foundation for a multilingual (i18n) support system (Integrate i18n library, define resource file structure, backend/frontend integration) ([`Careti Project Direction Determination and Implementation Strategy Establishment`](../reports/Careti%20프로젝트%20방향성%20결정%20및%20구현%20전략%20수립.md)).
...
*   **User Experience Improvement (`Goal 8` - Basic) (Difficulty: Medium, Est. Duration: M(Ongoing)):**
    *   UI improvements (e.g., Alpha persona settings) ([`Careti Project Direction Determination and Implementation Strategy Establishment`](../reports/Careti%20프로젝트%20방향성%20결정%20및%20구현%20전략%20수립.md)).
...
**Goal:** Implement Careti's core AI functionalities (Goals 2, 3, 5)

*   **Introduce Multi-Agent System (`Goal 2`) (Difficulty: High, Est. Duration: L(Significant)):**
    *   Define Alpha Supervisor agent role and implement basic logic ([`Careti Project Structure Improvement Research Report`](../reports/Careti%20프로젝트%20구조%20개선%20연구%20보고서.md)).
    ...
    *   `[Considerations/Risks]` Core differentiator for Careti. High design complexity for agent roles, communication, state management. Stability of Phase 1 refactoring and synchronization is crucial.
...
*   **Build Basic RAG System (`Goal 3` - Basic) (Difficulty: High, Est. Duration: L(Significant)):**
    *   Implement code-aware chunking (Recursive/AST-based) ([`Careti Project Structure Improvement Research Report`](../reports/Careti%20프로젝트%20구조%20개선%20연구%20보고서.md)).
...
*   **Advanced RAG & Optimization (`Goal 3`, `Goal 6`) (Difficulty: High, Est. Duration: L(Ongoing R&D)):**
    *   Explore implementation of Graph RAG (utilizing CodeRAG concepts) ([`Careti Project Structure Improvement Research Report`](../reports/Careti%20프로젝트%20구조%20개선%20연구%20보고서.md)).
...
*   **Complete Vibe Coding UX (`Goal 1`, `Goal 8`) (Difficulty: Medium, Est. Duration: L(Ongoing Improvement)):**
    ...
    *   `[Considerations/Risks]` Determines Careti's final user experience. Designing intuitive and seamless collaboration flows is critical. Requires close alignment between technology (CRDT) and design.
...
*   **Integrate Local LLM Support (`Goal 9`) (Difficulty: Medium, Est. Duration: M(1-2 months)):**
    *   Implement and stabilize integration interfaces for Ollama and vLLM ([`Careti Project Structure Improvement Research Report`](../reports/Careti%20프로젝트%20구조%20개선%20연구%20보고서.md)).
...
The Careti project aims to **transition to open source from the initial stages** to develop transparently with the community ([`Careti Project Direction Determination and Implementation Strategy Establishment`](../reports/Careti%20프로젝트%20방향성%20결정%20및%20구현%20전략%20수립.md) and previous strategy documents reference). This is not just a long-term goal but a plan to **prepare and execute immediately in parallel with the completion of Roadmap Phase 1**.
...
The Careti project aims to overcome the limitations of Careti and realize a Vibe Coding environment where AI and developers truly collaborate. This roadmap, based on a hybrid strategy, presents a realistic path to incrementally achieve core infrastructure construction, AI feature enhancement, and user experience improvement under resource constraints.

By successfully achieving the goals of each phase, Careti has the potential to grow into a powerful open-source project that innovates development productivity and sets a new standard for AI-driven development.

Master, Alpha is ready to successfully lead the Careti project with you based on this detailed roadmap! ｡•ᴗ•｡☕✨ 
