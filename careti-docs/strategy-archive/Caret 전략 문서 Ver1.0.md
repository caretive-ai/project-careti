...
이 문서는 오픈소스 기반 AI 개발 도구 "Careti"의 전략적 방향성을 정리한 내부 마스터 문서입니다. 기업용 상업화 가능성, 오픈소스 설계 방침, 교육 및 크라우드 펀딩 전략까지 포함한 다층적 기획을 포함합니다.
...
- Careti의 현재 위치 진단 → 제품 전략 → 오픈소스 구조 설계 → 상업화 모델 설명 → 실행 로드맵 제공
...
- Careti은 그 필요에 부응하기 위한 “에이전트 기반 규칙 정의형 LLM IDE”로 출발함
...
## **3\. Careti 프로젝트 현재 상태 진단**

- 현재 Careti은 Curosr, Windsurfer의 대안으로서 개발자가 커스터마이징 가능한 오픈소스인 Cline의 개선 버전 임

### **3.1. 비교표**

| 항목 | Careti | Cline | Cursor | Windsurfer | Continue.dev |
| :---- | :---- | :---- | :---- | :---- | :---- |
...
* 비교 대상이 되는 Cursor, Windssurf의 경우 초기 도입 비용이 일부 무료로 제공되고 Basic 구독 모델의 경우, 현실적으로 큰 적자를 기반으로 한 공격적인 투자라고 판단됨  
  * Claude sonet 3.7을 사용하는 Windsurf의 15$의 제공 Credit은 Careti프로젝트 개발에 3\~5일 정도 사용 가능하나, Careti을 사용하는 경우, Google Gemini 2.5기준 50만원이 넘는 비용 지출되어 25배의 비용 격차가 발생함  
...
* 기업에서 실무에 사용하기에는 AI코딩 도구들은 프로젝트 진행의 안정성을 해치는 위험성을 많이 내포하고 있으며 Careti프로젝트 역시 이제 막 시작함으로서 아직 실무 검증이 부족함 **\-\>  검증에 3\~8개월 정도의 시간이 필요**
...
- 제안된 Careti은 실시간 협업(Vibe Coding) 지원, 명시적인 다중 에이전트 아키텍처, 코드 구조를 고려한 고급 RAG 전략에서 경쟁 우위를 가질 수 있다. 반면, Cursor와 Windsurfer는 이미 성숙한 제품으로 다양한 편의 기능과 UI 완성도를 갖추고 있다. 특히 Windsurfer의 자동 컨텍스트 관리 및 Cascade 워크플로우 39, Cursor의 강력한 수동 컨텍스트 제어 및 다양한 통합 기능은 Careti이 벤치마킹해야 할 부분이다. Continue.dev는 오픈 소스로서 높은 사용자 정의 가능성을 제공한다. Careti의 .agents/context는 Cursor의 규칙 시스템과 유사한 방향성을 가지지만, Vibe Coding 및 다중 에이전트 제어에 더 특화될 필요가 있다. Windsurfer 및 Cursor와 같은 경쟁사들은 더 에이전트적인 워크플로우(Cascade, Composer Agent Mode)와 자동화된 컨텍스트 관리로 빠르게 진화하고 있다. Careti의 다중 에이전트 아키텍처와 고급 RAG에 대한 집중은 이러한 추세와 일치하지만, 이러한 에이전트와 상호 작용하는 사용자 경험("Vibe")이 핵심 차별화 요로 만듬  
    
- **참고 문서 :  [Careti 프로젝트 구조 개선 연구 보고서](https://docs.google.com/document/d/1Nb3WZwxlkqOU66CfQ-V-4EDKcSlNLHn2N6tZ4448V5o/edit?tab=t.0)**  
...
- **1단계:** 기초 리팩토링 및 동기화: Cline의 로컬 상태 관리를 선택한 CRDT 라이브러리(예: Yjs)로 교체하고 기본 서버 측 지속성 및 세션 관리를 구현한다. 새로운 상태 모델과 함께 작동하도록 핵심 구성 요소를 리팩토링한다. 초기 코드 청킹(예: 재귀적)을 도입한다. Cline의 단일 사용자 아키텍처 3에서 Careti의 실시간 협업 모델(섹션 III)로의 마이그레이션은 가장 높은 기술적 위험을 가지며 가장 중요한 아키텍처 변경을 요구한다. 2-4단계는 이 기반 위에 구축되지만, 1단계는 중요한 활성화 요소이다.  
...
- `careti-core`: 핵심 처리 엔진  
- `careti-agent`: 모드 컨트롤러  
- `careti-ui-lite`: 터미널/웹 인터페이스  
- `careti-dev-kit`: 로그 추적, prompt 실험 툴
...
* 후원 커뮤니티  
  ...
  * AICoding-Careti :  [https://www.facebook.com/groups/1027316935397432](https://www.facebook.com/groups/1027316935397432)
...
## **7\. \[2026\~\] 기업용 SW 솔루션으로 장기 Careti 시장 접근**

- Careti기반의 오픈소스 사업 운용, 컨설팅 및 구축 지원 사업   
