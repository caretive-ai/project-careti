...
## 2. 아키텍처 (v2 - CaretiGlobalManager 패턴)
- **최소 침습 원칙**: Cline 원본 코드의 수정을 최소화합니다.
- **CaretiGlobalManager 활용**: Careti 전용 통합 상태 관리 패턴 사용
...
### 새로운 방식 (구현 완료)
```typescript
// ChatView.tsx - CaretiGlobalManager 패턴으로 구현됨
...
const enhancedMessageHandlers = useMemo(() => ({
    ...
    handleSendMessage: async (text: string, images: string[], files: string[]) => {
        addToHistory(text) // CaretiGlobalManager를 통한 즉시 저장
...
### 파일 1: `webview-ui/src/careti/hooks/usePersistentInputHistory.ts` (구현 완료)
```typescript
// CARETI MODIFICATION: Persistent input history hook using CaretiGlobalManager pattern
...
export function usePersistentInputHistory() {
    ...
    // Initialize CaretiGlobalManager resolver and load history
    useEffect(() => {
        ...
        CaretiGlobalManager.initInputHistoryResolver(saveToBackend)

        // Load existing history
        const loadHistory = async () => {
            try {
                const history = await CaretiGlobalManager.getInputHistory()
                setLocalHistory(history)
                console.log(`[INPUT-HISTORY] Loaded ${history.length} items from CaretiGlobalManager`)
            } catch (error) {
    ...
    const addToHistory = useCallback(async (text: string) => {
        ...
        // Save to backend via CaretiGlobalManager
        try {
            await CaretiGlobalManager.setInputHistory(newHistory)
...
### 파일 2: `webview-ui/src/careti/hooks/useInputHistory.ts` (기존 유지)
```typescript
...
+import { usePersistentInputHistory } from "@/careti/hooks/usePersistentInputHistory"

-	// CARETI MODIFICATION: Filter user messages for input history functionality
-	const userMessageHistory = useMemo(() => {
-		return messages
...
-					userMessageHistory={userMessageHistory} // CARETI MODIFICATION: Pass user message history for input history
+					inputHistory={inputHistory} // CARETI MODIFICATION: Pass persistent input history
 				/>
```
...
-	userMessageHistory?: string[] // CARETI MODIFICATION: Input history functionality
...
+	inputHistory?: string[] // CARETI MODIFICATION: Persistent input history functionality
 		...
 		onHeightChange,
 		onFocusChange,
-		userMessageHistory = [], // CARETI MODIFICATION: Input history functionality
+		inputHistory = [], // CARETI MODIFICATION: Persistent input history functionality
 	},
 	ref,
 ) => {

-		// CARETI MODIFICATION: Input history functionality - hook for handling arrow key navigation through message history
...
message UpdateSettingsRequest {
  // ... 기존 필드들
  // CARETI MODIFICATION: Input history for chat persistence (904 + 100 = 1004 to avoid conflicts)
  repeated string input_history = 1004;
}
```

### CaretiGlobalManager 확장 (구현 완료)
```typescript
...
```typescript
// CARETI MODIFICATION: Add input history
...
return {
    // ... 기존 필드들
    // CARETI MODIFICATION: Include input history
    inputHistory,
}
```
...
```typescript
// Before: 폴링으로 CaretiGlobalManager 체크
...
useEffect(() => {
    if (stateInputHistory !== undefined) {
        ...
        CaretiGlobalManager.setInputHistoryCache(stateInputHistory)
    }
}, [stateInputHistory])
```
