...
- **기능 분리**: 핵심 로직은 Careti 전용 커스텀 훅(`useInputHistory.ts`)으로 분리하여 `webview-ui/src/careti/hooks/` 디렉토리에 위치시킵니다.
...
### 파일 3: `webview-ui/src/careti/hooks/useInputHistory.ts` (신규 생성)
...
- VS Code Timeline 패널에서 다음 파일들의 히스토리 확인:
  ...
  - `webview-ui/src/careti/hooks/useInputHistory.ts` (새로 생성되었던 파일)
