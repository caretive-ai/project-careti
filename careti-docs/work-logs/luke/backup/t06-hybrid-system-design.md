...
Careti의 **'콘텐츠 관리 용이성(JSON)'**과 `cline-latest`의 **'기술적 유연성 및 안정성(TypeScript)'**이라는 두 시스템의 핵심 장점을 융합하여, 현존하는 가장 이상적인 프롬프트 시스템을 구축한다.

## 2. 핵심 아키텍처: `CaretJsonComponentProvider` (어댑터 패턴)

`careti-src/core/prompts/` 내부에 `CaretJsonComponentProvider.ts` 라는 어댑터 클래스를 구현한다. 이 클래스는 두 시스템을 연결하는 브릿지(bridge) 역할을 수행한다.

### `CaretJsonComponentProvider`의 역할

1.  **JSON 로딩**: `careti-src/core/prompts/sections/` 디렉토리 내의 모든 `.json` 파일을 로드한다.
...
4.  **레지스트리 등록**: `PromptRegistry`에 변환된 모든 Careti 컴포넌트를 등록하는 단일 메소드(`registerAll()`)를 제공한다.

## 3. 구현 예시: `SYSTEM_INFORMATION` 컴포넌트

| `Careti` (JSON) | `CaretJsonComponentProvider` (Adapter) | `cline-latest` (TS) |
| :--- | :--- | :--- |
...
4.  **Phase 4: 검증**
    ...
    -   Careti의 컴포넌트(JSON 기반)가 `cline-latest`의 프롬프트에 올바르게 포함되어 출력되는지 확인한다.
