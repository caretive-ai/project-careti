...
**핵심 목표:** 현재 `cline-latest` 기반 프로젝트(`careti-merging`)에 `careti-main`의 핵심 철학인 **CHATBOT/AGENT 모드**와 **JSON 기반 프롬프트 관리 시스템**을 **어댑터 패턴**을 통해 완벽하게 통합합니다.
...
- 현재 프로젝트에 이미 존재하는 `PromptRegistry` 시스템을 **활용**하여, `careti-main`의 JSON 시스템을 통합하는 방향으로 계획을 전면 수정합니다.
- **"구조는 cline-latest, 영혼은 Careti"** 라는 핵심 전략은 그대로 유지하되, "구조를 가져온다"가 아닌 **"이미 있는 구조를 활용한다"** 로 접근 방식을 변경합니다.
...
**목표:** 현재 시스템의 `PromptRegistry` 동작 방식을 완벽히 파악하고, 이를 바탕으로 Careti의 JSON 시스템을 연결할 어댑터의 핵심 구조를 TDD로 구현합니다.
...
3.  **[파일 생성]** `careti-src/core/prompts/` 경로에 다음 파일들을 생성합니다.
...
6.  **[GREEN] 관리자 및 연동 구현:**
    -   `PromptSystemManager`가 'careti' 모드일 때, `PromptRegistry.getInstance().registerComponent()`를 호출하여 기존 `tool_use_section` 컴포넌트를 `CaretJsonComponentProvider`의 어댑터로 **대체(override)**하도록 구현합니다.
...
-   기존 `PromptRegistry` 시스템 위에서 Careti의 JSON 기반 컴포넌트가 최소 1개 이상 성공적으로 동작합니다.
-   `npm run compile`이 성공합니다.

### Phase 2: Careti 핵심 철학 이식 및 기능 융합
...
-   '작업 관리 루프'가 Careti 철학에 맞게 차별화되어 동작합니다.
...
2.  **[GREEN] 프론트엔드 UI 구현:**
    -   `webview-ui/src/components/settings/providers/CaretiProvider.tsx`에 모드 전환 UI를 추가하고, `vscode.postMessage`를 호출하도록 구현합니다.
3.  **[GREEN] 백엔드 컨트롤러 구현:**
