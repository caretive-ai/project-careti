...
**최종 목표:** `cline-latest`의 **컴포넌트 기반 아키텍처**와 **작업 관리 루프**를 구조적 뼈대로 채택하고, 그 위에 Careti의 **CHATBOT/AGENT 행동 철학**과 **JSON 기반 콘텐츠 관리**의 장점을 완벽하게 통합합니다.
...
-   **`PromptSystemManager.ts` (전략 관리자):** `workspaceState`에 저장된 모드(`'careti'` | `'cline'`)에 따라 사용할 컴포넌트 제공자(전략)를 동적으로 선택합니다.
-   **`CaretJsonComponentProvider.ts` (어댑터):** Careti의 JSON 섹션들을 `cline-latest`의 `ComponentFunction` 형식으로 실시간 변환하여 `PromptRegistry`에 제공합니다.
...
-   **성과:** `cline-latest`와 Careti 시스템 분석을 완료하고, 통합 아키텍처 설계를 마쳤습니다.
...
4.  **[파일 생성]** `careti-src/core/prompts/` 경로에 `PromptSystemManager.ts`, `CaretJsonComponentProvider.ts`, 그리고 테스트를 위한 `__tests__/CaretJsonComponentProvider.test.ts` 파일을 생성합니다.
...
7.  **[GREEN] 관리자 연동:**
    -   `PromptSystemManager`가 'careti' 모드일 때, 다른 모든 컴포넌트는 `cline-latest` 순정을 사용하되, 오직 `tool_use_section`만 `CaretJsonComponentProvider`의 어댑터를 사용하도록 구현합니다.
8.  **[VERIFY]** `Mission 1B-1` 관련 테스트를 포함한 전체 테스트를 실행하여 통과하는지 확인합니다.

**완료 기준:** `Mission 1B-1` 테스트가 통과하고, `cline-latest` 시스템 위에서 Careti의 JSON 기반 컴포넌트가 하나 이상 성공적으로 동작함.

### Phase 2: Careti 핵심 철학 이식 및 신규 기능 융합

**목표:** Careti의 핵심 철학(CHATBOT/AGENT, 도구 제한)을 시스템에 완전히 통합하고, `cline-latest`의 신규 기능(작업 관리 루프)을 Careti 철학에 맞게 변형하여 적용합니다.
...
**완료 기준:** CHATBOT/AGENT 모드 및 도구 제한이 완벽하게 동작하고, '작업 관리 루프'가 Careti 철학에 맞게 차별화되어 동작함.

### Phase 3: 프론트엔드 통합 및 E2E 모드 전환 구현

**목표:** 사용자가 UI 설정을 통해 'Careti 하이브리드 시스템'과 'Cline 순정 시스템'을 실시간으로 전환할 수 있는 완전한 기능을 구현합니다.

1.  **[RED] E2E 테스트 작성 (`test:webview`):**
    -   `webview-ui/src/components/settings/providers/CaretiProvider.test.tsx` (가칭) 파일에 다음 시나리오를 검증하는 E2E 테스트를 작성합니다.
        1.  초기 상태는 'Cline' 모드이며, 생성된 프롬프트에 "ACT MODE"가 포함된다.
        2.  UI에서 'Prompt System'을 'Careti'으로 변경하는 이벤트를 발생시킨다.
        3.  `vscode.postMessage`로 `{ type: 'promptSystem/setMode', payload: 'careti' }` 메시지가 전송된다.
        4.  백엔드 `workspaceState`가 `'careti'`으로 업데이트된다.
        5.  이후 생성되는 프롬프트에는 "AGENT MODE"가 포함되고 "ACT MODE"는 포함되지 않는다.
2.  **[GREEN] 프론트엔드 UI 구현:**
    -   `webview-ui/src/components/settings/providers/CaretiProvider.tsx`에 `<select>` UI를 추가하고, `onChange` 이벤트 발생 시 위에서 정의한 `postMessage`를 호출하도록 구현합니다.
3.  **[GREEN] 백엔드 컨트롤러 구현:**
    -   `careti-src/controllers/PromptSystemController.ts` (신규) 파일에 `promptSystem/setMode` 메시지를 수신하는 핸들러를 구현합니다. 이 핸들러는 `workspaceState`를 업데이트하고 `PromptSystemManager.switchMode()`를 호출합니다.
