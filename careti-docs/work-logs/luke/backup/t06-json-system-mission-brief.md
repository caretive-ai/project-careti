...
`careti-main` 브랜치에서 이미 구현 및 검증 완료된 **JSON 기반의 동적 시스템 프롬프트**를 현재 `cline-latest` 기반의 `careti-merging` 프로젝트에 성공적으로 이식(merging)한다.

## 2. 배경 및 컨텍스트

- **Careti JSON 시스템의 우수성**: 기존 시스템은 `careti-main`에서 실제 운영 및 성능 테스트를 통해 이전 버전의 Cline 하드코딩 프롬프트(707라인) 대비 우수함이 **실험적으로 검증**되었다. (토큰 효율성 15% 향상, 모듈성, 확장성 확보)
...
- **구조적 차이**: `careti-merging` 프로젝트의 기반이 되는 `cline-latest`는 `careti-main` 개발 당시의 Cline 버전과 구조가 다를 수 있어, 단순 파일 복사 방식의 이식은 불가능하다.
...
- **실행**:
    ...
    2. **(중요)** `cline-latest`의 신규 컴포넌트(`TODO` 등 3개)를 제외한 나머지 컴포넌트들의 총 토큰(또는 문자) 양과, 그에 대응하는 Careti의 JSON 섹션들의 총 토큰 양을 비교하여, **Careti 시스템의 토큰 효율성 우위가 여전히 유효한지 검증**한다.

2.  **Phase 1: 핵심 시스템 이식 (Core System Migration)**
    - **목표**: 검증된 Careti의 JSON 시스템(어셈블러, 18개 섹션 파일, 테스트 등)을 `careti-src` 디렉토리로 안전하게 이식한다.

3.  **Phase 2: 시스템 통합 및 행동 원칙 적용 (Integration & Principle Application)**
    - **목표**: 분석 단계에서 식별한 통합 지점에 Careti JSON 시스템을 연결하고, '핵심 구현 원칙'에 명시된 `chatbot`/`agent` 모드의 행동 철학을 프롬프트 로직에 반영한다.
