...
### 🎯 최종 목표
`cline-latest`의 **진보된 컴포넌트 아키텍처**와 **작업 관리 루프**를 구조적 뼈대로 채택하고, Careti의 **CHATBOT/AGENT 행동 철학**과 **JSON 기반 콘텐츠 관리**의 장점을 완벽하게 통합한 **궁극의 하이브리드 프롬프트 시스템** 구축

### 핵심 전략: "구조는 cline-latest, 영혼은 Careti"
- **기반 아키텍처**: `cline-latest`의 `PromptRegistry` + 컴포넌트 시스템
- **핵심 철학**: Careti의 CHATBOT/AGENT 모드 + JSON 콘텐츠 관리 
...
- **토큰 효율성 검증**: Careti 시스템 14.1% 우위 확인
...
**[RED] TDD - 실패하는 테스트 작성**
-   `careti-src/core/prompts/`에 `PromptSystemManager.test.ts` 파일을 생성하고, `switchMode`가 'careti' 모드일 때 `CaretJsonComponentProvider`의 `registerCaretComponents` 메소드를 호출하는지 검증하는 실패하는 테스트를 작성합니다.

**[GREEN] TDD - 테스트를 통과하는 최소 코드 구현**
```typescript
// careti-src/core/prompts/CaretJsonComponentProvider.ts
...
// careti-src/core/prompts/PromptSystemManager.ts (전략 패턴)
import { CaretiGlobalManager } from "@careti/managers/CaretiGlobalManager";

export class PromptSystemManager {
    // ... (싱글톤 구현)
    
    public switchMode(mode: 'careti' | 'cline'): void {
        const registry = PromptRegistry.getInstance();
        // registry.clear(); // TODO: clear 로직 확인 필요

        // CaretiGlobalManager를 통해 중앙에서 관리되는 모드 사용
        if (CaretiGlobalManager.currentMode === 'careti') {
...
**검증 기준**:
- [ ] **`tsconfig.json`에 `@cline-latest` 경로 별칭 추가 완료** (계획 수정: `cline-latest`는 참고용이므로 직접 경로를 추가하지 않고, 필요한 로직을 `careti-src`에 재구현하는 방식으로 변경)
- [ ] **TDD 테스트 케이스(실패) 작성 완료**
- [ ] TypeScript 컴파일 오류 0개
- [ ] JSON 파일 로딩 정상 동작 (개선된 경로 로직)
- [ ] 최소 1개 컴포넌트 어댑터 작동 확인
- [ ] **`CaretiGlobalManager` 연동 확인**
...
**목표**: Careti 핵심 JSON 섹션을 cline-latest 컴포넌트로 변환 + CHATBOT/AGENT 철학 반영
...
**핵심 문제 해결**: cline-latest의 기술적 `Act/Plan` 용어를 Careti의 철학적 `Chatbot/Agent` 용어로 교체

```typescript
// [RED] 테스트: Careti 철학 반영
...
#### **2.3 신규 작업 관리 루프 Careti화 (1.5일)**
**목표**: cline-latest의 3대 신규 컴포넌트를 Careti 철학과 융합
...
# 토큰 효율성 재검증
node careti-scripts/utils/token-efficiency-analyzer.js
...
- [ ] **토큰 효율성 14.1% 이상 유지**
- [ ] **신규 작업 관리 루프 Careti 버전 완성**

---

### **Phase 3: 프론트엔드 통합 및 모드 전환 시스템 (2-3일)**
**목표**: 사용자가 UI에서 Careti/Cline 프롬프트 시스템을 전환할 수 있는 완전한 시스템 구현
...
describe('Prompt System Mode Switching E2E', () => {
    it('should switch from Cline to Careti mode via UI', async () => {
        // 1. 초기 상태: Cline 모드
        const initialPrompt = await buildSystemPrompt();
        expect(initialPrompt).toContain('ACT MODE');
        
        // 2. UI 조작: Careti 모드로 전환
        await webview.selectOption('#prompt-system-selector', 'careti');
        
        // 3. 백엔드 상태 변경 확인
        const savedMode = await workspaceState.get('careti.promptSystem.mode');
        expect(savedMode).toBe('careti');
...
const PromptSystemSelector: React.FC = () => {
    const [currentMode, setCurrentMode] = useState<'careti' | 'cline'>('cline');
    
    const handleModeChange = (newMode: 'careti' | 'cline') => {
    ...
    return (
        <div className="prompt-system-selector">
            ...
            <select 
                ...
                value={currentMode} 
                onChange={(e) => handleModeChange(e.target.value as 'careti' | 'cline')}
            >
                <option value="cline">Cline 원본</option>
                <option value="careti">Careti 하이브리드</option>
            </select>
            <p className="description">
                {currentMode === 'careti' 
                    ? 'Careti의 CHATBOT/AGENT 모드 + JSON 콘텐츠 관리'
                    : 'Cline 원본 프롬프트 시스템'
                }
            </p>
        </div>
...
```typescript
// careti-src/controllers/PromptSystemController.ts
class PromptSystemController {
    async handleSetMode(message: { payload: 'careti' | 'cline' }): Promise<void> {
        const newMode = message.payload;
        
        // 1. workspaceState에 모드 저장
        await this.context.workspaceState.update('careti.promptSystem.mode', newMode);
...
export function activate(context: vscode.ExtensionContext) {
    ...
    // 확장 시작 시 저장된 모드로 초기화
    const savedMode = context.workspaceState.get('careti.promptSystem.mode', 'cline');
...
node careti-scripts/utils/token-efficiency-analyzer.js \
  --careti-system "careti-src/core/prompts/" \
...
Gate 2: Careti 철학 반영 + 핵심 컴포넌트 + 토큰 효율성 유지
...
| **Phase 2** | 3-4일 | 핵심 컴포넌트 + Careti 철학 | CHATBOT/AGENT 반영 + 토큰 효율성 |
...
**예상 결과**: `cline-latest`의 **구조적 발전**과 Careti의 **철학적 우위**가 완벽하게 융합된 **궁극의 하이브리드 프롬프트 시스템** 완성
