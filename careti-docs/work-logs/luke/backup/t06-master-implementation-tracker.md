...
### 최종 목표
`cline-latest`의 **컴포넌트 기반 아키텍처 + 작업 관리 루프**와 Careti의 **CHATBOT/AGENT 철학 + JSON 콘텐츠 관리**를 완벽 융합한 **궁극의 하이브리드 프롬프트 시스템** 구축

### 핵심 전략: "구조는 cline-latest, 영혼은 Careti"
- **기반**: `cline-latest` PromptRegistry + 컴포넌트 시스템
- **철학**: Careti CHATBOT/AGENT + JSON 관리 + 모드별 도구 제한
...
##### 1.2 어댑터 뼈대 구현 (1일)
**디렉토리 생성**:
- [ ] `careti-src/core/prompts/` 디렉토리 생성
- [ ] `careti-src/core/prompts/__tests__/` 디렉토리 생성

**핵심 파일 생성**:
```typescript
// 1. PromptSystemManager.ts (전략 관리자)
- [ ] PromptSystemManager 클래스 뼈대
- [ ] switchMode('careti' | 'cline') 메서드
...
class PromptSystemManager {
    switchMode(mode: 'careti' | 'cline'): void { /* 구현 */ }
}
```
...
### **Phase 2: 핵심 컴포넌트 이식 + Careti 철학 통합 (3-4일)**
**목표**: Careti 핵심 철학 완전 반영 + 신규 작업 관리 루프 융합  
**상태**: ⏳ Phase 1 완료 후
...
// 어댑터 메서드 구현
- [ ] adaptAutoTodo() - TODO 관리 Careti 스타일
- [ ] adaptTaskProgress() - 진행상황 Careti 스타일  
- [ ] adaptFeedback() - 피드백 Careti 스타일
```
...
- [ ] node careti-scripts/utils/token-efficiency-analyzer.js 실행
...
```typescript
// [RED] E2E 테스트 파일 생성
// webview-ui/src/components/settings/providers/CaretiProvider.test.tsx
describe('Prompt System Mode Switching E2E', () => {
    it('should switch Cline to Careti mode via UI', () => {
        // 1. 초기: Cline 모드, "ACT MODE" 포함
        // 2. UI 조작: Careti 모드로 전환  
        // 3. 백엔드: workspaceState 'careti'으로 변경
...
- [ ] **select/toggle UI 구현** (Careti ↔ Cline 선택)
- [ ] **상태 관리** (`useState` 훅 사용)
...
- **Phase 2 완료**: Careti 철학 통합 (5-7일 후)
...
- **Phase 2 완료 후**: Careti 철학 반영 품질 검토  
