...
- **Careti 고유기능 보존 확인됨** (mode.agent, settings.uiLanguage 등)
...
### 대상 JSON 파일
```
webview-ui/src/careti/locale/[lang]/
├── common.json
├── welcome.json  
├── settings.json
├── chat.json (일부 언어)
...
1. **누락 키 리스트 분석**
   ```bash
   # 누락 키 보고서 확인
   cat careti-scripts/i18n-missing-keys-report.md
   ```

2. **기준 파일 구조 파악**
   ```bash
   # English JSON 파일들 구조 확인
   ls webview-ui/src/careti/locale/en/
   ```
...
```bash
# 1. Japanese 파일 현황 확인
ls webview-ui/src/careti/locale/ja/

# 2. English와 비교하여 누락 키 식별
grep -r "특정키" webview-ui/src/careti/locale/en/
grep -r "특정키" webview-ui/src/careti/locale/ja/
...
```bash  
# 1. Chinese 파일 현황 확인
ls webview-ui/src/careti/locale/zh/
...
```bash
# 키 존재 확인
grep -r "키명" webview-ui/src/careti/locale/
...
```bash
# Git 커밋 (작업 단위별)
git add webview-ui/src/careti/locale/
