...
1.  **파일 생성**: `webview-ui/src/careti/services/CaretModelInfo.ts` 파일을 생성합니다.
2. 
...
3.  **주석 추가**: 파일 상단에 이 파일이 Careti의 확장 로직임을 명시하는 주석을 추가합니다.
    ```typescript
    // CARETI MODIFICATION: This module imports original model definitions from 'src/shared/api.ts'
...
원본 `src/shared/api.ts`에 있던 모든 `description` 문자열을 `webview-ui/src/careti/locale/en/settings.json`에 추가합니다.
...
1.  **`import` 경로 변경**: 모델 정보를 사용하던 모든 UI 컴포넌트에서 `import` 경로를 `src/shared/api` 대신 새로운 `webview-ui/src/careti/services/CaretModelInfo`로 변경합니다.
