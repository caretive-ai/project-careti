...
현재 구조 (복잡함):
careti-b2b/tools/
├── cline-brand-change.js         # cline ↔ careti 전용
├── codecenter-brand.js           # careti ↔ codecenter 전용  
...
설정 파일 분산:
├── assets/brand-config.json    # cline ↔ careti 설정
└── brands/codecenter/brand.json      # codecenter 설정 (다른 형식)
```

### 2. 터미널 아이콘/이름 문제
**파일**: `careti-main/src/integrations/terminal/TerminalRegistry.ts:24-34`
```javascript
// 하드코딩된 문제
name: "Careti",                    // 고정된 이름
...
│   ├── careti/
│   │   ├── brand-config.json    # careti ↔ cline 매핑
│   │   └── assets/icons/        # careti 이미지  
│   └── codecenter/
│       ├── brand-config.json    # codecenter ↔ careti 매핑
...
node careti-b2b/tools/brand-converter.js careti forward   # careti config의 정방향 (careti→cline)
node careti-b2b/tools/brand-converter.js careti backward  # careti config의 역방향 (cline→careti)
node careti-b2b/tools/brand-converter.js codecenter forward   # codecenter config의 정방향 (careti→codecenter)
node careti-b2b/tools/brand-converter.js codecenter backward  # codecenter config의 역방향 (codecenter→careti)

# 상태 확인
node careti-b2b/tools/brand-converter.js --status   # 현재 브랜드 확인
```
...
- [ ] `assets/icons/` → `brands/careti/assets/icons/` 복사  
- [ ] `assets/brand-config.json` → `brands/careti/brand-config.json` 이동
...
#### 2.2 설정 파일 통일
- [x] `assets/brand-config.json` → `brands/careti/`로 이동
...
describe('Unified Brand Converter', () => {
  test('브랜드 자동 감지')
  test('careti ↔ cline 양방향 변환') 
  test('careti ↔ codecenter 양방향 변환')
...
#### 3.2 실제 변환 테스트
- [ ] careti → cline 변환 + 터미널 확인
- [ ] cline → careti 변환 + 터미널 확인  
- [ ] careti → codecenter 변환 + 터미널 확인
- [ ] codecenter → careti 변환 + 터미널 확인
...
# codecenter 설정 기반 변환  
node careti-b2b/tools/brand-converter.js codecenter forward   # careti → codecenter
node careti-b2b/tools/brand-converter.js codecenter backward  # codecenter → careti
```

## 📋 다음 단계 활용 방안

### ✅ 머징 작업 최적화
1. **머징 전 cline 변환**: 충돌 최소화를 위해 careti → cline 변환
2. **머징 후 careti 복원**: 새로운 기능 추가 후 cline → careti 변환
