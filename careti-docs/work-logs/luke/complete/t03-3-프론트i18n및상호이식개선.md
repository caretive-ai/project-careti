...
7. 캐러티 시작하기 페이지
 - 하드코딩 추정 : Careti supports various LLMs. You can subscribe to Careti service for free access, use ChatGPT, Claude, Gemma APIs, or configure local LLMs.

8. 프로바이더 번역 누락
 1) Careti
   {{providerName}} API Key
  providers.careti.login
  Enter API Key...
...
providers.careti.features
providers.careti.feature1
providers.careti.feature2
providers.careti.feature3
providers.careti.feature4

providers.careti.visit 
careti.ai providers.careti.getApiKey
 ...
 * 백엔드는 cline(upstream) <-> careti <-> codecenter(other brands) 의 스크립트의 완전 전환 지원
 * 프론트엔드는 cline(upstream) 기반의 현재 프론트코드의 i18n작업을 진행하고, careti고유의 기능을 잭 구현하여 cline과 분리
    -> 향후 머징은 cline의 구조적 변경을 파악후 반영 여부를 결정
    -> careti 과 codecenter는 스크립트를 통한 완전 전환 지원 (i18n파일과 필요 파일의 자동 교체로 구현)

## 🎯 작업 목적
### **왜 이 작업이 필요한가?**
- 프론트엔드에 하드코딩된 "Cline" 텍스트들이 브랜드 변환되지 않음
- 디자인 변경된 페이지들을 효율적으로 이식 필요
- careti-main에서 이미 구현된 다국어 지원 웰컴페이지를 활용 가능
- 완전 번역된 careti 프론트페이지의 브랜드 변환된 codecenter상호 변환 스크립트 지원필요

### **이 작업의 목표**
- 프론트엔드 i18n 적용 및 브랜드 변환 가능하게 만들기
- careti-main의 웰컴페이지 및 다국어 기능 이식
- brand-converter.js로 프론트엔드는 careti <-> codecenter로 일괄 변환 가능하게
 ...
 * cline의 원본 코드의 변경이지만, i18n은 CARETI MODIFICATION  주석을 달지 않는다.
 * careti의 모듈 삽입시에는 최소 침습 구현으로 외부 Careti 코드를삽입하고 CARETI MODIFICATION  주석을 달아 코드 블럭을 구분한다.
 * 구현시 캐러티의 개발가이드에 맞게 TDD를 반드시 수행 하도록 한다.
...
### Step1. i18n 기본 시스템 구현 (완료)
  ...
  * 전역 변수로 구축 하여 careti mode와 언어는 어디서나 접근할 수 있게 하며, cline의 기본 구현 아키텍처를 따른다.
   
### Step2. Cline의 코드를 기반으로 된 Careti-merging 프론트 코드에 i18n 적용 
  2.1. [ ] 현황 파악 및 작업 범위 구축 : 아래의  기능이 완벽히 구현되어있는지를 확인한다.
    [x] i18n과 CaretWebviewLogger는 최소 침습으로 구현 여부
      - App.tsx에 삽입하여 프론트의 모든 코드에서 사용할 수 있게 하라고 했었다. 
        - ✅ `webview-ui/src/App.tsx` - CaretiI18nProvider로 완전 구현됨
        - ✅ `webview-ui/src/context/ExtensionStateContext.tsx` - preferredLanguage 존재
        - ✅ **CaretWebviewLogger** - `webview-ui/src/careti/utils/CaretWebviewLogger.ts`에 완전 구현됨
  ...
  2.2.1. [x] `careti-scripts/tools/report-i18n-missing-file.js` - 누락된 번역 탐지  및 보고서 작성 스크립트 개발
   ...
   * [x] **기존 키 확인 스크립트 작성** - `careti-scripts/tools/find-existing-i18n-key.js`
   ...
   * [x] **2단계: careti-main 번역 파일 복사 및 한글로 복구** - 기존 번역 그대로 가져와서 복구 완료
  * [x] **임시 조치: i18n fallback 언어를 한글로 설정** - 다른 언어 파일 빈 상태에서 에러 방지
     - `webview-ui/src/careti/utils/i18n.ts`에서 fallback을 영어에서 한글로 변경
   ...
   * [ ] **5단계: 병렬 작업 병합 및 후속 작업** - `webview-ui-luke-parallel`의 작업물을 `webview-ui`로 통합하고 i18n 완성
     - **5.1. [x] 병합 및 충돌 해결**: `webview-ui-luke-parallel`과 `webview-ui` 디렉토리의 차이점을 비교(`diff`)하고, 병렬 작업의 변경 사항을 메인으로 수동 병합합니다. ✅ **차이점 분석 완료**
       ...
       - **분석 결과**: 
         ...
         * **프로바이더 차이**: main은 `CaretiProvider.tsx`, parallel은 없음 (다른 AI 작업과 충돌)
         * **병합 우선순위**: parallel의 더 많은 번역과 개선사항을 main으로 이전 필요
       - **⚠️ 다른 AI를 위한 작업 지시문**:
         * **병합 대상**: `webview-ui-luke-parallel/` → `webview-ui/`로 수동 병합
         * **우선 병합 파일**: 
           - `src/careti/locale/ko/settings.json` (30,956 bytes → main의 12,119 bytes 대체)
           - `src/careti/locale/ko/chat.json` (21,243 bytes → main의 20,100 bytes 대체)  
           - `src/careti/locale/en/browser.json` (parallel에만 존재 → main에 추가)
           - `src/careti/locale/ko/browser.json` (parallel에만 존재 → main에 추가)
           - `src/careti/locale/ko/menu.json` (parallel에만 존재 → main에 추가)
           - `src/careti/locale/en/menu.json` (parallel에만 존재 → main에 추가)
         * **CaretI18nContext.tsx 병합**: parallel의 단순화된 버전으로 교체 (VS Code state 감지 로직 제거)
         * **충돌 회피**: `CaretiProvider.tsx`는 main에만 존재하므로 건드리지 않음 (Luke Yang이 작업한 파일)
   ...
   * [ ] **6단계: 최종 검증 및 커밋 푸시** - 모든 i18n 작업 완료 확인
     ...
     - **6.2. [ ] i18n 종합 분석 실행**: `node careti-scripts/tools/report-i18n-unused-key.js`
       * 생성된 보고서(`careti-scripts/i18n-unused-keys-report.md`) 확인
  ...
  2.6. Careti-main 을 참고하여 Welcome 페이지, Announce 페이지의 개선을 진행한다. 
    [x] 코드를 확인하여 구현 범위를 확인하고 2.6.1.Careti고유 페이지 개선 계획 항목에 체크리스트 형태로 계획을 아래에 세운다. 사용자의 허락이 떨어지면 아래의 작업을 수행하고, 
    2.6.1. Careti 고유 페이지 개선 계획 (**상세 분석 완료 - careti-main vs careti-merging 비교**)
      - **현재 상태 (careti-merging)**: WelcomeView.tsx "Hi, I'm Cline" 등 완전 하드코딩 (83줄), Announcement.tsx 부분 i18n 적용 (110줄)
      - **완성 상태 (careti-main)**: WelcomeView.tsx 완전 i18n + 고도화된 UX (278줄), Announcement.tsx 완전 i18n + 동적 불렛 (114줄)
      ...
      **Step 2.6.1-A: WelcomeView.tsx 완전 개선 (50-60줄 → 278줄 변환)**
      ...
      - [ ] **2.6.1-A2**: Careti 전용 컴포넌트 도입 (향후 작업)
        - CaretWelcomeSection 컴포넌트 추가 (careti-main 참조 필요)
        - CaretApiSetup 컴포넌트 추가 (careti-main 참조 필요) 
        - CaretFooter 컴포넌트 추가 (careti-main 참조 필요)
        - CaretUILanguageSetting 컴포넌트 추가 (careti-main 참조 필요)
      - [ ] **2.6.1-A3**: 고도화된 UX 플로우 구현 (향후 작업)
        - 언어 선택 UI 2단 배치 (UI 언어, AI 응답 언어)
        - API 설정 전용 페이지 분리
        - 단계별 진행 플로우 (언어 선택 → API 설정 → 완료)
      - [x] **2.6.1-A4**: Cline 로고 → Careti 배너 교체 ⚠️ **부분 완료** 
      ...
      **Step 2.6.1-B: Announcement.tsx 완전 개선 (하드코딩 링크 제거)**  
      - [x] **2.6.1-B1**: 하드코딩 링크 완전 제거 ✅ **완료**
        - "X, discord, r/cline" → Careti Facebook, GitHub 링크로 교체
      ...
      - [ ] **2.6.1-B2**: 동적 불렛 시스템 구현 (향후 작업) 
        - bullets.current.1~10 시스템으로 확장 (careti-main 참조 필요)
        - bullets.previous.1~20 시스템으로 확장 (careti-main 참조 필요)  
        - 버전별 동적 불렛 생성 지원
      ...
      **✅ 실제 완료된 작업 상세**:
      - **WelcomeView.tsx**: 5개 하드코딩 텍스트 → i18n 키 변환
        ...
        - i18n import 추가: `import { t } from "@/careti/utils/i18n"`
        - 언어 변수 추가: `const currentLanguage = "ko"` (임시)
      - **Announcement.tsx**: 하드코딩 링크 → Careti 브랜드 링크 교체
        - `"X, discord, r/cline"` → Facebook, GitHub 링크로 교체
        - URL 하드코딩 → i18n 키 변환: `t("announcement.links.facebook", "common")` 등
        - Careti 브랜드 URL 적용: `https://www.facebook.com/groups/aicodingcaret`, `https://github.com/aicoding-careti/careti`
      ...
      **❌ 남은 작업 (향후 단계)**:
      - **2.6.1-A2~A3**: 고도화된 UX 컴포넌트 구현 (careti-main 참조 필요)
      ...
      - **2.6.1-C1~C4**: 의존성 컴포넌트 구현 (모두 careti-main 참조 필요)
      - **Careti 이미지 에셋**: caretBanner 이미지 추가
      - **일본어/중국어 번역**: ja/zh locale 파일 완성
      - **언어 동적 설정**: ExtensionStateContext에서 실제 언어 가져오기 (현재 "ko" 하드코딩)
   2.7. [x] `careti-scripts/tools/report-i18n-unused-key.js` - 미사용 키 탐지 및 보고서 작성 스크립트 개발
    * ✅ **스크립트 구현 완료**: 636개 키 중 596개 미사용(6.3% 사용률) 탐지
    * ✅ **보고서 생성**: `careti-scripts/i18n-unused-keys-report.md` 상세 분석 리포트
    * ✅ **매뉴얼 작성**: `careti-b2b/docs/i18n-unused-key-analyzer.md` 사용법 가이드
    * ✅ **README 연동**: careti-b2b README.md에 문서 링크 추가  
  2.7.1. 사용자 확인 후 커밋 푸시를 수행한다.
      

### Step3. i18n파일의 Cline <-> CodeCenter 전환 기능 구축
  3.1. [x] careti의 i18n 파일을 참고하여 codecenter의 i18n파일을 careti-b2b/레파지토리의 codecenter 에 만든다.
    * ✅ **백업 locale 폴더 생성**: `webview-ui/src/careti/locale-backup-step3` - 다른 AI 세션 작업과 독립적으로 진행
    * ✅ **CodeCenter 브랜드 설정**: `careti-b2b/brands/codecenter/brand-config-front.json` 완성
      - 한국어 브랜드 매핑: "캐러티" → "코드센터" 등 자연스러운 한국어 변환
      - 영어 브랜드 매핑: "Careti" → "CodeCenter" 포함
      - 비용 숨김 기능 설정: CodeCenter 요구사항 반영
    
  3.2. [x] 별도의 프론트엔드 전용 브랜드 변환 스크립트 구현
    * ✅ **frontend-brand-converter.js** 개발 완료 (`careti-scripts/tools/frontend-brand-converter.js`)
  ...
  3.3. [x] 테스트 및 검증 완료
    ...
    * ✅ **사용법**:
      ```bash
      # Careti → CodeCenter 변환
      node careti-scripts/tools/frontend-brand-converter.js careti forward --include-i18n-pages
      
      # CodeCenter → Careti 복구
      node careti-scripts/tools/frontend-brand-converter.js careti backward --include-i18n-pages
      
      # Dry-run 테스트
      node careti-scripts/tools/frontend-brand-converter.js careti forward --include-i18n-pages --dry-run --verbose
      ```
...
- **[ ] 4.4. Welcome 페이지 구조 개선 및 About 섹션 통합**
  - [ ] `welcome/WelcomeView.tsx`에 `careti-main`과 유사한 Footer 컴포넌트 구조 추가
...
### **🔍 careti-merging 현재 i18n 구현 상태 분석 완료**
#### ✅ **이미 구현된 i18n 인프라**
- **locale 파일들**: `webview-ui/src/careti/locale/` (30개 JSON 파일)
  - ko/en/ja/zh 4개 언어 지원
  - common, settings, welcome 등 7개 네임스페이스
- **i18n 유틸리티**: `webview-ui/src/careti/utils/i18n.ts` (고도화된 시스템 - 성능모니터링, 캐싱, 지연로딩)
- **Context Provider**: `CaretiI18nProvider` App.tsx에 완전 통합
...
### **🎯 핵심 발견사항 (코드 분석 기준)**
- **careti-merging**: 고도화된 i18n 인프라 100% 완성, **실제 활용 0%**
...
- **상세 보고서**: `careti-scripts/i18n-missing-files-report.md` (최신 버전으로 업데이트됨)
...
**1. (사전 확인) 기존 번역 키 탐색**
   ...
   - 터미널에서 아래 스크립트를 실행하세요.
     ```bash
     node careti-scripts/tools/find-existing-i18n-key.js "번역할 텍스트"
     ```
...
**3. (번역 추가) `locale` JSON 파일 수정**
   - `webview-ui/src/careti/locale/ko/` 와 `en/` 폴더 밑의 네임스페이스 JSON 파일(`settings.json` 등)을 엽니다.
...
3. **전환**: careti↔codecenter는 locale 폴더 전체 교체

## 📊 careti-merging 현재 상태 상세 분석 (2025-09-02)
...
- **careti-merging/**: 현재 작업 중 (i18n 부분 적용)
- **careti-main/**: 완전한 i18n 구현 (참조용)
...
#### **Stage 1: cline ↔ careti (하드코딩 → i18n 변환)**
...
- **중요**: 브랜드명은 JSON 파일 내에서 관리
  ...
  // en/chat.json
  {
    "clineWantsToExecute": "Careti wants to execute this command"
  }
  ```

#### **Stage 2: careti ↔ codecenter (locale 폴더 교체)**
- **목적**: 브랜드별로 다른 locale 폴더 사용
- **방식**: 폴더 전체를 교체 (파일 수준 작업)
- `careti-b2b/brands/careti/locale/` → `webview-ui/src/careti/locale/`
- `careti-b2b/brands/codecenter/locale/` → `webview-ui/src/careti/locale/`
- **중요**: 코드는 동일, JSON 파일만 교체

#### **Phase 1: 최소 침습적 i18n 통합**
1. **Level 1 아키텍처 유지**
   - `/src/careti/` 폴더 내 독립적 i18n 시스템
   - 기존 Cline 코드 최소 수정
   - CARETI MODIFICATION 주석으로 추적
...
2. **i18n 키 기반 렌더링**
   ...
   // locale/en/chat.json (careti 브랜드)
   {
     "clineWantsToExecute": "Careti wants to execute this command"
   }
...
##### **brand-config-careti-to-codecenter.json (파일 교체용)**
```json
{
  "from": "careti",
  "to": "codecenter",
  "files": {
    "locale": {
      "source": "careti-b2b/brands/codecenter/original/locale/",
      "target": "webview-ui/src/careti/locale/",
      "backup": "careti-b2b/brands/careti/backup/locale/"
    },
    "i18n": {
      "source": "careti-b2b/brands/codecenter/original/utils/i18n.ts",
      "target": "webview-ui/src/careti/utils/i18n.ts",
      "backup": "careti-b2b/brands/careti/backup/utils/i18n.ts"
    }
  }
}
```
...
#### **A. 컴포넌트 이식**
```
/webview-ui/src/careti/pages/welcome/
...
node careti-b2b/tools/brand-converter.js careti forward --include-i18n-pages
node careti-b2b/tools/brand-converter.js careti backward --include-i18n-pages
node careti-b2b/tools/brand-converter.js codecenter forward --include-i18n-pages
node careti-b2b/tools/brand-converter.js codecenter backward --include-i18n-pages
```
...
describe('I18n Pages Auto Conversion', () => {
  ...
  test('should support cline ↔ careti ↔ codecenter conversion')
})
```
...
### **구현 전략**
1. **cline → careti**: 하드코딩 텍스트를 i18n 키로 변환 (TSX 파일 수정)
2. **careti ↔ codecenter**: locale + i18n.ts 파일 전체 교체 (파일 스왑)
...
node careti-b2b/tools/brand-converter.js frontend apply
...
│   └── converted/            # Cline→Careti 변환 후 파일
│       ├── locale/           # 기본 영어 locale (BRAND: "Cline")
│       └── utils/i18n.ts    # i18n 시스템
├── careti/
│   ├── original/             # Careti 원본 (i18n 적용)
│   │   ├── locale/           # 다국어 locale (BRAND: "Careti")
│   │   │   ├── ko/*.json
│   │   │   ├── en/*.json
│   │   │   ├── ja/*.json
│   │   │   └── zh/*.json
...
webview-ui/src/careti/locale/**/*.json

# 브랜딩 설정 
careti-b2b/brands/careti/brand-config.json 
