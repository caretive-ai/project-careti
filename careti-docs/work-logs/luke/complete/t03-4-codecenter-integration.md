...
### 1. **백엔드 메시지 i18n 미연동**
- **현황**: `src/core/task/tools/handlers/*.ts`에 "Careti wants to..." 하드코딩 메시지 10개 이상
...
- ✅ **완료**: 현재 `webview-ui/src/careti/locale/` 상태 확인 (11개 파일, 4개 언어)
...
- 📂 **소스**: `webview-ui/src/careti/locale/*`
- 📂 **타겟**: `careti-b2b/brands/codecenter/locale/*` 
...
- ✅ **완료**: webview 빌드 시 assets 자동 복사 (`careti-scripts/build/copy-template-characters.cjs`)
- ✅ **완료**: .vscodeignore 수정으로 careti-src 디렉토리 VSIX 포함
...
#### 2.3 전체 변환 테스트
- 🔄 **테스트 시나리오**: careti ↔ codecenter ↔ cline 순환 변환
...
- 📋 **대상 메시지들**:
  ```typescript
  "Careti wants to edit/create {filename}"
  "Careti wants to use a browser and launch {url}"
  "Careti wants to fetch content from {url}"
  "Careti wants to use {tool_name} on {server_name}"
  "Careti wants to access {uri} on {server_name}"
  "Careti wants to search files for {regex}"
  "Careti wants to create a github issue..."
  "Careti wants to read {filename}"
  ```

#### 3.2 i18n 키 직접 매핑 방식 구현 (방식 B)
- 🗃️ **구현**: `webview-ui/src/careti/locale/*/common.json`에 백엔드 메시지 키 추가
- 🔧 **키 구조**:
  ```json
  {
    "Careti wants to edit {{filename}}": "{{brand.appName}}이 {{filename}}을 편집하려고 합니다",
    "Careti wants to create {{filename}}": "{{brand.appName}}이 {{filename}}을 생성하려고 합니다",
    "Careti wants to use a browser": "{{brand.appName}}이 브라우저를 사용하려고 합니다"
  }
  ```
...
- 📁 **구조**: 
  ```
  careti-src/core/prompts/templates/
  ├── careti/          # Careti 전용 템플릿
...
3. ✅ **Careti 최신 i18n**: 14개 locale 파일, 4개 언어 완성
...
- **작업 내용**:
  1. Careti의 최신 locale 파일을 CodeCenter로 복사
  2. 브랜드명 일괄 변환 (Careti → CodeCenter, 캐러티 → 코드센터)
...
- **작업 내용**:
  ```bash
  # 신규 스크립트 필요
  node careti-scripts/tools/locale-brand-converter.js careti codecenter
  ```
  - Careti locale → CodeCenter locale 자동 변환
  - 브랜드명 replace 규칙 적용
  - JSON 유효성 검증
...
1. **완전한 브랜드 전환**:
   - 3개 명령어로 careti ↔ codecenter 완전 변환
...
- [ ] **성능**: 변환 시간 2분 이내
- [ ] **안정성**: 순환 변환 (careti ↔ codecenter) 무오류

## 🔧 사용할 도구 및 스크립트

### **기존 도구**
- `careti-b2b/tools/brand-converter.js` - 통합 브랜딩 변환
- `careti-b2b/tools/brand-converter-frontend.js` - 프론트엔드 변환 (통합 예정)
- `careti-scripts/tools/i18n-key-synchronizer.js` - i18n 키 동기화

### **새로 개발할 도구**
- `careti-scripts/tools/backend-message-i18n-mapper.js` - 백엔드 메시지 i18n 매핑
- `careti-scripts/tools/brand-locale-sync.js` - 브랜드별 로케일 동기화
- `careti-scripts/tools/json-prompt-template-manager.js` - JSON 템플릿 관리

### **Phase 5: 추가 발견 및 해결된 문제들** (2025-01-09 추가)

#### 5.1 VSIX 빌드 시스템 개선
- ✅ **완료**: `package:release` 스크립트 경로 수정 (`codecenter-scripts` → `careti-scripts`)
- ✅ **완료**: `@types/lodash` 의존성 누락 해결
- ✅ **완료**: `.vscodeignore` whitelist 방식에서 `careti-src` 디렉토리 누락 해결
...
- ✅ **완료**: `careti-scripts/build/copy-template-characters.cjs` 스크립트 생성
- ✅ **완료**: ES Module 호환성 문제 해결 (`.js` → `.cjs`)

#### 5.3 Extension 활성화 오류 해결
- 🚨 **발견**: VS Code Extension Host에서 `careti-src\core\prompts\sections` 디렉토리 누락 오류
- ✅ **해결**: `.vscodeignore`에 `!careti-src/**` 추가하여 VSIX 포함
- ✅ **검증**: 새 VSIX 패키지에서 `careti-src/ (43 files) [102.73 KB]` 포함 확인
...
- **브랜딩 규칙**: `careti-b2b/brands/codecenter/brand-config.json`
- **i18n 시스템**: `careti-docs/features/f02-multilingual-i18n.md`
