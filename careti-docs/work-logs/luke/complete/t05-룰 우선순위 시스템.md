 ...
 * careti-compare 프로젝트에 이미 구현 되어있으므로 해당 프로젝트 참고
...
├── external-rules.ts              # 핵심 우선순위 로직 (CARETI MODIFICATION)
├── rule-helpers.ts                # 규칙 동기화 헬퍼 함수들
└── cline-rules.ts                 # Cline 전용 규칙 처리

시스템 프롬프트 통합:
src/core/task/
└── index.ts                       # 우선순위 시스템 통합 (CARETI MODIFICATION)

백엔드 제어 로직:
src/core/controller/file/
└── refreshRules.ts                # UI 데이터 전송 (CARETI MODIFICATION)

Careti 백엔드 분리 로직:
careti-src/core/controller/file/
└── toggleCaretRule.ts             # .agents/context 토글 기능 (NEW)

파일 시스템 정의:
src/core/storage/
├── disk.ts                        # .agents/context 파일명 정의 (CARETI MODIFICATION)
├── state-keys.ts                  # 상태 키 정의 (CARETI MODIFICATION)
└── utils/state-helpers.ts         # 상태 초기화 (CARETI MODIFICATION)

프로토콜 정의:
proto/cline/
└── file.proto                     # gRPC 통신 프로토콜 (CARETI MODIFICATION)

UI 컴포넌트:
webview-ui/src/components/cline-rules/
└── ClineRulesToggleModal.tsx      # 규칙 토글 UI (CARETI MODIFICATION)

UI 상태 관리:
webview-ui/src/context/
└── ExtensionStateContext.tsx      # 전역 상태 관리 (CARETI MODIFICATION)
```
...
- [ ] 기존 단위 테스트 이식
  ```bash
  cp careti-main/src/test/rule-priority.test.ts src/test/
  cp careti-main/src/test/rule-priority-integration.test.ts src/test/
  ```
- [ ] 테스트 실행 확인
  ```bash
...
- [ ] 핵심 로직 파일 이식
  ```bash
  cp careti-main/src/core/context/instructions/user-instructions/external-rules.ts \
     src/core/context/instructions/user-instructions/
  ```
- [ ] toggleCaretRule 기능 이식
  ```bash
  cp careti-main/careti-src/core/controller/file/toggleCaretRule.ts \
     careti-src/core/controller/file/
  ```
- [ ] 시스템 프롬프트 통합
  ```bash
...
- [ ] 저장소 파일 수정
  ```bash
  # src/core/storage/disk.ts에 caretRules: ".agents/context" 추가
...
```typescript
// CARETI MODIFICATION: Implement rule priority system (.agents/context)
...
if (caretHasFiles) {
    // .agents/context has highest priority - disable all others
    ...
    // .agents/context has second priority - disable cursor
    updatedLocalCursorToggles = disableAllToggles(updatedLocalCursorToggles)
}
...
```typescript
// CARETI MODIFICATION: Rule priority system (.agents/context)
...
// CARETI MODIFICATION: Use priority system - only pass the active rule instead of all rules
...
- [ ] 마커 확인: `// CARETI MODIFICATION:` 주석으로 수정 부분 명확히 표시
...
- [ ] UI 통합: Rules Toggle Modal에 .agents/context 표시
- [ ] 우선순위 로직: 파일 존재 시 낮은 우선순위 비활성화
- [ ] CARETI MODIFICATION: 모든 수정 부분 주석 표시

## 호환성 및 마이그레이션

### 기존 사용자 호환성
- Cline 사용자: 기존 `.agents/context` 파일 그대로 사용 가능
- Cursor 사용자: 기존 `.agents/context` 파일 그대로 사용 가능  
- Windsurf 사용자: 기존 `.agents/context` 파일 그대로 사용 가능

### 마이그레이션 가이드
1. 기존 규칙 확인
2. 우선 규칙 선택 (`.agents/context`로 복사 또는 이름 변경)
3. 중복 제거
