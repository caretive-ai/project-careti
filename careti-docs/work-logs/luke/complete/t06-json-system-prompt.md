...
`cline-latest`의 진보된 **컴포넌트 아키텍처**와 **'작업 관리 루프'**를 구조적 뼈대로 채택하고, 그 위에 Careti의 **CHATBOT/AGENT 행동 철학**과 **JSON 기반 콘텐츠 관리**의 장점을 완벽하게 통합하여, 두 시스템의 장점만을 결합한 **궁극의 하이브리드 프롬프트 시스템**을 구축한다.

---

## 2. 📜 Careti 개발 원칙

이 작업과 모든 하위 작업은 다음의 Careti 핵심 개발 원칙을 반드시 준수해야 합니다.
...
## 4. 핵심 전략: "구조는 cline-latest, 영혼은 Careti"

-   **기반 아키텍처**: 현재 프로젝트에 이미 존재하는 `cline-latest`의 `PromptRegistry`와 컴포넌트 시스템을 그대로 활용한다.
-   **핵심 철학**: Careti의 CHATBOT/AGENT 모드, `mode_restriction`을 통한 도구 제한, JSON을 통한 콘텐츠 관리 및 토큰 효율성의 장점을 모두 계승한다.
