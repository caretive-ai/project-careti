...
`cline-latest`의 구조적 발전, 특히 **'작업 관리 루프'(`auto_todo`, `task_progress`, `feedback`)**를 코드 레벨에서 완벽하게 분석하고, 그 기능과 철학을 수용할 수 있도록 기존 Careti JSON 시스템을 확장하여 두 시스템 간의 **기능적 동등성(Functional Parity)**을 확보한다.
...
### 3.2. Careti JSON 확장 설계 (⚠️ 영어 + 토큰 효율성 중심)
...
### 3.3. 신규 JSON 파일 생성
- [x] `careti-src/core/prompts/sections/` 디렉토리 존재 여부 확인 및 생성
...
### 3.4. 토큰 효율성 사전 검증
- [x] **기존 토큰 측정 스크립트 확장**: `careti-main/careti-scripts/utils/system-prompt-token-measurement.js` 수정
    - [x] `measureSystemPrompts()` 함수에 하이브리드 모드 측정 로직 추가:
      ```javascript
      // 4. 하이브리드 모드 (cline-latest PromptRegistry + Careti JSON)
      ...
      ```
    - [x] 하이브리드 vs Cline vs Careti 3-way 비교 결과 출력
...
- [x] **Phase 검증 스크립트 생성**: `careti-scripts/tools/phase-validator.js` 신규 생성
    ```javascript
    class PhaseValidator {
        validatePhase1() {
            ...
            requiredFiles.forEach(file => {
                if (!fs.existsSync(`careti-src/core/prompts/sections/${file}`)) {
...
- [ ] **AI 시맨틱 분석** (선택사항): `careti-scripts/utils/ai-semantic-analyzer.js` 활용
...
### 필수 완료 기준
- [x] `cline-latest`의 '작업 관리 루프' 3개 컴포넌트의 모든 기능이 Careti의 CHATBOT/AGENT 철학이 반영된 3개의 신규 JSON 파일로 완벽하게 재정의됨.
- [ ] **토큰 효율성**: 하이브리드 시스템이 기존 Cline 대비 최소 10% 이상 토큰 절약 효과 검증됨.
- [ ] **검증 자동화**: `careti-scripts/tools/phase-validator.js`를 통한 자동 검증이 100% 통과함.
