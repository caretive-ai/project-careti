...
`cline-latest`의 `PromptRegistry` 아키텍처와 Careti의 JSON 기반 시스템을 연결하는 **어댑터(Adapter)와 전략(Strategy) 패턴의 핵심 뼈대**를 `careti-src/` 내에 구축한다. 이 과정에서 Careti의 핵심 철학인 **CHATBOT/AGENT 모드 분리**와 **`mode_restriction` 기반 도구 제한** 로직을 새로운 구조에 이식하여, 시스템의 '영혼'을 불어넣는다.
...
- [x] **`PromptSystemManager` (전략 패턴) 설계**
    - [x] `careti-src/core/prompts/system/` 디렉토리 생성
    - [x] `PromptSystemManager.ts` 파일 생성: `cline` 또는 `careti` 시스템을 선택하고 위임하는 역할
    - [x] `IPromptSystem.ts` 인터페이스 정의: `getPrompt(context)` 메서드를 포함
- [x] **`CaretiJsonAdapter` (어댑터 패턴) 설계**
    - [x] `careti-src/core/prompts/system/adapters/` 디렉토리 생성
    - [x] `CaretiJsonAdapter.ts` 파일 생성: `IPromptSystem` 인터페이스를 구현하며, Phase 1에서 생성한 JSON 파일들을 조합하는 역할
...
- [x] **테스트 파일 생성**
    - [x] `careti-src/__tests__/tdd/` 디렉토리에 `T06PromptSystemIntegration.test.ts` 파일 생성
- [x] **통합 시나리오 테스트 작성 (실패 예상)**
    - [x] `PromptSystemManager`가 'careti' 모드일 때 `CaretiJsonAdapter`를 통해 CHATBOT/AGENT 모드별 프롬프트를 올바르게 생성하는지 검증하는 테스트 코드 작성
...
- [x] **`PromptSystemManager.ts` 기본 로직 구현**
    - [x] 생성자에서 `CaretiJsonAdapter`와 `ClineLatestAdapter`를 초기화
    - [x] `getPrompt(context)` 메서드에서 `context.modeSystem` 값에 따라 적절한 어댑터로 위임하는 로직 구현
- [x] **`CaretiJsonAdapter.ts` 핵심 로직 구현**
...
- [x] **상수화 및 타입 정의**
    - [x] `careti-src/shared/constants/`에 `PromptSystemConstants.ts` 파일 생성
    - [x] `"careti"`, `"cline"`, `"chatbot"`, `"agent"` 등 모든 문자열을 상수로 대체
- [x] **로직 분리**
    - [x] `CaretiJsonAdapter` 내에서 JSON 파일을 로드하는 부분, 조합하는 부분, 필터링하는 부분을 명확한 private 메서드로 분리
- [x] **주석 및 문서화**
...
- [x] **Phase 검증 스크립트 확장**: `careti-scripts/tools/phase-validator.js` 수정
    - [x] `validatePhase2()` 메서드 추가
    - [x] `PromptSystemManager.ts`, `CaretiJsonAdapter.ts`, `ClineLatestAdapter.ts` 파일 존재 여부 검증
...
1. **사용자/다른 AI에게 검증 요청**:
   ...
   - PromptSystemManager가 careti/cline 모드를 올바르게 전환하는지
   - CaretiJsonAdapter가 JSON 파일을 정상적으로 로드하고 조합하는지  
...
- [ ] `PromptSystemManager`와 두 개의 어댑터(`CaretiJsonAdapter`, `ClineLatestAdapter`) 파일이 `careti-src/` 내에 생성되고, TDD 통합 테스트를 통과함.
- [ ] `CaretiJsonAdapter`는 CHATBOT/AGENT 모드에 따라 동적으로 JSON 프롬프트를 조합하며, `mode_restriction` 철학을 완벽하게 구현함.
- [ ] `careti-scripts/tools/phase-validator.js`의 `validatePhase2()` 검증이 100% 통과함.
