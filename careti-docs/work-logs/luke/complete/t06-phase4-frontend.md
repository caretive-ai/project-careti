...
DEBUG [getSystemPrompt] Current mode: careti
DEBUG [getSystemPrompt] Using Careti PromptSystemManager for AGENT MODE
DEBUG [PromptSystemManager] Using adapter: careti
...
- Careti 모드에서 초기화 실패 오류 노출 (챗봇 모드, 에이전트 모드 동일, 현재 cline의 plan, act모드는 정상 동작 확인하였음)
...
workbench.desktop.main.js:55   ERR [Extension Host] [ErrorService] Logging Error: JsonTemplateLoader has not been initialized. Call initialize() first.
	at _ClineError.transform (d:\dev\careti-merge\dist\extension.js:437664:18)
	at ErrorService.toClineError (d:\dev\careti-merge\dist\extension.js:437743:40)
	at Task.recursivelyMakeClineRequests (d:\dev\careti-merge\dist\extension.js:886946:47)
	at async Task.initiateTaskLoop (d:\dev\careti-merge\dist\extension.js:886052:30)
	at async Task.resumeTaskFromHistory (d:\dev\careti-merge\dist\extension.js:886046:9)
workbench.desktop.main.js:4784 [Extension Host] [ErrorService] Logging Error: JsonTemplateLoader has not been initialized. Call initialize() first.
	...
	at Task.recursivelyMakeClineRequests (d:\dev\careti-merge\dist\extension.js:886946:47)
	at async Task.initiateTaskLoop (d:\dev\careti-merge\dist\extension.js:886052:30)
	at async Task.resumeTaskFromHistory (d:\dev\careti-merge\dist\extension.js:886046:9) (at console.<anonymous> (file:///c:/Users/Luke(%EC%96%91%EB%B3%91%EC%84%9D)/AppData/Local/Programs/c…/resources/app/out/vs/workbench/api/node/extensionHostProcess.js:200:31986))
workbench.desktop.main.js:55   ERR [Extension Host] Error setting up file subscription: Error: ENOENT: no such file or directory, watch 'c:\Users\Luke(양병석)\AppData\Roaming\Cursor\User\globalStorage\caretive.careti\tasks\1757332446552\focus_chain_taskid_1757332446552.md'
workbench.desktop.main.js:4784 [Extension Host] Error setting up file subscription: Error: ENOENT: no such file or directory, watch 'c:\Users\Luke(양병석)\AppData\Roaming\Cursor\User\globalStorage\caretive.careti\tasks\1757332446552\focus_chain_taskid_1757332446552.md' (at console.<anonymous> (file:///c:/Users/Luke(%EC%96%91%EB%B3%91%EC%84%9D)/AppData/Local/Programs/c…/resources/app/out/vs/workbench/api/node/extensionHostProcess.js:200:31986))
ExtensionStateContext.tsx:422 [DEBUG] returning new state in ESC
ExtensionStateContext.tsx:431 [DEBUG] ended "got subscribed state"
ExtensionStateContext.tsx:422 [DEBUG] returning new state in ESC
ExtensionStateContext.tsx:431 [DEBUG] ended "got subscribed state"
workbench.desktop.main.js:55  WARN [mainThreadStorage] large extension state detected (extensionId: caretive.careti, global: true): 1332.716796875kb. Consider to use 'storageUri' or 'globalStorageUri' to store this data on disk instead.
ExtensionStateContext.tsx:422 [DEBUG] returning new state in ESC
ExtensionStateContext.tsx:431 [DEBUG] ended "got subscribed state"
﻿
--- 백엔드 로그
...
- **참고 문서**: `careti-docs/features/f02-multilingual-i18n.md`
- **해결 방법**: 올바른 네임스페이스와 JSON 키 매핑 수정 필요

### 2. Careti 모드 시 하단 버튼 변경
**문제**: Careti 토글 활성화 시 하단 Plan/Act 버튼이 Chatbot/Agent 모드 버튼으로 변경되어야 함
- **현재 상태**: cline-merge에 미구현 (careti-main에는 구현됨)
- **구현 필요**: careti-main 참고하여 모드별 하단 버튼 전환 로직 구현
...
### 🎯 해결된 문제들
1. **앱 첫 설치 시 기본값 불일치**
   - `extension.ts`: workspaceState 기본값 `'cline'` → `'careti'`
...
3. **완전 초기화 기능 구현**
   - `resetWorkspaceState()`: Careti 기본값 복원 로직 추가
   - `resetGlobalState()`: 전역 상태도 Careti 기본값으로 초기화
   - `SettingsView.tsx`: localStorage 초기화 + 페이지 리로드

### 📝 상세 수정 내용
- **백엔드**: `state-helpers.ts`, `resetState.ts` - CaretiGlobalManager 재초기화
- **프론트엔드**: 앱 시작 시 `GetPromptSystemMode()` 호출로 상태 동기화
- **설정 디버그**: "완전 초기화" 버튼으로 모든 Careti 설정을 기본값으로 복원
...
### 🔧 해결된 네임스페이스 문제
1. **Proto 생성 스크립트 수정**
   - `careti-scripts/build/generate-protobus-setup.mjs`: `CaretSystemService`도 `careti` 네임스페이스 처리
   - `careti-scripts/build/build-proto.mjs`: webview-ui 파일 후처리 추가

2. **gRPC 클라이언트 수정**
   - `webview-ui/src/services/grpc-client.ts`: `"cline.CaretSystemService"` → `"careti.CaretSystemService"`
   - Request/Response 타입도 모두 `careti.*` 네임스페이스로 정상 생성
...
### 📝 구현 요약
**핵심 아키텍처**:
1. **gRPC 서비스 정의** - `proto/careti/system.proto`
...
2. **백엔드 gRPC 핸들러** - `src/core/controller/caretSystem/`
   ...
   - `CaretiGlobalManager` 연동으로 실시간 상태 관리

3. **프롬프트 시스템 분기** - `src/core/prompts/system-prompt/index.ts` (CARETI MODIFICATION)
   - `CaretiGlobalManager.currentMode` 기반 Option 4 분기 로직
   - `careti` 모드: `PromptSystemManager` + `CaretiJsonAdapter` (AGENT MODE)
...
*   **L1 독립 모듈 선호**: `careti-src/` 내의 독립적인 모듈 구현을 최우선으로 하여 Cline 원본 코드 수정을 최소화합니다.

---

## 2. 🎯 Phase 목표

사용자가 VSCode 설정 UI를 통해 **'Careti 하이브리드 시스템'**과 **'Cline 순정 시스템'**을 실시간으로 전환할 수 있는 완전한 기능을 구현한다. 이 모든 과정은 E2E(End-to-End) 테스트를 통해 철저히 검증하며, 사용자의 선택이 확장 프로그램을 재시작해도 유지되도록 **설정 영속성**을 확보한다.
...
- [x] **테스트 파일 생성**: `webview-ui/src/careti/components/__tests__/PromptSystemSwitcher.e2e.test.tsx` 파일 생성.
- [x] **E2E 테스트 시나리오 작성:**
    - [x] **(시나리오 1: 모드 전환)**
        - [x] 초기 상태('Cline' 모드)에서 생성된 프롬프트에 "ACT MODE"가 포함되는지 검증 (lines 57-58).
        - [x] UI에서 'Careti' 모드로 변경하는 이벤트를 시뮬레이션 (line 61).
        - [x] `vscode.postMessage`로 `{ type: 'promptSystem/setMode', payload: 'careti' }` 메시지가 전송되는지 검증 (lines 64-69).
        - [x] 백엔드 `workspaceState`가 `'careti'`으로 업데이트되었음을 가정하고, 이후 생성되는 프롬프트에 "AGENT MODE"가 포함되고 "ACT MODE"는 포함되지 않는지 검증 (lines 75-77).
    - [x] **(시나리오 2: 설정 영속성)**
        - [x] 'Careti' 모드로 설정 후, 확장 프로그램 재시작을 시뮬레이션 (lines 82-83, 98-100).
        - [x] 재시작 후에도 별도 조작 없이 생성된 프롬프트에 "AGENT MODE"가 포함되는지 검증 (lines 86-87, 102-103).

### 3.2. [GREEN] 프론트엔드 UI 및 로직 구현
- [x] **UI 컴포넌트 생성**: `webview-ui/src/careti/components/PromptSystemSwitcher.tsx` 파일 생성.
    - [x] `select` 또는 토글 스위치를 사용하여 'Careti'과 'Cline' 모드를 선택할 수 있는 UI 구현.
...
- [x] **설정 페이지 통합**:
    ...
    - [x] `webview-ui/src/careti/components/CaretGeneralSettingsSection.tsx` 파일에서 `PromptSystemSwitcher` 컴포넌트가 line 10에서 import되고 lines 32-34에서 렌더링됨.
    - [x] `// CARETI MODIFICATION` 주석과 함께 적절한 위치에 통합 완료.
...
- [ ] **메시지 프로토콜 정의**: 프론트엔드-백엔드 간 통신 인터페이스 명확화
    ...
    interface PromptSystemModeStateMessage {
        ...
        payload: {
            currentMode: 'careti' | 'cline'
            isInitialized: boolean
        }
    }
    ```
- [x] **컨트롤러 생성**: `careti-src/controllers/PromptSystemController.ts` 파일 생성.
    - [x] `handleSetMode(mode: 'careti' | 'cline')` 메서드 구현 (lines 15-36).
    - [x] 이 메서드는 `workspaceState.update('careti.promptSystem.mode', mode)`를 호출하여 설정을 영속적으로 저장 (line 20).
