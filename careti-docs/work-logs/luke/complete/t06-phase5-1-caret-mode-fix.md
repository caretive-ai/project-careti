# t06-phase5-1: Careti Mode System 정리 및 안정화
...
현재 Careti 모드 시스템(chatbot/agent)이 f06+f07 하이브리드 방식으로 구현되어 있으나, 아키텍처 문서와 실제 구현 간 불일치가 발생하고 있어 이를 정리하고 안정화하는 것이 목적입니다.
...
4. proto/ - 설정 필드
5. src/core/controller/index.ts - CaretiGlobalManager 연동

완전 독립 구현: careti-src/ 전체
```
...
```typescript
// 테스트: 하이브리드 시스템 동작 검증
describe('Careti Hybrid Mode System', () => {
...
3. **기본값 통일**: 모든 컴포넌트에서 CARETI 기본값 사용
...
3. **중복 상태 관리**:
   - 3중 모드 시스템(Cline plan/act, CaretiGlobalManager, CaretModeManager) 정리
   - 단일 진실 원천(Single Source of Truth) 확립

**정리 작업**:
```typescript
// 기존: 복잡한 3중 시스템
CaretModeManager.chatbot → CaretiGlobalManager.careti → Cline.plan

// 개선: 단순한 해석 변환
CaretiGlobalManager.careti + mode → CaretiPromptWrapper → JSON 프롬프트
```
...
grep -r "완전.*독립\|plan.*우회\|act.*우회" careti-src/
grep -r "CaretIndependentModeSystem\|ComplexMappingSystem" careti-src/
grep -r "3중.*모드.*시스템\|triple.*mode" careti-docs/

echo "=== 올바른 대체품 패턴 확인 ==="  
grep -r "대체품.*방식\|어댑터.*패턴" careti-src/
grep -r "plan/act.*재활용\|인프라.*활용" careti-docs/
```
...
describe('Architecture Cleanup Validation', () => {
  test('중복 모드 관리 시스템 완전 제거', async () => {
    ...
    expect(modeManagers).toEqual([
      'CaretiGlobalManager',  // 시스템 선택 (careti/cline)
  ...
  test('상태 관리 단일 진실 원천 확보', async () => {
    ...
    expect(stateManagers.secondary).toEqual([
      'localStorage', // 동기화용만
      'CaretiGlobalManager.currentMode' // 캐시용만  
    ])
  })
})

### **Phase 4: TDD 및 표준 로깅 시스템 적용**
**목표**: 완전한 TDD 적용 및 Careti 표준 로깅으로 모드 관련 로그 통일
...
describe('Careti Mode System E2E', () => {
