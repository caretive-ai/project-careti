...
앞의 careti-docs\work-logs\luke\t06-phase5-cross-validation.md 와 careti-docs\features\f02-multilingual-i18n.md 의 미 구현 부분이 많습니다. 해당 개요와 소스를 반드시 파악하고 문제에 접근하세요.
...
UI (ModeSystemToggle) 
  ...
  ↓ CaretiPromptWrapper (context.mode)
  ↓ AI Prompt (실제 모드 인식)
```
...
```typescript
// 로그 출력
DEBUG [CaretiPromptWrapper] Mode debug info: {
  "caretMode":"chatbot",
  "mappedPlanAct":"plan",
  "initialized":true
}
...
- CaretiPromptWrapper는 올바른 모드를 받았으나 AI는 이전 컨텍스트 혼동
...
**근본 원인**:
- Careti 전용 메시지 키가 다국어 파일에 추가되지 않음
...
**Problem**: Cline과 Careti 모드 시스템 충돌

**현재 구조**:
```typescript
// 혼재된 모드 관리
if (modeSystem === "careti") {
  // Careti CHATBOT/AGENT 로직
} else {
  // Cline PLAN/ACT 로직  
}
```
...
Luke 로그: `[CaretiJsonAdapter] ❌ No tools available from Cline system`

**근본 원인**: CaretiJsonAdapter.ts:118에서 사용하는 mockVariant 구조가 cline-latest의 새로운 요구사항과 불일치

**기존 코드 (문제 있음)**:
```typescript
// CaretiJsonAdapter.ts:103-117 - 몇 주 전 cline 버전용
const mockVariant = {
    id: 'careti-tools',
    version: 1,
...
**1단계: mockVariant 수정**
```typescript
// CaretiJsonAdapter.ts:103-117 수정
const mockVariant = {
    id: 'careti-tools',
    version: 1,
    tags: [] as readonly string[],
...
**2단계: 디버그 로깅 강화**
```typescript
// CaretiJsonAdapter.ts:100에 디버깅 코드 추가
...
**문제**: CaretModeManager와 CaretiJsonAdapter에서 동일한 도구 제한 로직 중복

**해결책**: CaretiJsonAdapter에서만 필터링하고 CaretModeManager는 검증용으로만 사용

```typescript
// CaretiJsonAdapter.ts:144-155 수정
private async getClineToolsSection(context: CaretSystemPromptContext, isChatbotMode: boolean): Promise<string | null> {
...
- CaretModeManager, CaretiPromptWrapper: 완전 독립 구현 ✅  
- 문제는 아키텍처가 아닌 cline-latest 호환성

#### B. Careti 완전 독립 시스템 구축
...
export class CaretModeManager {
    ...
    static setCaretMode(mode: "chatbot" | "agent") {
        this.caretMode = mode
        // Careti 전용 워크스페이스 키 사용 (Cline과 완전 분리)
...
**구현 예시**:
```typescript
// CaretiPromptWrapper.ts에서 Cline 원본 도구 시스템 사용
...
{
  "chat": {
    "caretWantsToUseBrowser": "Careti wants to use the browser",
...
export function getBrandedMessage(key: string, brand: "careti" | "cline"): string {
    if (brand === "careti") {
        return key.replace("Cline", "Careti")
    }
    return key
}
```
...
```bash
# 파일: careti-src/core/prompts/system/adapters/CaretiJsonAdapter.ts
```
...
```bash  
# 파일: careti-src/core/modes/CaretModeManager.ts
```
- [ ] **Line 93-110**: isToolAllowed 메서드를 검증 전용으로 변경
- [ ] CaretiJsonAdapter에서만 실제 필터링 수행하도록 조정

#### **3단계: JSON 모드 프롬프트 보강 (20분)**
```bash
# 파일: careti-src/core/prompts/sections/CHATBOT_AGENT_MODES.json
```  
...
# 추가할 디버깅 포인트들
```
- [ ] **CaretiPromptWrapper.ts**: 모드 전달 과정 추적
- [ ] **CaretiJsonAdapter.ts**: 도구 로딩 과정 상세 로깅  
...
#### **6단계: UI 연결 확인 (15분)**
```bash
# 파일: webview-ui/src/careti/components/ModeSystemToggle.tsx
```
...
```bash
# 파일: webview-ui/src/careti/locale/*/
```
...
t("chat.caretWantsToUseBrowser", "Careti wants to use the browser:")  // ❌ chat 네임스페이스에 browser 키
...
#### **Phase 1 완료 기준**:
✅ Luke 로그에서 `[CaretiJsonAdapter] ❌ No tools available from Cline system` 메시지 사라짐  
...
3. **완전한 분리**: Careti과 Cline 시스템 독립성 확보
4. **점진적 개선**: 안정성 우선, 기능 추가는 차후 진행

### 6.2 장기적 비전

**t06 프로젝트의 궁극적 목표 달성**:
- "구조는 cline-latest, 영혼은 Careti" 철학 구현
- JSON 기반 유연한 프롬프트 관리 (단순 컴포넌트)
- Cline 호환성 유지와 Careti 독창성 양립
- 개발자 친화적 아키텍처 완성
