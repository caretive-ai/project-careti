...
### 전체 맥락
- **현재 위치**: `careti-merge` 디렉토리에서 작업 중
- **프로젝트 목표**: cline-latest + careti-main 머징 및 통합
- **핵심 과제**: cline-latest의 시스템 프롬프트 구조 변경(13개 컴포넌트)에 맞춰 Careti JSON 시스템 재구축

### 아키텍처 변경 사항
- **기존**: careti-main에서 잘 작동하던 프롬프트 시스템
- **현재**: cline-latest 기반의 완전히 새로운 구조
- **도전**: 기존 Careti 철학(CHATBOT/AGENT)을 새로운 cline 구조에 통합
    ...
    class CaretJsonComponentProvider {
        ...
        private preloadAllJsonFiles(): void {
            ...
            jsonFiles.forEach(file => {
                try {
                    const content = JSON.parse(fs.readFileSync(path.join('careti-src/core/prompts/sections', file), 'utf8'));
...
- [ ] **최종 성공 기준 체크리스트 검증**:
    ...
    - [ ] `cline-latest`의 3대 신규 기능('작업 관리 루프')이 Careti 철학에 맞게 융합되었는가?
    - [ ] 프론트엔드 UI를 통한 실시간 모드 전환 및 설정 영속성이 보장되는가?
    - [ ] 기존 Careti 사용자 경험이 100% 보존되는가?
