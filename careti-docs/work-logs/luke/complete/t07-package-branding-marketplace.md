...
grep -r "careti\." package.json | wc -l    # 결과: 42개+ (변경 완료)
...
# - Careti 확장과 Cline 확장 동시 설치 후 명령 팔레트에서 충돌 없는지 확인
```

### **검증 기준**:
- ✅ 컴파일 오류 없음
- ✅ package.json에서 `cline.*` 명령어 0개 확인  
- ✅ package.json에서 `careti.*` 명령어 42개+ 확인
- ✅ 백엔드 컨텍스트 키 `careti.*` 변경 확인
...
#### **목적**: 
Cline extension과의 동시 설치를 위해 기본 명령어를 `cline.*`에서 `careti.*`로 변경
...
예시:
- cline.plusButtonClicked → careti.plusButtonClicked
- cline.addToChat → careti.addToChat
- cline.isDevMode → careti.isDevMode
- cline.isGeneratingCommit → careti.isGeneratingCommit
```
...
#### **확장 대상**:
- [ ] `careti-b2b/tools/brand-converter.js` - 메인 브랜드 전환 스크립트
- [ ] `careti-b2b/brands/*/brand-config.json` - 각 브랜드별 설정에 명령어 매핑 추가
...
{
  "command_mappings": {
    "careti.plusButtonClicked": "codecenter.plusButtonClicked",
    "careti.addToChat": "codecenter.addToChat",
    "careti.isDevMode": "codecenter.isDevMode"
    // ... 모든 명령어 매핑
  }
}
```
...
```json
{
  ...
  "bugs": {
    "url": "https://github.com/aicoding-careti/careti/issues",
  ...
  "badges": [
    {
      "url": "https://img.shields.io/github/stars/aicoding-careti/careti",
      "href": "https://github.com/aicoding-careti/careti",
...
#### **현재 구현된 기능들**:
- ✅ `careti-b2b/tools/brand-converter.js` - 완전 구현된 브랜드 전환 스크립트
- ✅ `careti ↔ cline` 양방향 브랜드 전환 지원
- ✅ `careti ↔ codecenter` 양방향 브랜드 전환 지원  
...
```bash
# 각 브랜드 전환 후 확인사항
node careti-b2b/tools/brand-converter.js careti forward    # → cline 브랜드
node careti-b2b/tools/brand-converter.js careti backward   # → careti 브랜드  
node careti-b2b/tools/brand-converter.js codecenter forward   # → codecenter 브랜드
node careti-b2b/tools/brand-converter.js codecenter backward  # → careti 브랜드
```
...
1. package.json commands 섹션 변경 (`cline.*` → `careti.*`)
...
### **✅ 기존 브랜딩 시스템 현황**:
- `careti-b2b/tools/brand-converter.js` - **완전 구현됨** (40,257 라인)
- `cline ↔ careti ↔ codecenter` 3-way 브랜드 전환 지원
...
### **🎯 실제 작업 범위 (수정됨)**:
1. **Phase 1**: 기본 명령어를 `cline.*` → `careti.*`로 변경 (Cline 동시설치용)
