...
| 구분            | Cline Plan/Act               | Careti Chatbot/Agent              |
| --------------- | ---------------------------- | -------------------------------- |
...
- **Factory 패턴**: Careti/Cline 시스템 완전 분리
...
- [ ] 어댑터 패턴 테스트 이식
  ```bash
  cp -r careti-main/careti-src/core/mode-system/__tests__ \
        careti-src/core/mode-system/__tests__
  ```
- [ ] 테스트 실행 확인
  ```bash
...
- [ ] ModeSystemRegistry 이식
  ```bash
  cp careti-main/careti-src/core/mode-system/ModeSystemRegistry.ts \
     careti-src/core/mode-system/
  ```
- [ ] 상수 시스템 이식
  ```bash
  cp careti-main/careti-src/shared/constants/ModeSystemConstants.ts \
     careti-src/shared/constants/
  ```

### Phase 3: JSON 프롬프트 시스템 이식 (f07 통합)
- [ ] JSON 섹션 어셈블러 이식
  ```bash
  cp -r careti-main/careti-src/core/prompts/ \
...
- [ ] 도구 핸들러 이식
  ```bash
  cp careti-main/careti-src/core/tools/CaretToolHandler.ts \
     careti-src/core/tools/
  
  cp careti-main/careti-src/core/tools/CaretToolSelector.ts \
     careti-src/core/tools/
  ```
...
- [ ] 메시지 핸들러 팩토리 이식
  ```bash
  cp -r careti-main/careti-src/core/messaging/ \
        careti-src/core/messaging/
  ```

### Phase 6: 백엔드 통합 (최소 수정)
- [ ] build-system-prompt.ts 분기 로직 추가 (4라인)
  ```typescript
  // CARETI MODIFICATION: modeSystem 기반 프롬프트 분기
...
- [ ] 모드 시스템 설정 UI 이식
  ```bash
  cp careti-main/webview-ui/src/careti/components/CaretModeSystemSetting.tsx \
...
- [ ] 변환 로직 이식
  ```bash
  cp careti-main/careti-src/shared/proto-conversions/state/careti-settings-conversion.ts \
     careti-src/shared/proto-conversions/state/
  ```
...
class CaretModeAdapter implements ModeSystemAdapter {
    ...
    async buildSystemPrompt(mode: string, context: any): Promise<string> {
        // JSON 기반 Careti 전용 프롬프트 생성 (f07 통합)
...
```json
// careti-src/core/prompts/sections/TOOL_DEFINITIONS.json
{
    "execute_command": {
...
```typescript
// careti-src/shared/constants/ModeSystemConstants.ts
...
export class MessageHandlerFactory {
    static create(modeSystem: "careti" | "cline"): MessageHandlerInterface {
        return modeSystem === "careti"
            ? new CaretMessageHandler() // 순수 Careti 로직만
            : new ClineMessageHandler() // 순수 Cline 로직만
    }
}

// Careti 전용 핸들러 (Optimistic Update 포함)
...
│   │   │   ├── CaretMessageHandler.ts            # Careti 전용 로직
│   │   │   ├── ButtonConfigFactory.ts            # 버튼 설정 팩토리
│   │   │   ├── CaretButtonConfigHandler.ts       # Careti 버튼 로직
│   │   │   └── interfaces/
...
└── webview-ui/src/ (프론트엔드 - Careti 확장)
    ├── components/chat/
...
3. **시스템 모드 선택**: 설정에서 Careti 모드 ↔ Cline 모드 전환 가능
...
- `build-system-prompt.ts`: Careti 시스템 프롬프트 분기
...
- `careti-settings-conversion.ts`: 모드 시스템 변환 로직
...
- **혼합 사용**: 언제든지 Careti ↔ Cline 모드 전환 가능
