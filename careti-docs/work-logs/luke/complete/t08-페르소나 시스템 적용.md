...
- careti-compare 프로젝트에 검증된 구현체가 존재
...
#### 2. gRPC 서비스 오류 ❌
- **문제**: `careti.PersonaService` 서비스를 찾을 수 없음
...
- **해결방법**: careti-compare의 검증된 Base64 injection 방식 적용 필요
...
#### 기존 구조 (careti-merging)
- **CaretiGlobalManager**: Auth0 통합된 싱글톤 패턴 (`careti-src/managers/CaretiGlobalManager.ts`)
...
#### 검증된 구조 (careti-compare)  
- **CaretProviderWrapper**: webview 확장 패턴 (`careti-compare/careti-src/core/webview/CaretProviderWrapper.ts`)
...
- **careti-compare**: CSP 호환, Base64 데이터 URI 직접 사용
...
setEnablePersonaSystem: (enabled: boolean) => {
    ...
    localStorage.setItem("careti-enablePersonaSystem", JSON.stringify(enabled))
...
#### **구성 요소 상태 확인**:
- ✅ **Proto 정의**: `proto/careti/persona.proto` 존재
- ✅ **핸들러**: `src/core/controller/persona/` 모든 핸들러 구현됨
- ✅ **타입 생성**: `src/shared/proto/careti/persona.ts` 정상 생성
- ✅ **ProtoBus 설정**: `scripts/generate-protobus-setup.mjs` careti 서비스 처리
...
```bash
# 1. 기존 페르소나 컴포넌트 제거
rm -rf webview-ui/src/careti/components/PersonaManagement.tsx
rm -rf webview-ui/src/careti/components/PersonaTemplateSelector.tsx  
rm -rf webview-ui/src/careti/components/PersonaAvatar.tsx

# 2. 기존 페르소나 상태 관리 제거
# webview-ui/src/context/ExtensionStateContext.tsx에서 persona 관련 상태 제거

# 3. 기존 gRPC 클라이언트 제거
rm -rf webview-ui/src/careti/services/CaretGrpcClient.ts
rm -rf webview-ui/src/careti/context/CaretStateContext.tsx
```
...
#### **2.1 백엔드 이미지 주입 시스템**
```typescript
// careti-src/core/webview/CaretProviderWrapper.ts (이미 완료)
...
#### **2.2 프론트엔드 이미지 로딩**
```typescript
// webview-ui/src/careti/utils/convertAssetToBase64.ts (신규 생성)
...
setEnablePersonaSystem: (enabled: boolean) => {
    // 1. CaretiGlobalManager 업데이트 (t01 미션과 연계)
    CaretiGlobalManager.get().setPersonaSystemEnabled(enabled)
...
#### **2.4 새로운 페르소나 컴포넌트 (careti-compare 기반)**
```typescript
// webview-ui/src/careti/components/PersonaAvatar.tsx (재구축)
...
// CARETI MODIFICATION: PersonaAvatar 통합 (조건부 렌더링)
{enablePersonaSystem && modeSystem === "careti" && (
    <PersonaAvatar 
...
- CaretiGlobalManager 연동 (t01 미션과 통합)

### **3순위**: UI 재구축 (careti-compare 패턴)
- 기존 컴포넌트 삭제 후 재작성
- 단순한 상태 관리 적용
...
webview-ui/src/careti/components/PersonaManagement.tsx
webview-ui/src/careti/components/PersonaTemplateSelector.tsx  
...
webview-ui/src/careti/utils/persona-utils.ts (있다면)
```
...
- Prepare for clean careti-compare persona system port

🤖 Generated with [Claude Code](https://claude.ai/code)"
git push
```

### **Phase B: careti-compare 이식 계획 (📦 Port)**

#### **B.1 careti-compare 분석 대상**
```bash
# 분석해야 할 careti-compare 디렉토리
careti-compare/careti-src/core/webview/CaretProviderWrapper.ts
careti-compare/webview-ui/src/careti/components/PersonaAvatar.tsx
careti-compare/webview-ui/src/careti/components/PersonaTemplateSelector.tsx
careti-compare/webview-ui/src/careti/components/PersonaManagement.tsx
careti-compare/webview-ui/src/careti/services/
careti-compare/webview-ui/src/careti/context/
```

#### **B.2 경로 변경 규칙**
```bash
# careti-compare → careti-merging 경로 변경
...
webview-ui/src/careti/services/
...
- [ ] modeSystem="careti"일 때만 페르소나 기능 활성화
...
- **담당자**: Claude Code
- **전략**: Clean Slate → careti-compare 완전 이식 → 조건부 렌더링
- **우선순위**: 즉시 실행 (사용자 지시사항)