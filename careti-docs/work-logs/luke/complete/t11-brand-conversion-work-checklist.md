...
**🔧 핵심 구현:**
- **페르소나 숨김**: `brandConfig.showPersonaSettings && modeSystem === "careti"`
- **페르소나 기본값**: 브랜드별 `defaultPersonaEnabled` 설정
- **리다이렉트 분기**: API 설정 후 persona vs home 선택
- **동적 설정**: `.careti-feature-config.json` 파일 기반
...
- [x] **CaretBrandConfig.ts 설계 및 구현**
  ...
  - [x] 브랜드별 설정 분리 (Careti vs CodeCenter)

- [x] **동적 브랜드 설정 시스템**
  - [x] .careti-feature-config.json 파일 기반 설정
