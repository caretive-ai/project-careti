...
#### 2. Careti Provider 문제 해결 (완료)
- ✅ 부적절한 개인정보처리방침 콘텐츠 제거 (CaretiProvider.tsx lines 66-81)
- ✅ 번역 키 누락 문제 해결 (providers.careti.login 등)
- ✅ 4개 언어 일관성 확보
...
## 🚨 강화된 작업 방법론 (Careti Provider 실수 반영)

### Phase 0: 사전 준비 (실패 방지)
```bash
# 백업 생성 (필수)
node careti-scripts/provider-i18n-consolidator.js --backup

# 검증 스크립트 실행하여 현재 상태 파악
node careti-scripts/provider-i18n-validator.js > validation-report.txt
```

### Phase 1: 완전 분석 및 계획 (누락 방지)
```bash
# 1. 현재 구조 파악
grep -r "Provider.*:" webview-ui/src/careti/locale/en/settings.json
grep -r "ModelPicker.*:" webview-ui/src/careti/locale/en/settings.json

# 2. ⚠️ 중요: 실제 사용 키 완전 검색 (Careti 실수 방지)
...
#### 2.3 영어 파일 검증 (🚨 필수 - Careti 실수 방지!)
```bash
# 영어 파일에 모든 필수 키가 있는지 검증
echo "=== 영어 파일 키 검증 ===" > en-validation.txt
while read key; do
  keypath=$(echo $key | sed 's/t("//' | sed 's/".*//')
  if ! grep -q "\"$keypath\"" webview-ui/src/careti/locale/en/settings.json; then
...
for lang in ko ja zh; do
  ...
  while read key; do
    keypath=$(echo $key | sed 's/t("//' | sed 's/".*//')
    if ! grep -q "\"$keypath\"" webview-ui/src/careti/locale/$lang/settings.json; then
...
# 제거 후 검증
grep -r "ollamaProvider\|ollamaModelPicker" webview-ui/src/careti/locale/
# 결과가 없어야 함!
```
...
### ⚠️ 중요 경고사항
1. **절대 단계를 건너뛰지 말 것** - Careti 실수 재발 방지
...
1. **문서 업데이트**: f02-multilingual-i18n.md에 새로운 표준 반영
