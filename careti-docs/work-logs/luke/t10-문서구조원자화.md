...
### 기존 문서 구조 문제점
```
careti-docs/development/: 22개 문서 (개발자만 접근)
careti-docs/guides/: 6개 가이드 (개발자만 접근)
.agents/context/workflows/: 4개 워크플로우 (AI만 접근)
...
### 구조 원칙
1. **1:1 매핑**: .agents/context ↔ careti-docs 완전 대응
2. **분류 원칙**: 
   - 규칙 (JSON) = 불변 핵심 원칙
   - 워크플로우 (MD) = 동적 작업 절차
...
"atomic_workflows": {
  ...
  "/comment-protocol": "CARETI MODIFICATION 마킹"
}
```
...
- ✅ 3단계 검증 방법론
- ✅ 토큰 효율성 실험 및 검증
- ✅ **의미론적 동등성 검증 도구 통합** (careti-main에서 발견 및 적용)
- ✅ **유틸리티 스크립트 시스템 완성** (핵심 도구들)
- ✅ **개발 가이드 업데이트** (스크립트 존재 알림)

### 🛠️ 유틸리티 스크립트 시스템 완성
**careti-main에서 발견된 강력한 도구들을 scripts/로 이동 및 강화**:

#### 새로 추가된 핵심 도구들:
1. **`careti-scripts/semantic-equivalence-checker.js`**
   - Mission 1B 방법론 기반 의미론적 동등성 검증
   - 95.2% 목표 점수로 Markdown vs JSON 품질 보장
   - 자동화된 토큰 효율성, 기능 커버리지, 실행 동등성 분석

2. **`careti-scripts/token-efficiency-analyzer.js`**
   - 다중 형식 간 토큰 사용량 정밀 측정
   - 4가지 추정 방법론 (conservative, standard, aggressive, char-based)
   - 효율성 개선 정량화

3. **`careti-scripts/careti-cline-comparison.js`**
   - Careti vs Cline API 제공자 및 모델 커버리지 완전 비교
   - 누락/추가 기능 자동 식별 및 달성률 계산
   - 개발 우선순위 결정 지원

4. **`careti-scripts/universal-semantic-analyzer.js`**
...
🆚 Careti vs Cline → 📈 careti-cline-comparison.js → 📋 격차 분석
```
...
- **✅ 3c**: 문서 편집 가이드라인 추가 완료
  ...
  - 사람 개발자: .agents/context 직접 편집 피하기
...
- ✅ **CARETI 수정 주석**: 최소화 수정으로 자동 머징 지원
...
5. **JSON 변환**: 영문 문서 → 캐러티룰 JSON 규칙으로 옮김  
5. **JSON 변환**: 영문 문서 → 캐러티룰 JSON 규칙으로 옮김
...
### 1. **루트 레벨** (careti-docs/development/) - 🤖 AI 진입점
**역할**: AI가 처음 읽고 전체를 이해하는 문서
- **index.md**: AI 작업 시작점, 전체 구조 안내
...
#### 1. **frontend-backend-interaction-patterns.md**
- gRPC 서비스 구현 방법
- proto/careti 디렉토리 구조
- 단일 필드 업데이트 원칙
...
### Step 1: 누락 내용 통합
1. **frontend-backend-interaction-patterns.md** 내용 → `message-flow.md`에 추가
2. **component-architecture-principles.md** 내용 → `new-component.md`에 추가
3. **ai-message-flow-guide.md** 내용 → `ai-feature.md`에 추가

### Step 2: 구조 정리
1. 루트에 `index.md` 생성 - AI 진입점 문서
...
- `careti-docs/development-en/careti-rules.md` → `careti-docs/development/careti-rules.ko.md` 1:1 번역
- 13개 영문 워크플로우 → 한글 워크플로우 완전 번역
  - 주요 워크플로우: ai-work-index, ai-work-protocol, careti-development 등
...
- `.agents/context/careti-rules.json` 완전 재구성
...
2. **25개 한글 문서 삭제 불가**: 핵심 내용이 workflows에 통합되지 않음
   - frontend-backend-interaction-patterns.md → gRPC 서비스 패턴 누락
   - component-architecture-principles.md → 컴포넌트 원칙 누락
   - ai-message-flow-guide.md → 상세 플로우 부분만 통합

3. **영문 문서 중복 예정**: development-en은 .agents/context와 중복되어 삭제 예정

### 삭제 대상 파일 목록 (25개)

#### MDX 파일 (22개)
1. `ai-message-flow-guide.md` - AI 메시지 처리 플로우
2. `ai-work-index.md` - AI 작업 인덱스
3. `backend-i18n-system.md` - 백엔드 다국어 시스템
4. `careti-architecture-and-implementation-guide.md` - Careti 아키텍처 가이드
5. `checkpoint-architecture.md` - 체크포인트 아키텍처
6. `component-architecture-principles.md` - 컴포넌트 아키텍처 원칙
7. `documentation-guide.md` - 문서화 가이드
8. `file-storage-and-image-loading-guide.md` - 파일 저장 및 이미지 로딩
9. `frontend-backend-interaction-patterns.md` - 프론트엔드-백엔드 상호작용
10. `index.md` - 개발 문서 인덱스
11. `json-comment-conventions.md` - JSON 주석 규칙
12. `link-management-guide.md` - 링크 관리 가이드
13. `locale.md` - 로케일 시스템
14. `logging.md` - 로깅 시스템
15. `message-processing-architecture.md` - 메시지 처리 아키텍처
16. `new-developer-guide.md` - 신규 개발자 가이드
17. `support-model-list.md` - 지원 모델 리스트
18. `system-prompt-implementation.md` - 시스템 프롬프트 구현
19. `testing-guide.md` - 테스팅 가이드
20. `ui-to-storage-flow.md` - UI-스토리지 플로우
21. `utilities.md` - 유틸리티 함수
22. `webview-extension-communication.md` - 웹뷰-익스텐션 통신

#### MD 파일 (3개)
23. `button-system-architecture-guide.md` - 버튼 시스템 아키텍처
24. `korean-docs-cleanup.md` - 한글 문서 정리 가이드
25. `careti-rules.ko.md` - Careti 규칙 한글판 (유지 필요할 수도 있음)
...
- **development/**: AI 진입점, 인덱스, 가이드 (index.md 생성 필요)
...
#### 3. 중복 제거
- development-en/ 폴더 전체 삭제 (.agents/context와 중복)
