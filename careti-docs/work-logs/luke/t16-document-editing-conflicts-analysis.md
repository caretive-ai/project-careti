...
Careti에서 발생하는 문서 편집 문제는 크게 세 가지로 분류됩니다:
...
```typescript
// careti-src/core/editing/IndependentFileEditor.ts
...
```typescript
// CARETI MODIFICATION: 토큰 효율적인 에러 응답
return formatResponse.toolError(
    `${(error as Error)?.message}\n\n` +
    `File: ${relPath}\n` +
...
```typescript
// CARETI MODIFICATION: 편집 전 실시간 충돌 체크
...
```typescript
// careti-src/core/editing/SmartEditEngine.ts
...
```typescript
// careti-src/core/conflict/MergeResolver.ts
...
CLI 도구들이 성공하는 핵심은 **"작고 정확한 편집"**과 **"명시적 충돌 해결"**입니다. Careti도 이 원칙을 따라:
