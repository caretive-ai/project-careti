# Spec-Kit과 Careti 연계 분석 보고서: AI 문서관리 시스템 통합 방안

**작성일**: 2024-09-17
**작성자**: Claude Code AI Assistant
**목적**: Spec-Kit과 Careti의 AI 문서관리 시스템 분석 및 PRD 자동 생성 기능 설계
...
- **철학**: Cline 코어 보존 + careti-src/ 확장 구조
- **방향성**: 레거시 프로젝트 → .agents/context 문서 생성 (Brownfield)
- **강점**: 이중 언어 시스템, Atomic Workflow, 완전한 TDD 프레임워크

**핵심 구성요소**:
- `.agents/context/` 시스템: JSON 규칙 + MD 워크플로우
- 이중 언어 문서: 한국어/영어 병행 관리
- Atomic workflows: 재사용 가능한 작은 단위 워크플로우
- CARETI MODIFICATION 프로토콜: 원본 코드 보호 시스템
...
### 2.2 Careti의 AI 문서관리 시스템

#### 구조적 특징
```
.agents/context/
├── careti-rules.json               # AI용 구조화된 규칙
├── workflows/                     # 영문 워크플로우
│   ├── ai-work-index.md
│   ├── ai-work-protocol.md
│   └── atoms/                     # 원자 단위 워크플로우
...
1. **방향성 상보성**: Spec-Kit(Forward) + Careti(Reverse) = 완전한 소프트웨어 라이프사이클
...
└── prd-generator/                 # PRD 자동 생성 모듈
    ...
    └── rule-adapter/              # .agents/context 어댑터
```

#### Option 2: Hybrid System
- Spec-Kit 명령어를 Careti의 .agents/context 시스템과 통합
- Careti의 TDD 원칙을 Spec-Kit의 Constitutional Gates에 추가
...
/careti-specify [description]      # 요구사항 → Careti-style 스펙
/careti-plan [tech-stack]          # 스펙 → .agents/context 호환 계획
/careti-tasks                      # 계획 → TDD 태스크

# PRD 역생성 명령어 (레거시 프로젝트용)
/careti-analyze [directory]        # 레거시 코드 분석
/careti-extract-prd               # 코드 → PRD 역생성
/careti-generate-rules            # PRD → .agents/context 생성
```
...
#### 프로세스 플로우
```
레거시 코드베이스 → 코드 분석 → 의도 추출 → 스펙 생성 → .agents/context 적용
```
...
#### Phase 1: 코드 구조 분석
```typescript
// careti-src/prd-generator/legacy-analyzer/
...
```bash
# 1. 프로젝트 분석 시작
$ /careti-analyze ./my-legacy-project
...
# 5. .agents/context 생성
$ /careti-generate-rules
# → .agents/context/legacy-project-rules.json 생성
# → careti-docs/legacy-analysis/ 문서 생성
```
...
3. **migration-plan.md**: .agents/context 적용 계획
...
- [ ] Spec-Kit 템플릿을 Careti 구조로 어댑터 개발
- [ ] `/careti-specify` 명령어 기본 구현
...
#### 1. 파일럿 프로젝트 시작
```bash
# 현재 Careti 프로젝트에 Spec-Kit 명령어 통합
careti-src/spec-integration/ 디렉토리 생성
```
...
*이 보고서는 Spec-Kit과 Careti의 통합 가능성을 분석하고, 실행 가능한 구현 방안을 제시합니다. 두 프로젝트의 상보적 특성을 활용하여 완전한 AI-Driven Development 생태계를 구축할 수 있을 것으로 기대됩니다.*