...
본 실험의 주요 목적은 **Careti**과 **Cline** AI 코딩 어시스턴트의 성능을 정량적으로 비교 분석하는 것입니다. Careti은 Cline을 기반으로 개선되었으며, Cline 모드를 지원하므로 두 에이전트 간의 직접적인 성능 비교가 가능합니다.

또한, **Cursor**와의 비교를 통해 외부 AI 에이전트와의 성능 차이도 함께 분석하여 Careti의 상대적 성능 수준을 파악하고자 합니다.
...
*   **Careti vs. Cline (주요 비교)**
    *   **AI 에이전트**: `careti`, `cline`
    *   **사용 모델**:
    ...
    *   **아래는 세션의 명령 예시**
```
@/careti-docs\reports\experiment\json_caret_performance_test_20250713\calcualtor-comparison-instructions.md  에 따라 실험 진행해줘.
AI에이전트명은 careti-gemini-2.5-pro-preview-06-05-think1024
```

```
@/careti-docs\reports\experiment\json_caret_performance_test_20250713\json-processor-comparison-instructions.md  에 따라 실험 진행해줘.
AI에이전트명은 careti-gemini-2.5-flash-preview-05-20-think1024
```

*   **Careti vs. Cursor (부가 비교)**
    *   **AI 에이전트**: `careti`, `cursor`
    *   **사용 모델**: `claude-3.7-sonet`
    *   **비교 조합**: 4개 과제 x 1개 모델 x 2개 AI 에이전트 = **총 8회 실험**
    *   **특이사항**: 공정한 비교를 위해, 이 실험에서는 Careti 에이전트도 외부 API 설정을 통해 `claude-3.7-sonet` 모델을 사용하도록 구성합니다. 이를 통해 동일한 모델 조건 하에서 각 에이전트의 순수 성능(처리 속도, API 호출 효율성 등)을 측정합니다.
...
3.  모든 단계가 완료되면, 5단계에 명시된 **성능 보고서 생성 스크립트**를 실행합니다.
    ```bash
    # 예시: calculator 과제를 careti 에이전트로 수행한 경우
    node ./generate-report.js calculator careti
    ```
4.  생성된 성능 보고서(`{AI에이전트명}-{yyyymmdd}-report.md`)를 해당 과제 폴더에 저장합니다.

### 3.2. 에이전트별 수행 환경

*   **Careti/Cline**: Careti VSCode 익스텐션 내에서 `Careti` 또는 `Cline` 모드를 선택하여 실험을 진행합니다.
    *   **Careti vs. Cursor 비교 시**: Careti의 모델 설정을 `claude-3.7-sonet`으로 변경하여 외부 API를 통해 실험을 진행합니다.
...
본 실험을 통해 각 AI 에이전트와 모델 조합의 성능 특성을 명확히 파악하고, Careti의 개선 방향과 성능 최적화를 위한 구체적인 데이터를 확보할 수 있을 것으로 기대합니다.

**예상 결과:**
- Careti과 Cline의 정량적 성능 차이 분석
- 각 모델(Pro/Flash)의 비용-성능 트레이드오프 파악
- 과업 유형별 최적 에이전트-모델 조합 도출
- 향후 Careti 개발 방향성에 대한 데이터 기반 인사이트
