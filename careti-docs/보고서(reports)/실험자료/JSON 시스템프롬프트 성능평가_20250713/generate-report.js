	...
	try {
		...
		lines.forEach((line) => {
			...
			if (line.includes("[PROMPT-USAGE]") && !systemPromptInfo) {
				...
				if (lengthMatch && tokenMatch) {
					...
					// 모드 판별
					const isCaretMode = line.includes("CARETI 모드")
					...
					systemPromptInfo = {
						...
						approxTokens: tokens,
						mode: isCaretMode ? "careti" : isClineMode ? "cline" : "unknown",
		...
		lines.forEach((line) => {
			...
			if (line.includes("[API_RESPONSE]")) {
				...
				// CARETI MODIFICATION: 실제 캐시 필드명으로 수정
		...
		if (systemPromptInfo) {
			...
			if (caretModeMatch) actualMode = "careti"
...
function parseLogFiles(taskDir) {
	...
	try {
		...
		let totalCost = 0
		let apiCallCount = 0
		// CARETI MODIFICATION: 시스템 프롬프트 검증을 위한 변수 추가
		let systemPromptInfo = null
		const modeCheckLogs = []
		...
		uiMessages.forEach((msg) => {
			if (msg.type === "say" && msg.say === "api_req_started") {
				try {
					...
					// CARETI MODIFICATION: 실제 캐시 필드명 수정 (cacheWrites + cacheReads)
					...
					// CARETI MODIFICATION: 향상된 시스템 프롬프트 정보 추출
					if (!systemPromptInfo) {
						...
						if (apiData.systemPromptInfo) {
							systemPromptInfo = {
								...
								mode: apiData.systemPromptInfo.mode || "unknown", // CARETI MODIFICATION: 모드 정보 추가
							}
						}
						// 기존 방식으로 폴백 (우선순위 2)
							...
							if (systemMessage && systemMessage.content) {
								...
								systemPromptInfo = {
									...
									mode: "unknown", // CARETI MODIFICATION: 모드 정보 기본값
								}
							}
						}
						// 레거시 구조로 폴백 (우선순위 3)
			...
			// CARETI MODIFICATION: 모드 체크 로그 추출
		...
		return {
			...
			endTime,
			totalLatency,
			// CARETI MODIFICATION: 시스템 프롬프트 정보 추가
			systemPromptInfo,
			modeCheckLogs,
			actualMode, // 실제 세션 모드 추가
			actualSessionType, // 실제 세션 타입 추가
...
function createMarkdownReport(experimentName, agentName, data) {
	...
	// CARETI MODIFICATION: 실제 데이터 기반 시스템 프롬프트 정보 분석
...
function main() {
	...
	if (args.length !== 2) {
		...
		console.error("예시: node generate-report.js calculator careti-gemini2.5-pro-preview-06-05-tink1024-1")
		process.exit(1)
	}
