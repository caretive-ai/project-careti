...
// CARETI MODIFICATION: Use downloaded protoc when available (Windows .exe) and fall back on grpc-tools or system protoc.
...
async function main() {
	...
	await postProcessGeneratedFiles() // CARETI MODIFICATION: Add post-processing step AFTER all generation
}
...
// CARETI MODIFICATION: Post-process generated files to fix common issues
async function postProcessGeneratedFiles() {
	...
	for (const file of generatedFiles) {
		try {
			...
			// CARETI MODIFICATION: Fix import paths for fetchLiteLlmModels
			...
			// CARETI MODIFICATION: Fix import paths for Persona controller
			...
			if (caretMatches.length > 0) {
				...
				content = content.replace(/cline\.CaretAccountServiceService/g, "careti.CaretAccountServiceService")
				content = content.replace(/cline\.CaretSystemServiceService/g, "careti.CaretSystemServiceService")

				// Fix string references in service definitions
				content = content.replace(/"cline\.CaretAccountService"/g, '"careti.CaretAccountService"')
				content = content.replace(/"cline\.CaretSystemService"/g, '"careti.CaretSystemService"')

				// Fix all Caret type references
				content = content.replace(/cline\.CaretAuthState/g, "careti.CaretAuthState")
				content = content.replace(/cline\.CaretAuthStateChangedRequest/g, "careti.CaretAuthStateChangedRequest")
				content = content.replace(/cline\.CaretUserCreditsData/g, "careti.CaretUserCreditsData")
				content = content.replace(
					/cline\.GetCaretOrganizationCreditsRequest/g,
					"careti.GetCaretOrganizationCreditsRequest",
				)
				content = content.replace(/cline\.CaretOrganizationCreditsData/g, "careti.CaretOrganizationCreditsData")
				content = content.replace(/cline\.CaretUserOrganizationsResponse/g, "careti.CaretUserOrganizationsResponse")
				content = content.replace(
					/cline\.CaretUserOrganizationUpdateRequest/g,
					"careti.CaretUserOrganizationUpdateRequest",
				)

				// Fix CaretSystem service types
				content = content.replace(/cline\.SetPromptSystemModeRequest/g, "careti.SetPromptSystemModeRequest")
				content = content.replace(/cline\.SetPromptSystemModeResponse/g, "careti.SetPromptSystemModeResponse")
				content = content.replace(/cline\.GetPromptSystemModeRequest/g, "careti.GetPromptSystemModeRequest")
				content = content.replace(/cline\.GetPromptSystemModeResponse/g, "careti.GetPromptSystemModeResponse")
				content = content.replace(/cline\.SetCaretModeRequest/g, "careti.SetCaretModeRequest")
				content = content.replace(/cline\.SetCaretModeResponse/g, "careti.SetCaretModeResponse")
				content = content.replace(/cline\.GetCaretModeRequest/g, "careti.GetCaretModeRequest")
				content = content.replace(/cline\.GetCaretModeResponse/g, "careti.GetCaretModeResponse")
				// CARETI MODIFICATION: Fix LiteLLM service types
				content = content.replace(/cline\.FetchLiteLlmModelsRequest/g, "careti.FetchLiteLlmModelsRequest")
				content = content.replace(/cline\.FetchLiteLlmModelsResponse/g, "careti.FetchLiteLlmModelsResponse")
				// CARETI MODIFICATION: Fix BizRouter service types
				content = content.replace(/cline\.FetchBizRouterModelsRequest/g, "careti.FetchBizRouterModelsRequest")
				content = content.replace(/cline\.FetchBizRouterModelsResponse/g, "careti.FetchBizRouterModelsResponse")
