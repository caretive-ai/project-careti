...
import { CaretiJsonAdapter } from "./adapters/CaretiJsonAdapter"
...
// Define a context type that includes the system selector and is compatible with Careti's context.
type SystemManagerContext = { modeSystem: "careti" | "cline" } & Partial<Omit<CaretSystemPromptContext, "modeSystem">>
...
export class PromptSystemManager {
	...
	constructor() {
		...
		this.adapters.set("careti", new CaretiJsonAdapter())
	...
	public async getPrompt(context: SystemManagerContext): Promise<string> {
		...
		// CARETI MODIFICATION: Enhanced debug logging for adapter selection
