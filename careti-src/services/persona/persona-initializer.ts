...
export class PersonaInitializer {
	...
	private async checkPersonaImagesExist(): Promise<boolean> {
		try {
			...
			const profileWebpPath = path.join(personaDir, "agent_profile.webp")
			const thinkingWebpPath = path.join(personaDir, "agent_thinking.webp")
			const profilePngPath = path.join(personaDir, "agent_profile.png")
			const thinkingPngPath = path.join(personaDir, "agent_thinking.png")

			// 두 파일이 모두 존재하는지 확인 (webp 우선, png 레거시 허용)
			const profileExists = (await fileExistsAtPath(profileWebpPath)) || (await fileExistsAtPath(profilePngPath))
			const thinkingExists = (await fileExistsAtPath(thinkingWebpPath)) || (await fileExistsAtPath(thinkingPngPath))
			return profileExists && thinkingExists
		} catch (error) {
	...
	private async updatePersonaMd(persona: any): Promise<void> {
		try {
			...
			// CARETI MODIFICATION: persona.md 파일을 쓰기 전에 디렉토리가 존재하는지 확인하고 없으면 생성
	...
	private async updatePersonaImagesInGlobalStorage(persona: any): Promise<void> {
		try {
			...
			// 이미지 파일 복사
			const profileDst = path.join(personaDir, "agent_profile.webp")
			const thinkingDst = path.join(personaDir, "agent_thinking.webp")
