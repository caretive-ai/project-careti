...
- npm 글로벌 설치 시 토큰이 필요하지 않습니다. 퍼블리시는 `.env`의 `CARET_NPM_TOKEN`을 사용하며, `publish-careti-cli.sh`가 자동으로 로드합니다.
...
- `publish-careti-cli.sh`는 `cli-careti/.npmrc`를 생성해 토큰을 주입한 뒤 `npm pack`과 `npm publish`를 수행합니다.
