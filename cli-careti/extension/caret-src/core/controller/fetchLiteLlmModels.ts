...
export async function fetchLiteLlmModels(
	...
	try {
		...
		if (!healthCheckAvailable) {
			...
			Logger.warn("[CaretSystemService] ⚠️ Health check succeeded but no healthy models were reported")
			filteredModels = []
		} else {
...
// CARETI MODIFICATION: Export with PascalCase name for generated code compatibility
