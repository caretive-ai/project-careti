...
<Note>
캐럿 Rule Priority: `caret-docs/features/f05-rule-priority-system.md` 기준으로 `.caretrules > .clinerules > .cursorrules > .windsurfrules` 단일 선택 우선순위를 적용하여 중복 로딩을 막습니다. 캐럿에서는 우선순위가 높은 파일 하나만 사용되므로 룰 작성 시 이 점을 명시하세요.
</Note>
