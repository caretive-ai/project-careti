...
<Note>
캐러티(Careti) 기준 문서입니다. Cline v3.38.1 머지본을 따르며, 캐러티 전용 정책(AGENTS.md 사용 시 차이, 규칙 저장 위치/스코프, 인증/라우팅)이 있을 경우 본문에서 `<Note>`로 표시합니다.
</Note>

<Note>
캐러티 Agent Standardization: `careti-docs/features/f06-agent-standard-claude-compat.md` 기준으로 `.agents/context` SoT + `AGENTS.md` 계층 + `/init` 스캐폴드로 표준화합니다. 레거시 규칙 경로는 읽지 않습니다.
</Note>
...
Cline automatically processes **all Markdown files** inside the `.agents/context/` directory, combining them into a unified set of rules. The numeric prefixes (optional) help organize files in a logical sequence.
...
cp clinerules-bank/clients/client-b.md .agents/context/
```
...
cp clinerules-bank/frameworks/react.md .agents/context/
```
...
-   Consider git-ignoring the active `.agents/context/` folder while tracking the `clinerules-bank/`
...
To make managing both single `.agents/context` files and the folder system even easier, Cline v3.13 introduces a dedicated popover UI directly accessible from the chat interface.
...
-   **Quickly Toggle Rules:** Enable or disable specific rule files within your workspace `.agents/context/` folder with a single click. This is perfect for activating context-specific rules (like `react-rules.md` or `memory-bank.md`) only when needed.
