...
Caret's terminal integration lets you bring your terminal output directly into your conversations with Caret. Instead of copying and pasting error messages or command results, you can send them to Caret with a simple right-click in the terminal.
...
When you're working in the VSCode terminal and see output you want to discuss with Caret:

1. Right-click in the terminal
2. Select "Add to Caret" from the context menu
3. The terminal output is immediately sent to the Caret chat panel
...
Instead of switching context to the Caret chat panel and typing a description of the problem, you can send the terminal output directly to Caret with just a couple of clicks.

Alternatively, you can use the [`@terminal`](/features/at-mentions/terminal-mentions) mention to send the full terminal output to Caret.
...
When you use the right-click terminal integration, Caret:
...
This gives Caret the full context it needs to provide accurate help with terminal-related issues.

## Behind the Scenes

The terminal integration uses a clever technique to capture terminal output:

1. When you trigger the integration, Caret:
...
-   **Use terminal integration for error messages**: When you encounter an error in the terminal, sending it to Caret often results in faster resolution than trying to describe the error.
...
-   **Combine terminal outputs with file mentions**: After sending terminal output to Caret, you can enhance your question by mentioning relevant files using the @ mentions feature.
...
Next time you're staring at a cryptic error message in your terminal, try using Caret's terminal integration instead of copying and pasting. You'll get more accurate help because Caret can see the complete terminal context with proper formatting.
...
<Tip>
	**Quick Fix**: Most terminal issues can be resolved by switching to bash in the Caret settings and increasing the shell
