...
<Note>
캐럿(Caret) 기준 문서입니다. Cline v3.38.1 머지본을 따르며, 캐럿 전용 정책(허용 훅 스코프, 인증/라우팅, 로깅/텔레메트리 차이)이 있을 경우 본문에서 `<Note>`로 표시합니다.
</Note>

<Note>
Caret 계정/조직 훅: `caret-docs/features/f04-caret-account.md`에 따라 계정/조직 상태가 훅 데이터에 포함될 수 있습니다. 로깅/텔레메트리는 캐럿 유틸(`caret-src/utils/`) 경로를 사용합니다.
</Note>
