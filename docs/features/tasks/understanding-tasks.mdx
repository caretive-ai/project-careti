...
<Note>
캐러티(Careti) 기준 문서입니다. Cline v3.38.1 머지본을 따르며, 캐러티 전용 정책/제한(예: 작업 저장 위치, 체크포인트 정책, 인증/라우팅)이 있을 경우 본문에서 `<Note>`로 표시합니다.
</Note>

<Note>
입력 히스토리/계정: `careti-docs/features/f10-input-history-system.md`에 따라 캐러티 입력 히스토리/복원 기능이 동작할 수 있으며, 계정/조직 정보가 함께 관리될 수 있습니다.
</Note>
