...
<Note>
이 문서는 Cline v3.38.1 머지본을 기반으로 캐러티(Careti)에 맞춰 정리되었습니다. 캐러티 전용 정책/제한(모델 지원, 인증/라우팅)이 있는 경우 해당 섹션에 별도 `<Note>`로 표기합니다.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Customization & Control" icon="gear">
    <div style={{ display: 'grid', gap: '1rem', marginTop: '0.5rem' }}>
      <div>
        <strong><a href="/features/cline-rules">.agents/context</a></strong> • Define project-specific instructions including coding standards and patterns
      </div>
      <div>
