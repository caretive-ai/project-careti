...
- **소스:** `careti-docs/features/index.md` 및 하위 문서(F01~F11), `careti-features-specification.md`, `merging/*` 가이드들.
- **문서 반영:** 프로바이더/온보딩/기능 문서에 캐러티 전용 `<Note>` 또는 별도 섹션으로 병기.

### F01~F12 (careti-docs/features)
- **F01 공통 유틸**: `careti-docs/features/f01-common-util.md` — 캐러티 전용 헬퍼/유틸 존재. 개발 가이드에 언급 가능.
...
- **F10 Provider Setup 강화**: `f10-enhanced-provider-setup.md` — 프로바이더 설정 UX/검증 차이. 프로바이더 문서에 캐러티 추가 검증 언급.
- **F11 Input History**: `f11-input-history-system.md` — 입력 히스토리/복원. Tasks/UX 문서에 캐러티 입력 히스토리 존재 주석.
- **F12 지식 패리티**: `f12-ai-developer-knowledge-parity.md` — 캐러티 지식 동기화/정책. 필요 시 참고.

### 기타 소스
- `careti-features-specification.md`: Careti vs Cline 차별화 표 (Agent Standardization, Account/Org, i18n, Logging, Branding).
