...
<Note>
캐럿(Caret) 기준 문서입니다. Cline v3.38.1 머지본을 따르며, 캐럿 전용 정책(수집 항목/옵트인·옵트아웃, 인증/라우팅 로그, 저장 위치)이 있을 경우 본문에서 `<Note>`로 표시합니다.
</Note>

<Note>
캐럿 텔레메트리 차이(요약):
- **계정/조직 기반**: `webview-ui/src/caret/components/`의 계정/조직 설정에 맞춰 수집 범위/토글이 다를 수 있습니다.
- **로깅/헬퍼**: `caret-src/utils/`, `webview-ui/src/caret/utils/`에 경량 로깅 유틸이 있으며, 캐럿 기준 로깅 경로를 사용합니다.
- **Rule/Prompt 우선순위**: 캐럿 Rule Priority 적용으로 프롬프트/룰 관련 이벤트가 Cline과 다를 수 있습니다.
