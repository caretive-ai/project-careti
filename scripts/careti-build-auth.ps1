<# 
// CARETI MODIFICATION: Windows PowerShell build+auth script for Caret CLI.
#>
[CmdletBinding()]
param(
	[AllowNull()]
...
function Stop-CaretNodeProcesses {
	try {
		...
		foreach ($node in $nodes) {
			if ($node.CommandLine -and ($node.CommandLine -match "cline-core\\.js|careti-core\\.js|dist-standalone")) {
...
$LogPath = if ($LogPath) { $LogPath } else { Join-Path $buildRoot ("logs\\careti-build-auth-{0}.log" -f (Get-Date -Format "yyyyMMdd-HHmmss")) }
...
try {
	...
	try {
		...
		$sectionsSrc = Join-Path $buildRoot "careti-src\\core\\prompts\\sections"
		$sectionsDest = Join-Path $buildRoot "dist-standalone\\extension\\careti-src\\core\\prompts\\sections"
