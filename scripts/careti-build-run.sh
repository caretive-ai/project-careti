...
export CARET_FORCE_KILL="${CARET_FORCE_KILL:-1}" # CARETI MODIFICATION: ensure build copy can overwrite running binaries

pushd "$ROOT" >/dev/null
# CARETI MODIFICATION: default는 실행 중 인스턴스를 종료하고 빌드 (CARET_SKIP_KILL=1이면 보존)
if [ -n "${CARET_SKIP_KILL:-}" ]; then
  echo "[info] CARET_SKIP_KILL set; keeping existing host/core processes running during rebuild"
else
  pkill -f "careti-host|cline-host|cline-core|careti-core|/bin/caret" >/dev/null 2>&1 && \
...
npm run protos
npm run protos-go
# CARETI MODIFICATION: Node 버전 불일치로 인한 better-sqlite3 로딩 실패 방지
npm rebuild better-sqlite3
bash scripts/build-cli.sh
# CARETI MODIFICATION: standalone 빌드 산출물(cline-core.js) 포함
npm run compile-standalone
npm run postcompile-standalone
# CARETI MODIFICATION: ensure extension package.json exists for standalone core
