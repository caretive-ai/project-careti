...
async function generateStandaloneProtobusServiceSetup(protobusServices) {
	...
	for (const [name, def] of Object.entries(protobusServices)) {
		...
		for (const [rpcName, rpc] of Object.entries(def.service)) {
			imports.push(`import { ${rpcName} } from "@core/controller/${dir}/${rpcName}"`)
			// CARET MODIFICATION: Extract package from FQN (e.g., ".cline.EmptyRequest" → "cline.EmptyRequest")
			const requestFqn = rpc.requestType.type.fullName || ""
			const responseFqn = rpc.responseType.type.fullName || ""
			const requestType = requestFqn.startsWith(".") ? requestFqn.substring(1) : rpc.requestType.type.name
			const responseType = responseFqn.startsWith(".") ? responseFqn.substring(1) : rpc.responseType.type.name
			if (rpc.requestStream) {
				throw new Error("Request streaming is not supported")
			}
			if (rpc.responseStream) {
