...
export class OpenRouterHandler implements ApiHandler {
	...
	private ensureClient(): OpenAI {
		if (!this.client) {
			// CARET MODIFICATION: Merged upstream's lazy initialization with Caret's stability fix.
			// Prevents OpenAI SDK from looking for OPENAI_API_KEY env var by providing a dummy key.
			...
			try {
				this.client = new OpenAI({
					...
					apiKey: apiKey,
					defaultHeaders: {
						"HTTP-Referer": "https://caret.team", // CARET MODIFICATION: Use Caret branding
						"X-Title": "Caret", // CARET MODIFICATION: Use Caret branding
					},
				})
			} catch (error: any) {
