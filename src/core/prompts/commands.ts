...
When creating a new agent context rule file, you should NOT overwrite or alter an existing agent context rule file. To create the agent context rule file you MUST use the new_rule tool. The new_rule tool can be used in either of the PLAN or ACT modes.

The new_rule tool is defined below:

Description:
Your task is to create a new agent context rule file which includes guidelines on how to approach developing code in tandem with the user, which can be either project specific or cover more global rules. This includes but is not limited to: desired conversational style, favorite project dependencies, coding styles, naming conventions, architectural choices, ui/ux preferences, etc.
The agent context rule file must be formatted as markdown and be a '.md' file. The name of the file you generate must be as succinct as possible and be encompassing the main overarching concept of the rules you added to the file (e.g., 'memory-bank.md' or 'project-overview.md').

Parameters:
- Path: (required) The path of the file to write to (relative to the current working directory). This will be the agent context rule file you create, and it must be placed inside the ${BRAND_RULES_DIR} top-level directory (create this if it doesn't exist). The filename created CANNOT be "${DEFAULT_IGNORE_MARKDOWN}". For filenames, use hyphens ("-") instead of underscores ("_") to separate words.
- Content: (required) The content to write to the file. ALWAYS provide the COMPLETE intended content of the file, without any truncation or omissions. You MUST include ALL parts of the file, even if they haven't been modified. The content for the agent context rule file MUST be created according to the following instructions:
...
Usage:
<new_rule>
<path>${BRAND_RULES_DIR}/{file name}.md</path>
<content>Agent context rule file content here</content>
</new_rule>

Example:
<new_rule>
<path>${BRAND_RULES_DIR}/project-preferences.md</path>
<content>
## Brief overview
...
Below is the user's input when they indicated that they wanted to create a new agent context rule file.
</explicit_instructions>\n
`
...
// CARETI MODIFICATION: Added for Cline v3.49.1 parity (explain-changes feature)
