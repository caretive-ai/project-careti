...
const convertAssetToImagePath = async (assetUri: string): Promise<string> => {
	...
	console.log("Converting asset URI", { assetUri })

	// Check if we have window template images injected by CaretProviderWrapper first
	if (assetUri.includes("caret.png") || assetUri.includes("caret_profile.png")) {
		if ((window as any).templateImage_caret) {
			console.log("Found window.templateImage_caret")
			return (window as any).templateImage_caret
		}
		if ((window as any).personaProfile) {
			console.log("Found window.personaProfile")
	...
	if (assetUri.includes("caret_thinking.png")) {
		...
		if ((window as any).personaThinking) {
			console.log("Found window.personaThinking")
			return (window as any).personaThinking
		}
	}

	// Try window variables for all characters
	const characterMap = {
		"sarang.png": "templateImage_sarang",
		"sarang_thinking.png": "templateImage_sarangthinking",
		"sarang_illust.png": "templateImage_sarangillust",
		"ichika.png": "templateImage_ichika",
		"ichika_thinking.png": "templateImage_ichikathinking",
		"ichika_illust.png": "templateImage_ichikaillust",
		"cyan.png": "templateImage_cyan",
		"cyan_thinking.png": "templateImage_cyanthinking",
		"cyan_illust.png": "templateImage_cyanillust",
		"ubuntu.png": "templateImage_ubuntu",
		"ubuntu_thinking.png": "templateImage_ubuntuthinking",
		"ubuntu_illust.png": "templateImage_ubuntuillust",
		"caret_illust.png": "templateImage_caretillust",
	}

	for (const [filename, windowVar] of Object.entries(characterMap)) {
		if (assetUri.includes(filename)) {
			const windowImage = (window as any)[windowVar]
			if (windowImage) {
				console.log(`Found window variable: ${windowVar}`)
				return windowImage
			}
		}
	}

	// Convert asset:// to actual file path for Vite dev server
	if (assetUri.startsWith("asset:/assets/template_characters/")) {
		const fileName = assetUri.replace("asset:/assets/template_characters/", "")
		try {
			// Use Vite's static asset handling
			const imageUrl = new URL(`../../../../assets/template_characters/${fileName}`, import.meta.url).href
			console.log("Converted asset URI to Vite path", { assetUri, imageUrl })
			return imageUrl
		} catch (error) {
			console.error("Failed to convert asset URI with Vite, trying static path", { assetUri, error })
			// Fallback to static path
			const staticImageUrl = `/assets/template_characters/${fileName}`
	...
	console.warn("No conversion available for asset URI, using fallback", { assetUri, windowProps })
...
// Hook to handle asset:// URI conversion using the updated logic
const useAssetImage = (assetUri: string) => {
	...
	useEffect(() => {
		...
		convertAssetToImagePath(assetUri)
			.then(setImageUrl)
			.catch(() => {
...
export const PersonaTemplateSelector: React.FC<PersonaTemplateSelectorProps> = ({ isOpen, onClose, onSelectPersona }) => {
	...
	useEffect(() => {
		const defaultChar = templateCharacters.find((c) => c.isDefault) || templateCharacters[0]
		if (defaultChar) {
	...
	const handleSelect = async (character: any) => {
		...
		try {
			...
			const normalBase64 = await convertAssetToImagePath(character.avatarUri || "")
			const thinkingBase64 = await convertAssetToImagePath(character.thinkingAvatarUri || "")
