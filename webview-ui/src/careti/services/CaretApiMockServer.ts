// CARETI MODIFICATION: Mock CaretAPI Server for testing and development
// Simulates real Careti API responses with realistic data
...
/**
 * Mock Careti API Server - simulates real API behavior
...
export class CaretApiMockServer {
	...
	private constructor() {
		...
		this.mockUser = {
			id: "careti-user-mock-123",
			email: "test@careti.ai",
			displayName: "Careti Test User",
	 ...
	 * This would be called by CaretiApiProvider, but we're just logging here
	 */
	...
	public simulateError(errorType: "network" | "auth" | "rate_limit" | "server"): never {
		const errors = {
			network: new Error("Network error: Unable to reach Careti API"),
