// CARETI MODIFICATION: Centralized Careti settings to minimize Cline code changes
// All Careti-specific features grouped in one place

import { getCurrentFeatureConfig } from "./FeatureConfig"
import type { CaretModeSystem } from "./ModeSystem"

/**
 * Centralized Careti settings interface
 * All Careti-specific features are grouped here to minimize ExtensionState modifications
 */
export interface CaretSettings {
	/** Global Careti brand mode (careti/cline) */
...
/**
 * Helper functions for Careti settings
 */
export const CaretSettingsUtils = {
	...
	isFeatureEnabled(settings: CaretSettings | undefined, _feature: string): boolean {
		...
		return settings?.modeSystem === "careti"
	},
}
