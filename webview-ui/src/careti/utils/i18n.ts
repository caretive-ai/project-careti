// 글로벌 다국어/국제화 유틸 - Careti 전용 버전 (JSON 파일 기반)
// CARETI MODIFICATION: Integrated performance monitoring

// CARETI MODIFICATION: 언어 변환 로직 import
import { getLanguageKey, type LanguageDisplay, LLM_TO_UI_LANGUAGE_MAP } from "@shared/Languages"
// CARETI MODIFICATION: URL constants for template variable processing
...
// CARETI MODIFICATION: Sovereign Cloud 언어 추가 (fr, de, ru)
...
// CARETI MODIFICATION: Announcement component translations
...
// CARETI MODIFICATION: Removed urls dependency for cline-latest compatibility
...
let translations = {
	...
	// CARETI MODIFICATION: Sovereign Cloud 언어 (fr, de, ru)
	fr: {
		common: frCommon,
		welcome: frWelcome,
		persona: frPersona,
...
// CARETI MODIFICATION: 테스트 목적으로 translations 객체를 설정하는 함수 추가
...
// CARETI MODIFICATION: 웹뷰 전역 UI 언어 관리
...
// CARETI MODIFICATION: ExtensionState에서 언어를 가져오는 콜백 함수
...
// CARETI MODIFICATION: getCurrentLanguage를 getInternalCurrentLanguage로 대체
export const getCurrentLanguage = (): SupportedLanguage => {
	return getInternalCurrentLanguage()
}

// CARETI MODIFICATION: Helper function to get nested value using dot notation, with flat key support
...
// CARETI MODIFICATION: 한글 조사 규칙 매핑
...
// CARETI MODIFICATION: 받침 검사 헬퍼 함수 (export 추가)
...
// CARETI MODIFICATION: Enhanced template processing with performance monitoring
const replaceTemplateVariables = (
	...
	try {
		...
		// CARETI MODIFICATION: 한글 조사 처리 및 브랜드 참조 처리
		...
		// CARETI MODIFICATION: URL template variables replacement
		result = result
			...
			.replace(/\{\{caretiGithub\}\}/g, CARET_URLS.GITHUB_REPOSITORY)
			.replace(/\{\{caretiService\}\}/g, CARET_URLS.CARET_SERVICE)
...
// CARETI MODIFICATION: Enhanced translation function with performance monitoring and caching
export const t = (
	key: string,
	namespace: string = "common",
...
// CARETI MODIFICATION: Enhanced tWithLang with performance monitoring
...
// CARETI MODIFICATION: Integrated lazy loading system
