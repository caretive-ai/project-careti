// CARET MODIFICATION: Refactored to use modular provider components and integrated Caret-specific customizations.
// - Applied i18n for labels.
// - Reordered providers to prioritize Caret, Gemini, OpenAI, Anthropic.
// - Replaced ClineProvider with CaretProvider.
// - Integrated responsive design hooks.
...
import { useResponsive } from "@/hooks/useResponsive" // CARET MODIFICATION: Added for responsive layout
import { CaretProvider } from "./providers/CaretProvider" // CARET MODIFICATION: Changed from ClineProvider
...
const ApiOptions = ({ showModelOptions, apiErrorMessage, modelIdErrorMessage, isPopup, currentMode }: ApiOptionsProps) => {
	// Using Caret's i18n utility instead of react-i18next
	const { isSmallScreen } = useResponsive()
	// Use full context state for immediate save payload
	const { apiConfiguration } = useExtensionState()

	// CARET MODIFICATION: 임시 디버깅 - currentMode가 undefined인 경우 fallback
	const effectiveCurrentMode = currentMode || "agent"
	console.log("[DEBUG] 🔧 ApiOptions currentMode:", {
		received: currentMode,
		effective: effectiveCurrentMode,
	})

	const { selectedProvider } = normalizeApiConfiguration(apiConfiguration, effectiveCurrentMode)
		...
		<div style={{ display: "flex", flexDirection: "column", gap: 5, marginBottom: isPopup ? -10 : 0 }}>
			<DropdownContainer className="dropdown-container">
				<label htmlFor="api-provider">
					<span style={{ fontWeight: 500 }}>{t("settings.apiOptions.apiProviderLabel", "settings")}</span>
				</label>
				<VSCodeDropdown
					id="api-provider"
					...
					onChange={(e: any) => {
						...
						console.log("[DEBUG] 🎯 Provider dropdown onChange triggered:", {
			...
			{apiConfiguration && selectedProvider === "caret" && (
				<CaretProvider showModelOptions={showModelOptions} isPopup={isPopup} currentMode={currentMode} />
			)}
