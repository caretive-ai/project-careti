...
export function normalizeApiConfiguration(
	...
	const provider =
		(currentMode === "plan" ? apiConfiguration?.planModeApiProvider : apiConfiguration?.actModeApiProvider) || "gemini"
	...
	switch (provider) {
		...
		case "cline":
		case "caret":
			// CARET MODIFICATION: Caret provider uses Gemini models by default
			const caretModelId = modelId || geminiDefaultModelId
			let caretSelectedModelInfo: ModelInfo | undefined
			if (caretModelId in geminiModels) {
				caretSelectedModelInfo = geminiModels[caretModelId as keyof typeof geminiModels]
			} else {
			...
			return {
				...
				selectedModelInfo: caretSelectedModelInfo,
			}
		case "openai":
			const openAiModelId =
		...
		default:
			// CARET MODIFICATION: Default to Gemini provider to prevent auto-switching
			const defaultModelId = geminiDefaultModelId
			return {
				selectedProvider: "gemini",
				selectedModelId: defaultModelId,
				selectedModelInfo: geminiModels[defaultModelId],
			}
	}
}
...
export async function syncModeConfigurations(
	...
	switch (apiProvider) {
		...
		case "caret":
			// CARET MODIFICATION: Caret provider uses standard apiModelId
