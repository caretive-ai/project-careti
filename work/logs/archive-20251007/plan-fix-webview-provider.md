# 작업 계획: `WebviewProvider.ts` 생성자 수정

## 1. 목표
`WebviewProvider`가 `Controller`를 생성할 때 `HostProvider`로부터 `WorkspaceRootManager`를 가져와 전달하도록 수정하여, `Controller` 생성자 관련 타입 에러를 최종적으로 해결한다.
...
```xml
<replace_in_file>
<path>src/core/webview/index.ts</path>
<diff>
------- SEARCH
